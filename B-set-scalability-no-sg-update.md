...
- `#3-workaround-prose-reconciled-with-the-console-pin-hazard` `IN-REVIEW` developer — "Reconciliation only; status, severity and body deliberately unchanged — this entry records what is now known about this ticket's **Workaround** prose without rewriting it, per `B-console-command-sg-cvar-pin-freezes-scalability`'s reconciliation ask. Two corrections to that prose, in opposite directions. (1) `system.console_command {command:'scalability <N>'}` does NOT write 'at console priority' and does NOT 'land over any existing pin': the aggregate form is handled by `Scalability::ProcessCommand`'s numeric branch (`Scalability.cpp:827-834`) -> `SetQualityLevels` -> `SetQualityLevelCVar`, which ends in `Set(DesiredValue, ECVF_SetByScalability)` (`:907`) — the same lowest settable priority `performance.set_scalability` itself uses. `E-scalability-console-escape-hatch-misleading` already flagged this and it stands. (2) The harmlessness cuts the other way too, and is worth stating plainly: precisely because it writes at Scalability priority, the aggregate form CANNOT pin a group above the editor's own Scalability panel, so it is safe on that axis — unlike the per-CVar `sg.<Group> N` console form, which writes at `ECVF_SetByConsole` (`IConsoleManager.h:187`) and takes the panel away from the human user for the rest of the session. That is the defect `B-console-command-sg-cvar-pin-freezes-scalability` fixed. **Net effect on this ticket's advice:** the `scalability <N>` line stays safe to run and is NOT refused by the guard added there — only a leading `sg.` token is — but it should no longer be described as a way to beat a pin, because it is not. A caller who genuinely must beat one now uses `force: true` on `system.console_command` / `editor.console_command`, which is where the deliberate escape hatch was moved. No code, docs or status change made here."
