...
Confirmed by reading `Source/PinWright/Private/Handlers/Asset/AssetDumpHandler.h`
...
- `#5-repoint-citations-after-module-rename` `DONE` reporter — Citation maintenance only; **no claim in this ticket changes and the status is untouched**. The plugin module directory was renamed `Source/EditorAutomationRpcGateway/` → `Source/PinWright/` (plugin commit `8962f163`), and `Source/EditorAutomationRpcGatewayTests/` was folded into `Source/PinWright/Private/Tests/`, so every citation under the old root was an **unresolvable path** a fixer could not open — not a stale line number. 1 body citation repointed in place and verified against plugin HEAD `ef8a1f1b`. 1 citation sits in history rows and is left verbatim per the append-only rule. The one citation is in history row `#3` and stays verbatim; it has **no successor**. `Handlers/DataTable/DataTableDescribeHandler.cpp` was deleted in plugin `e0d0fe2c` along with its test, and `data_table.describe` greps to zero at HEAD. **So one of the four RPCs `#3` reports shipping has since been culled** — the `data_table` P1 parity gap is reopened in fact but not on the board. The other three verify: `static_mesh.describe` (`Handlers/Asset/StaticMeshDescribeHandler.cpp:14`), `audio.authoring.describe_sound_wave` (`Handlers/Audio/AudioAuthoringHandler.cpp:2922`), `animation.describe_sequence` (`Handlers/Animation/AnimSequenceDescribeHandler.cpp:10`). The builder the deleted handler called survives as `Handlers/DataTable/DataTableDumpBuilder.h:12` `BuildDataTableJson`; the nearest live read verb is `data_table.list_rows` (`Handlers/DataTable/DataTableAuthoringHandler.cpp:427`). Sweep-wide record, including every case that could not be repointed: `E-module-rename-citation-sweep`.
