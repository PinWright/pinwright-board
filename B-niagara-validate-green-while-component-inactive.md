...
- `#5-autoactivate-probe-never-auto-activated` `IN-REVIEW` developer — "Fixture correction only. The disclosure code `#4` shipped is unchanged and was never wrong. Two of the three tests in `Tests/Actor/TestComponentAutoActivateDisclosure.cpp` failed on their own precondition — `Expected 'the probe starts auto-activating' to be true` at lines 141 and 266 — because the premise the whole file was written on is false. `bAutoActivate` is NOT a universal CDO default of true: `UActorComponent` never assigns the bitfield and `USceneComponent` sets it to `false` outright (`SceneComponent.cpp:128`), so the entire `UStaticMeshComponent` chain — the old `AStaticMeshActor` probe — starts with the flag already OFF. Only classes that opt in start true (`UCameraComponent` at `CameraComponent.cpp:95`, `USkeletalMeshComponent`, `UAudioComponent`, `UParticleSystemComponent`, `UNiagaraComponent`, the movement components), which is why the field defect was a Niagara one. The probe is now an `ACameraActor` and its `UCameraComponent`: Engine module only, no asset to load, already a proven spawn target in this suite (`Tests/Sequencer/TestSequencerRepointActor.cpp`), and its `PostEditChangeProperty` only refreshes the editor visual representation. The precondition was NOT weakened or deleted — it is what stops the test passing on a probe that never auto-activated, and it now holds for the right reason. **`actor.add_component` did not fail for a reason worth recording:** it has NO precondition at all, and its old `componentType: SceneComponent` also starts with the flag off, so `bAutoActivate:false` wrote the value the component would have had anyway and every assertion after it held whether or not the write ever landed — green while measuring nothing. It now asks for `CameraComponent` and states the same premise off the CDO (`GetDefault<UCameraComponent>()->bAutoActivate`), the only place it can be taken, since its component does not exist until the verb runs. The file's `THE DEFECT` paragraph, which asserted the false blanket default and is what produced the broken fixture, is corrected, and the fixture choice is documented in the header. Handlers untouched: `DiscloseAutoActivateDisabled` (`ComponentHandler.cpp`) gates on the request naming the flag and then reads it back off the component; `property.set` gates on a single-segment path. Both are correct, and the failing runs are the evidence — every assertion AFTER the precondition passed even on the wrong fixture. **One honesty gap is left open deliberately, because it is a behaviour change and not a fixture one:** the shipped text in `Utils/AutoActivateDisclosure.h` states `bAutoActivate defaults to true, so the override is saved into the level`, which is false for any class that defaults it false — writing `false` onto a `UStaticMeshComponent` matches the CDO, is not serialised as an override, and still emits that warning. A CDO-aware gate (`Cast<UActorComponent>(Component->GetArchetype())`) would fix it, but it needs its own decision because the warning's other claim — 'this component will not start' — is true regardless of the CDO. NOT COMPILED and NOT RUN: the wave owner builds and runs the suite. Files changed: `Source/PinWright/Private/Tests/Actor/TestComponentAutoActivateDisclosure.cpp` only."
