...
- `B-compile-mgir-landscape-consumers-stale` — **the door this fix did not close.** Same
  defect, reached through `material.compile_mgir`. See `#4` below.
...
- `#4-verdict-was-verb-scoped-not-defect-scoped` `DONE` integrator — Re-verified independently and the fix **holds for this verb**: on the running build, wiring pure magenta into `M_DotaTerrain`'s `BaseColor` and stopping there moved a fixed frame **0.02%** (noise floor 0.18%), and the identical edit followed by `compile_material` alone moved it **93.82%** with `consumerRefresh {consumersFound:1, consumersRefreshed:1, subObjectsRefreshed:64, complete:true, refreshed:["DotaTerrain"]}`. Status stays `DONE`. **But the `DONE` verdict was scoped to the verb, not to the defect, and was read as the latter.** An audit of every material verb found `RefreshLandscapeConsumers` had exactly **one** call site across 99 registered methods, and `material.compile_mgir` — the bulk path the wiki recommends *over* the imperative one — still reached the same master with a bare `PostEditChange()`, no update context, no landscape rebuild, and a save. Measured pre-fix at **0.79%**. Filed and fixed as `B-compile-mgir-landscape-consumers-stale` (`f92a4d32`). Correction for the record: the acceptance criterion here ("the round-trip becomes unnecessary for a graph edit") was met, but "the round-trip is no longer needed" was published in `wiki-src/material.authoring.md` **without naming which verbs push**, so a caller using MGIR or an incremental mutator inherited a claim that did not cover them. That doc now names both lists. The transferable lesson is in `Docs/rpc-design.md` §5b: enumerate every door into the same engine state before calling a defect of this class closed.
