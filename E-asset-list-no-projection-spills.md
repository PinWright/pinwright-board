...
encounters: 3
lastSeen: 2026-07-04T19:35:26.6229648+03:00
---
...
(`Source/PinWright/Private/Handlers/Asset/AssetManageHandler.cpp:891-901`). The
...
## History
- `#4-implement-namesonly-fields-projection` `IN-REVIEW` developer — Implemented the per-row projection lever, mirroring the landed `actor.list` template. `asset.list` now parses the shared `FHandlerContext::ReadFieldProjection({name, path})` and conditionally emits each row field, so `namesOnly:true` (or `names_only`) returns just `name`+`path`, and a case-insensitive `fields:[...]` allow-list (valid keys `name`/`path`/`class`/`packagePath`/`tags`; `fields` wins over `namesOnly`) returns exactly the requested keys — the verbose per-row `tags` array is dropped unless kept, which is what busted the inline budget on the enumerate-to-pick shape. Default (no projection) output is byte-identical to the prior five-key shape. Registration gains two `RPC_PARAM_OPT`s (`fields`, `namesOnly`) and the summary now steers callers to them. Files: `Source/PinWright/Private/Handlers/Asset/AssetManageHandler.cpp` (registration params + summary at :709-718, projection resolve + per-field emit at the row-build loop ~:900-940), `docs/wiki-src/asset.md` (the `### asset.list` H3 now documents `namesOnly`/`fields` and drops the stale "not available today" note). Regression test: `Source/PinWright/Private/Tests/Assets/TestAssetHandlers.cpp` `FAssetListFieldProjectionTest` (`PinWright.asset.list.FieldProjection`) — builds an in-code scratch UMaterial registered under `/Game/PinWrightTests` via `FAssetRegistryModule::AssetCreated`, lists that folder three ways, and asserts default keeps all five keys, `namesOnly:true` keeps only `name`+`path` (drops `class`/`packagePath`/`tags`), and mixed-case `fields:["Name"]` keeps only `name` (case-insensitive, drops the rest); reverting the projection keeps all five keys and fails the drop assertions. Also refreshed the drifted in-body cite `866-883`->`891-901`.
- `#3-additional-blueprint-class-smallest-page` `OPEN` reporter — Additional evidence, broader scope + tightest page yet: the spill is not StaticMesh-specific and hits even an 8-row page. From a `drive.list_windows` asset-editor smoke-check audit (seed `drive.list_windows`, namespace `drive`, outcome **ergo** — the seed itself was clean: open/observe/close of `BP_Button_Parent` round-tripped and list_windows correctly showed 3->4->3 windows; the friction was entirely the incidental "pick a Blueprint asset to open" step, plus a graceful drive.observe overflow that correctly reported `omitted_count`). The doer's `asset.list {filter.class:Blueprint, pagination.limit:8}` — an already-paginated 8-row page — STILL spilled. Replay-confirmed at HEAD via `mcp__pinwright__call asset.list {filter:{class:Blueprint}, pagination:{limit:8}}` -> `{outputTooLong:true, message:"Response exceeds display limit (10998 chars, threshold 10000); full payload written to ...Saved/PinWright/HttpResponses/20260704T163310Z/<uuid>.json", file:{characters:10998, threshold:10000}}`. NEW ANGLE vs #1/#2 (both StaticMesh at `pagination.limit` 25/40, 47761/76218 chars): (a) a SECOND asset class — Blueprint — spills too, so the missing per-row projection is class-agnostic, not a StaticMesh-`tags` quirk; (b) the smallest page datapoint yet — even 8 rows (~1375 chars/row) bust the 10000 budget (marginally, ~10% over), so the heavy per-row payload drives the spill independent of class/count. Corrected source cite for HEAD: the per-row shape (name+path+class+packagePath + the verbose `tags` array iterating `Asset.TagsAndValues`) is now built at `AssetManageHandler.cpp:891-901` (the earlier `866-883` cite has drifted — that range is now the pagination/sub-folder code). Same method, same root cause (no `namesOnly`/`fields` projection lever to drop the `tags` weight) as this ticket — a new occurrence, not a new file. Bumped `encounters`->3, `lastSeen`.
