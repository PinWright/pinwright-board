...
- Inspect the Select-node decompile path (likely in the BPIR emitter under `Source/EditorAutomationRpcGateway/Private/Bpir/` — search for `select` instruction emission / `KCST_Select` / `UK2Node_Select`).
