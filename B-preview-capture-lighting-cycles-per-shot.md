...
**Verdict: PARTLY TRUE.** Source proves that `FScopedPreviewSceneRig` was constructed and restored once per `CaptureEditorViewportToPng` call while the wire docs promised one scope per set; source does not prove that this churn, rather than the per-shot viewport resize or another persistent renderer state, caused the measured period-4 shading cycle.

`CaptureCameraPoses` now validates and owns one preview-rig guard around warm-up, every real shot, and the final control, while individual captures only measure the rig in force; after the set exits, its component/profile/config restore receipt is copied to every reported capture. Non-time-driven sets retain pose 0 just long enough to take one discarded same-pose control and publish `poseSet.poseRepeatability {meanAbsDelta,maxDelta,changedPixelFraction}`; time-driven sets publish why that control was not measured rather than replaying a potentially non-idempotent simulation setter. Warm-up now publishes the same pixel-level difference signals beside mean luminance. The discarded warm-up uses a GUID-suffixed name, while coverage references and the repeatability control use the screenshot helper's collision-checked generated-name path, so cleanup cannot delete a caller-owned artifact.

Files changed: `Source/PinWright/Private/Handlers/Render/PoseListCapture.{h,cpp}`, `Source/PinWright/Private/Handlers/Render/PreviewViewportCaptureUtils.{h,cpp}`, `Source/PinWright/Private/Handlers/Render/PreviewSceneRig.{h,cpp}`, `Source/PinWright/Private/Handlers/Render/FlatRegionStats.{h,cpp}`, `Source/PinWright/Private/Utils/ScreenshotUtils.{h,cpp}`, `Source/PinWright/Private/Tests/Render/TestPoseListCaptureStability.cpp`, `Source/PinWright/Private/Tests/Render/TestCaptureVerbParameterParity.cpp`, `Source/PinWright/Private/Tests/Render/TestPoseListCaptureSubjectTime.cpp`, `Source/PinWright/Private/Tests/Render/TestAnimationPreviewSubjects.cpp`, `Source/PinWright/Private/Tests/Render/TestAssetPreviewSubjects.cpp`, `Docs/wiki-src/render.md`, `Docs/wiki-src/render.preview-scene-rig.md`, and `Docs/wiki-src/camera.md`. Tests: `PinWright.render.capture.PixelConvergenceDetectsLocalChange`, `PinWright.render.pose_list.PoseRepeatabilityEvidence`, `PinWright.render.pose_list.PreviewRigScopedOnce`, `PinWright.render.parameter_parity.PoseListSetFieldsAreReachableFromTheWire`, and `PinWright.render.pose_list.WarmupFrameIsNotOneOfTheShots`.

Deliberately not changed: `SetFixedViewportSize` still scopes each individual capture because its causal role is unproven and hoisting it changes live viewport/RHI lifetime; no stability failure gate was added before runtime data establishes a defensible threshold; time-driven subjects are not rewound for the control. No build, editor run, capture, or automation run was performed under this worker brief.
