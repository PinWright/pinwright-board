...
- `#2-effective-enabled-folded-from-walked-ancestors` `IN-REVIEW` developer — Fixed via the general ancestor fold rather than a CommonUI-specific predicate, in the new shared helper `Plugins/PinWright/Source/PinWright/Private/Handlers/Drive/DriveElementFactory.{h,cpp}` now used by both `MakeElement` producers (`Handlers/Drive/DriveLiveResolver.cpp`, `Handlers/Drive/DriveEditorChrome.cpp`): `WalkAndCollect` threads a `FAncestorState` whose `ForChildrenOf` applies `bAncestorsEnabled && Widget->IsEnabled()`, mirroring `SWidget::ShouldBeEnabled(bParentEnabled)` (`SlateCore/Public/Widgets/SWidget.h:1724-1727`), and `Element.bEnabled` is that folded value. `drive.expect widget_enabled` (`DriveConditionEval.cpp:166-180`) and the action gate (`DriveActionCommon.cpp:218`) read the same field and inherit it. **No CommonUI dependency was added, and none is needed for the reported case.** `#1` reads `UCommonButtonBase::SetIsEnabled` as never writing the Slate attribute; it does write it — `Super::SetIsEnabled(bInIsEnabled)` reaches `UWidget::SetIsEnabled`, which does `GetCachedWidget()->SetEnabled(bInIsEnabled)` (`Runtime/UMG/Private/Components/Widget.cpp:321-334`). What CommonUI suppresses is only the state BROADCAST, as its own comment says. The widget written is the button's OWN cached SWidget, an ANCESTOR of the inner `SCommonButton` leaf the walk reports — which is precisely why that leaf's local `IsEnabled()` stayed true, and why folding the walked ancestors recovers the state without reaching into CommonUI's private `bIsButtonEnabled`. This is also the generalization `#1` flagged as a possible High bump: every widget kind under a disabled ancestor was misreported, not only CommonUI-derived buttons. A hard CommonUI dependency was not available in any case — the main `PinWright` module deliberately keeps CommonUI in the gated `PinWrightCommonUI` sub-module. `interactable` is left alone, per this ticket. `docs/wiki-src/drive.md` now documents `enabled` as effective ancestor-folded state and records that `widget_enabled` is usable as a gate. Regression tests in `Source/PinWright/Private/Tests/Drive/TestDriveElementFactory.cpp`: `PinWright.drive.element_factory.DisabledAncestorDisablesChild` drives the REAL editor-chrome walk over a fixture window holding a disabled border around a labelled text leaf; `PinWright.drive.element_factory.AncestorStateFoldsVisibilityAndEnabled` pins the fold with no window. Counterfactual: revert `FillStateAndGeometry` to `Element.bEnabled = SlateWidget->IsEnabled()` and `DisabledAncestorDisablesChild`'s `TestFalse("a child of a disabled ancestor reads enabled:false")` fails — the test asserts anti-vacuously that the leaf's own `IsEnabled()` is still true and that a control leaf under an enabled ancestor still reads enabled, so the false can only come from the fold.
