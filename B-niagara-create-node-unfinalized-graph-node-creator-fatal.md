...
- `#6-live-verification` `DONE` developer — Live verification on a running editor (pid 27156, `PDS.uproject`, UE 5.8), against `/Game/PinWrightTests/NS_LiveVerify_Probe`. Ran both declared rejection shapes and checked liveness after each with a follow-up RPC plus a process check, because the pre-fix death arrived ~10 s AFTER a clean typed error. (1) `niagara.graph.create_node {assetPath:"/Game/PinWrightTests/NS_LiveVerify_Probe", nodeClass:"NiagaraNodeEmitter", x:100, y:100}` → `[UNSUPPORTED_NODE_CLASS]`; editor answering immediately and still answering 15 s later. (2) The path that runs INSIDE the creator's lifetime, which a class-check-only fix would not cover: `{nodeClass:"NiagaraNodeOp", x:200, y:200, payload:{opName:"ThisOpDoesNotExist_LiveVerify"}}` → `[INVALID_OP] Unknown Niagara op`; editor answering immediately and still answering 20 s later. `niagara.graph.get` before and after both calls is byte-identical (11,110-char payload, 5 nodes: `NiagaraNodeOutput_0`, `NiagaraNodeInput_0`, `NiagaraNodeInput_1`, `NiagaraNodeFunctionCall_0`, `NiagaraNodeOutput_1`) — net-zero as documented, so the `INVALID_OP` path really does finalize-then-remove rather than leaking a node. `PDS.log` over the window carries only the two `LogPinWrightSubsystem: Warning: Automation request failed` lines and a benign `LogUObjectGlobals: Warning: Failed to find object 'Class NiagaraNodeEmitter'` from short-name resolution; zero `Assertion failed`, zero `appError`, zero callstacks. Second independent confirmation of `#5`, on a different asset and a different editor process. Status left `DONE` — unchanged by this pass.
