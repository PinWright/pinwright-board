...
- `#5-repoint-citations-after-module-rename` `DONE` reporter — Citation maintenance only; **no claim in this ticket changes and the status is untouched**. The plugin module directory was renamed `Source/EditorAutomationRpcGateway/` → `Source/PinWright/` (plugin commit `8962f163`), and `Source/EditorAutomationRpcGatewayTests/` was folded into `Source/PinWright/Private/Tests/`, so every citation under the old root was an **unresolvable path** a fixer could not open — not a stale line number. No body citation was rewritten here. **Deliberately not repointed.** The body cites the pre-split monolith `Handlers/Animation/AnimationAuthoringHandler.cpp`, which does not exist at HEAD and does not appear in the plugin's visible history under that exact basename. **The split this ticket asks for is already done** and the ticket is `DONE`: `AnimationAuthoringHandler_Sequence.cpp`, `_BlendSpace.cpp` and `_AnimBlueprint.cpp` plus `AnimationAuthoringHelpers.{h,cpp}` all exist under `Source/PinWright/Private/Handlers/Animation/`. The old path is left as the historical description of the file that was split. Sweep-wide record, including every case that could not be repointed: `E-module-rename-citation-sweep`.
