...
**Fix:** preserve `peakLinear` as the pre-gain mono-layer peak after the generator, envelope and
layer `fx` chain, and publish a clearly named `peakLinearPostGain` after gain and equal-power pan
for the stereo layer contribution. Correct the two wiki sentences above so balance guidance uses
the post-gain value.

## Fix

The renderer intentionally measured the layer signal before `gainDb` and pan, but response assembly described that generator-stage value as the mix contribution; the fix keeps the legacy `peakLinear` value, publishes explicit `peakLinearPreGain` for the same pre-gain mono-layer peak after generator, envelope and layer FX, and publishes `peakLinearPostGain` as the maximum absolute sample across both stereo channels after gain and equal-power pan. Changed `Source/PinWright/Private/AudioGen/PwSynthDsp.{h,cpp}`, `Source/PinWright/Private/Handlers/Audio/AudioSynthGenerateHandler.cpp`, `Source/PinWright/Private/Tests/Media/TestPwSynthRender.cpp`, `Source/PinWright/Private/Tests/Media/TestAudioSynthGenerate.cpp`, and the synth wiki/cookbook; structural test ids are `PinWright.audio.synth.render.LayerReportPublishesPreAndPostGainPeaks` and `PinWright.audio.synth.generate.RendersCandidate`. Deliberately unchanged: the renderer's gain/pan/mix behavior, the legacy pre-gain signal, MetaSound rendering, unrelated handlers, and all existing dirty work; shared HTTP spill remains the sole response-size policy and only static verification was performed.
...
- `#6-layer-peak-stages` `IN-REVIEW` developer — Root cause fixed by retaining `peakLinear` as the pre-gain generator diagnostic and adding `peakLinearPreGain` plus `peakLinearPostGain`, measured after gain and pan for the actual layer contribution. Static test coverage is `PinWright.audio.synth.render.LayerReportPublishesPreAndPostGainPeaks`; the generate response test also asserts all three field names. The cookbook and namespace page now direct balance work to `render.layers[].peakLinearPostGain` and document the preserved pre-gain signal. No changes were made to DSP gain application, MetaSound rendering, or unrelated handlers.
- `#7-review-contract` `IN-REVIEW` developer — Review correction makes the pre-gain definition explicit (mono layer after generator, envelope and layer FX), verifies the post-gain value against the rendered stereo channels with non-center pan, and keeps exact target/report semantics. Static-only verification remains deliberate; no runtime or automation was run.
- `#8-response-budget` `IN-REVIEW` developer — Review correction temporarily explored local response compaction but it was declined after comparing the shared condensed HTTP spill policy; no bespoke compaction remains. Static-only verification remains deliberate; no runtime or automation was run.
- `#9-shared-spill-policy` `IN-REVIEW` developer — Verifier correction removes the obsolete handler-local 4,250-character pretty-writer budget and leaves shared `HttpResponseSpill` as the sole condensed 10,000-character response policy. The layer peak fields remain unchanged and inline in the handler contract; static diff inspection only.
