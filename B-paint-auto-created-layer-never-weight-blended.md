...
- `#2-blend-method-set-and-published` `IN-REVIEW` developer — Took **seat 1 in the form the ticket's own note prescribes**, and the packaging difference did not block it. Reproduction re-verified in THIS checkout first: `grep -rn "BlendMethod\|bNoWeightBlend\|FinalWeightBlending" Source/` outside `MaterialAuthoringHandler.cpp` returned exactly ONE hit — a comment naming the merge's shader pass — so nothing set it; and `[/Script/Landscape.LandscapeSettings]` in this project's `Config/DefaultEngine.ini` carries only `bShowDialogForAutomaticLayerCreation=False`, with no `TargetLayerDefaultBlendMethod` and no `DefaultLayerInfoObject` in `Config/`, `Saved/Config/` or `C:/UE_5.8/Engine/Config/`. So the compiled `None` was the live outcome here, exactly as filed. **`UE::Landscape::CreateTargetLayerInfo` was read and deliberately NOT called, and the reasoning is recorded in the code.** Its first act is `CreatePackage(InFilePath/InFileName)` and its last are `FAssetRegistryModule::AssetCreated` + `MarkPackageDirty` (`LandscapeUtils.cpp:286-320` on this host's 5.8): it *always* mints a `/Game` .uasset at a caller-supplied path. Adopting it wholesale would add a required path parameter, an asset on disk and a dirtied package to this verb's contract to fix a one-enum defect, and would contradict the private-per-landscape LayerInfo the verb's own `warnings[]` line documents. What WAS adopted is the half the factory uniquely holds and the ticket names as the disconnected control — `Settings->GetDefaultLayerInfoObject().LoadSynchronous()` and duplication (`:288-302`). New file-static `CreateLandscapePaintTargetLayerInfo` duplicates that template into the landscape actor (outer unchanged, `RF_Public | RF_Transactional` reapplied) when one is configured, and falls back to the previous `NewObject` when none is. **So a project that configures `DefaultLayerInfoObject` to a weight-blended template now gets it out of this verb**, which was the PinWright-specific half of the finding. **Blend method.** The auto-create then calls `SetBlendMethod(FinalWeightBlending | None, /*bInModify=*/false)` on 5.7+, `bNoWeightBlend = !x` below — the same shape `MaterialAuthoringHandler.cpp:3208-3214` already uses. It is applied when the caller asked OR when no template supplied one; a template's own method is left alone unless `weightBlended` is explicit, because that template IS the project's answer and overwriting it would re-disconnect the control this change just reconnected. New declared param `weightBlended` (`boolean`, default `true`). **It applies only on the auto-create path**, and supplying it for a layer that already has a LayerInfo produces a `warnings[]` line rather than a silent no-op OR a silent rewrite: that object is potentially a shared asset and a paint call must not change every landscape referencing it. **Published, measured rather than echoed:** `layerBlendMethod` (read back off the object the write bound to, whether this call made it, a template supplied it, or it was already there), `layersAffected[].blendMethod` per sibling — which is what tells "the paint did not displace this layer" apart from "this layer is not in the blend group at all" — and `layerInfoTemplatePath`, present only when the duplication branch ran so its absence means "constructed fresh", not "unknown". Two helpers, `LandscapeTargetLayerBlendMethodName` and `LandscapeTargetLayerIsWeightBlended`, carry the 5.7 split; the name helper returns an empty string for a null layer info so the field is omitted rather than guessed. **The `warnings[]` line the ticket asked for** fires whenever the painted layer is not weight-blended, naming the measured method and saying the merge will NOT reduce any sibling's weight. **In-band overstatement corrected:** the registered summary's `COSTS THE SIBLING LAYERS` is now `COSTS THE SIBLING LAYERS, BUT ONLY IF THEY ARE WEIGHT-BLENDED`, with the auto-create's new default stated; `Docs/wiki-src/landscape.md` gains the matching paragraph, the template/parameter precedence order, and `blendMethod` in the `layersAffected[]` table row. **Rider folded in, as the ticket directed:** the comment at the pre-5.5 `CreateLayerEditorSettingsFor` call no longer says the symbol is "a deprecated empty stub" on 5.5+ — a recursive grep over `C:/UE_5.8/Engine/Source` finds it nowhere, so it was removed outright and the comment now says so and says the call must stay inside the pre-5.5 branch. **Regression test** `PinWright.landscape.create_procedural_terrain.AutoCreatedLayerInfoIsWeightBlended`: paints a declared layer with no LayerInfo, asserts `layerInfoAutoCreated`, then reads the engine object DIRECTLY (`GetBlendMethod() == FinalWeightBlending` on 5.7+, `!bNoWeightBlend` below) rather than trusting the self-report, asserts `layerBlendMethod == "FinalWeightBlending"` and that `layerInfoTemplatePath` is ABSENT; then paints a second declared layer with `weightBlended: false` and asserts the object is `None`, the field reports `None`, and a `warnings[]` line says it takes no part in group normalisation. Both halves fail today — the first two on `None` and a missing field while `success` still reads `true`, which is the filed state. The test **skips through `PinWrightTestSkip::SkipAssertions`** when the host configures `DefaultLayerInfoObject`, because the branch under test is the constructed one and asserting `FinalWeightBlending` against a project's own template would be the host-dependent-fixture failure this suite refuses to reproduce. **Did NOT touch `RegionPaintLeavesOtherLayersOutsideRegion`** or the `CreateTargetLayerSettingsFor` registration one line above the changed code; that test's sibling assertion is on weighted texels OUTSIDE the region, where a sole weight-blended layer at 255 normalises to 255, so enabling the pass does not move it. **Consequence to record for the picker:** `B-paint-census-counts-texels-not-weight` was rated Medium on the argument that it is LATENT because nothing on this verb's default path is weight-blended. This change ends that — renormalisation now happens on the default path, so a sibling driven 255 -> 1 is a real precision gap in `otherLayerTexelsLostInRegion` today. Its own `#2` said it escalates to High when this ticket reaches DONE; it is not edited here, since this is IN-REVIEW and not that ticket's call to make. **NOT DONE, deliberately:** NOT COMPILED and NOT RUN — the orchestrator builds and runs. The duplication branch is NOT exercised by any test: it needs a configured project setting, this host sets none, and minting one would change the project's landscape configuration for every other agent in this checkout. `material.authoring.add_landscape_layer`'s inverted `noWeightBlend` default (`B-add-landscape-layer-noweightblend-inverted-default`) is a different handler and was not touched.
