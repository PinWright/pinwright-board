...
- `#3-compile-merges-authored-values-back` `IN-REVIEW` developer — `niagara.compile` now snapshots the rapid-iteration values before the request and merges them back after the wait, reporting `rapidIterationPreserved`. `PinWrightNiagara::FRapidIterationValueSnapshot` (`Source/PinWright/Private/Handlers/Niagara/NiagaraRapidIteration.{h,cpp}`) captures every store's values through `CopyParameterData` (LWC-safe) and `MergeBack` rewrites only entries that still exist at the same type and whose value the rebuild changed; `Source/PinWright/Private/Handlers/Niagara/NiagaraCompileHandler.cpp` captures before `RequestNiagaraCompile` and merges after the wait, because the store is rebuilt on the game thread during compile setup and the bake-out path empties and repopulates it again when the result lands. **Parameters the rebuild dropped are deliberately not re-added** — re-introducing a parameter the traversal no longer reaches makes the next `PrepareRapidIterationParameters` see a count mismatch and rebuild again (`Niagara/Private/NiagaraCommon.cpp:1669-1712`). The engine mechanism `#1` half-identified is now named: the system-script stores are *mirrors*, rebuilt from each system script's own graph traversal (`NiagaraEditor/Private/NiagaraScriptSource.cpp:623-632` removes everything that traversal does not reach, and the system scripts are registered with a null emitter so no emitter-stage or particle-stage constant survives it) and then overwritten from the emitter-stage stores with `bOnlyAdd=false` (`NiagaraCommon.cpp:1654-1666`) — which is exactly why `#1`'s emitter-scoped writes survive and the system-scoped ones do not, and why a deleted parameter comes back at the template default. The durable half of that is fixed at the source in `B-module-input-override-inert-runtime-uses-rapid-iteration-default` `#3`, where `set_module_input` writes the emitter-stage store too. Scope: systems only; an emitter-asset compile fans out to every loaded system using it and is not snapshotted. Docs: `docs/wiki-src/niagara.md` (`### niagara.compile`, and the batch-order paragraph that claimed a compile flatly "resets system-scope rapid-iteration values"). Tests: `Source/PinWright/Private/Tests/Niagara/TestNiagaraRapidIterationSync.cpp` — `PinWright.niagara.compile.RapidIterationSnapshotMergesAuthoredValues` seeds two constants, captures, reproduces a rebuild (overwrite one, drop the other), and asserts `MergeBack` returns 1, the authored value is back, the dropped parameter is still absent, and a second merge over an unchanged store returns 0; `PinWright.niagara.compile.ReportsRapidIterationPreserved` runs a real compile through the handler and asserts the field is present. Counterfactual: make `MergeBack` a no-op and the first test reads the rebuild's 1.0 instead of the authored 7.0 with a restored count of 0; make it a blanket `CopyParametersTo` and the dropped-parameter assertion fails. **Not compiled and not run.**
- `#4-merge-only-when-a-wait-ran` `IN-REVIEW` developer — Verifier follow-up on `#3`, which gated the merge on `bRequested` alone. That is wrong for `wait: false`: with rapid-iteration parameters baked out the prepare is deferred into `FNiagaraPrepareCompileTask::Tick` after the DDC search (`Niagara/Private/NiagaraAsyncCompile.cpp` ~740), so the merge ran BEFORE the rebuild it exists to undo, restored nothing, and published `rapidIterationPreserved: 0` — which the wiki defines as "the compile reproduced every value it found". A caller reading that number would conclude its values were safe at the exact moment they were destroyed. `Source/PinWright/Private/Handlers/Niagara/NiagaraCompileHandler.cpp` now gates on `System && bRequested && Wait.bWaited`, and when the merge does not run it emits `rapidIterationPreserved: null` plus `rapidIterationMerge` naming the reason — `skipped_no_wait`, `skipped_no_compile`, or `skipped_emitter_asset` (an emitter compile fans out to every loaded system using it and is not snapshotted). `merged` is the only value under which a count is published, so a count and a not-attempted can no longer be confused. Also from the same review: the merge is a real asset mutation and now runs inside an `FScopedTransaction`, cancelled when nothing was restored so an empty entry never reaches the undo stack. Test: `PinWright.niagara.compile.ReportsRapidIterationPreserved` now asserts the contract rather than the field — `rapidIterationMerge` is always present, a count exists only under `merged`, and a second `wait:false` call publishes no count and does not report `merged`. Counterfactual: restore the `bRequested`-only gate and the `wait:false` half fails, because the response carries a number again. Docs: `docs/wiki-src/niagara.md` `### niagara.compile` (new table rows and a paragraph on why the merge needs `wait: true`) and the batch-order paragraph. **Not compiled and not run.**
