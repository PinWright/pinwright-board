...
- `#3-attempt-abandoned` `OPEN` supervisor — attempt-failed: fuzz4's run was stopped (user fleet stop, 2026-07-13) mid-pipeline after implement; the fix never reached the plugin origin and the uncommitted diff was swept by the fleet hard-sync. Reopened for retry — the `#2` implementation notes (BuildSectionJson camera-cut branch, aspect version bump, regression test) remain valid as a starting point.
