...
- `#5-repoint-citations-after-module-rename` `DONE` reporter — Citation maintenance only; **no claim in this ticket changes and the status is untouched**. The plugin module directory was renamed `Source/EditorAutomationRpcGateway/` → `Source/PinWright/` (plugin commit `8962f163`), and `Source/EditorAutomationRpcGatewayTests/` was folded into `Source/PinWright/Private/Tests/`, so every citation under the old root was an **unresolvable path** a fixer could not open — not a stale line number. No body citation was rewritten here. 1 citation sits in history rows and is left verbatim per the append-only rule. The one citation is in a history row and stays verbatim; it has **no successor**. `Handlers/Debug/DebugHandler.cpp` was deleted in plugin `e0d0fe2c` (48 lines) and `debug.*` has zero registrations at HEAD — `Handlers/Debug/` now holds only Insights, Performance and TraceAnalysis. This ticket's own `#4` already records the removal. Sweep-wide record, including every case that could not be repointed: `E-module-rename-citation-sweep`.
