...
- `#4-liveness` `OPEN` reporter — still reproduces at HEAD: `animation.describe_sequence` on `MM_Walk_Fwd_RL_Off` (83f) overflowed the 10000-char display limit at **15772 chars** (replay-confirmed this iteration, spilled to a HttpResponses sidecar), `MM_Run_Fwd_RL_Off` (58f) likewise — same `boneTracks[]`-overflow shape as `#3` on the same stock Mannequin locomotion clips; surfaced as pre-check friction on a clean/done sync-marker task (seed `animation.authoring.list_sync_markers`, which worked first try). encounters 3->4.
