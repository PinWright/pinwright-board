...
- `#5-batch-fields-projection` `IN-REVIEW` developer — GO. Verified the defect is present in current source: `get_node_details_batch` (`BlueprintGraphInspectionHandler.cpp:695-763`) registered only `assetPath`/`nodeIds`/`graphName` and emitted `BuildNodeDetailsJson(TargetNode)` bare (`:749`) → the full per-node shape (pins-with-defaults + `nodeState`) always, no narrowing lever — strictly less controllable than its singular sibling `get_node_details` (`:490`/`:501` exposes `includePinDefaults`). Rejected the adversarial WONTFIX after source check: the "just use `get_nodes(namesOnly/fields)`" workaround is NOT equivalent because `get_nodes` has **no nodeId filter** (it iterates all `TargetGraph->Nodes` at `:422` with only a `limit` cap) → it returns the whole graph, not a targeted per-id subset; and the symmetric sibling `get_nodes` projection is already merged in HEAD, so accepting this is consistent precedent. Fix (additive — the full dump stays the default): added optional `fields`/`namesOnly`/`includePinDefaults` params to the batch, parsed via the shared `FHandlerContext::ReadFieldProjection` (`HandlerContext.cpp:327`) and applied inside the shared `BuildNodeDetailsJson` via a new per-key `FieldFilter` allow-list, so a targeted positions/wiring cross-check narrows the per-node `details` and stays inline instead of spilling. The per-item top-level `nodeId` echo is unaffected, so results stay correlatable even when `nodeId` is projected out of `details`. Files: `Plugins/PinWright/Source/PinWright/Private/Handlers/Blueprint/BlueprintGraphInspectionHandler.cpp`. Regression test: `PinWright.blueprint.graph.get_node_details_batch.FieldProjection` (`Tests/Blueprint/TestGetNodeDetailsBatchFieldProjection.cpp`) — builds an in-code transient Actor BP with a heavy PrintString node, asserts the no-projection call keeps `pins`/`nodeState` while `fields:["nodeId","x","y"]` and `namesOnly:true` drop them (reverting the lever flips the drop assertions + the param-registration checks to failure). Compiled clean (`EAContentExamples57Editor`), and the new test was executed headless → `Result={Success}`.
