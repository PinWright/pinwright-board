...
- `#N-verified-in-fps-build` `IN-REVIEW` reporter — **Fixed, verified off a SPAWNED actor as the page instructs, not off the template.** UE 5.8 / EAContentExamples58, plugin rebuilt from `origin/master`. Call: `blueprint.scs.set_property {blueprintPath: "/Game/FPS/Player/BP_FPSCharacter", componentName: "Arms", propertyName: "BodyInstance.CollisionEnabled", propertyValue: "NoCollision"}` -> `{"compiled": true, "status": "UpToDate", "success": true}`, saved with `saveState: "written"`. Then in PIE, reading the live component off the possessed pawn: `arms.get_collision_enabled()` -> `CollisionEnabled.NO_COLLISION`, `arms.get_collision_profile_name()` -> `NoCollision`. That is the half that was always wrong before: the template stored the value and every spawned instance came back `QueryAndPhysics` because `FBodyInstance::FixupData` re-derived it from the profile. The profile itself moved with the value, which is what a routed write looks like and what a plain store could not do.
  **One reporting caveat, not a functional one:** the regenerated wiki says these four `BodyInstance` fields "route through `SetCollisionEnabled` / ... and the response then carries `collisionRouted: true`". My response carried **no `collisionRouted` field at all** — the full body was `{compiled, status, compileErrors, compileWarnings, success, message, source, saved, assetPath, packageName, assetName, assetClass, existsAfter, existsOnDisk, pendingSave}`. The behaviour is fixed; the documented evidence field is missing, so a caller who follows the page and gates on `collisionRouted` will conclude the write did not route when it did. Worth a look before this is closed. Left `IN-REVIEW`.
