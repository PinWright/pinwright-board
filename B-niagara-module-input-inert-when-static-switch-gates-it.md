...
- `#2-implemented` `IN-REVIEW` developer — Both surfaces now name the gate. `Source/PinWright/Private/Handlers/Niagara/NiagaraEditTypes.{h,cpp}`: new `NiagaraEdit::FModuleInputGate` plus `ClassifyModuleInputReachability` (bulk, one switch-resolution pass per module) and `FindModuleInputGate` (single input, for the write path), and `MakeGatedByJson` so the readback and the write cannot describe a gate differently. The walk resolves every static switch's selected branch the way `BuildStaticSwitchInputs` does (caller-pin `DefaultValue` through `NiagaraStaticSwitch::DecodePinDefault`, else the graph's declared `GetStaticSwitchDefaultValue`), converts it to an option index, then walks each read of `Module.<name>` in the module script graph forward to its consumers — through `UNiagaraNodeReroute` / `UNiagaraNodeConvert` only, which is exactly the reroute+convert shape this ticket measured — and reports a gate only when *every* terminus is a switch option pin the switch does not select. The option index of a branch pin is derived from the `[option][output var]` pin layout `UNiagaraNodeStaticSwitch::AllocateDefaultPins` creates, so it needs no pin-name parsing; an unrecognised layout, an unresolvable switch, or any non-switch non-pass-through consumer yields `reachable: true` rather than a false alarm. `NiagaraDumpBuilder.cpp` `BuildModuleInputsJson` stamps `reachable` on every entry and `gatedBy: {switch, value, requiredValue, branchTaken}` when false; `NiagaraEditHandler.cpp`'s `niagara.set_module_input` response carries the same two fields after the write, so the inert write is visible at write time and the write is still kept (staging before flipping the switch is legitimate). `niagara_stack.json` 6->7 in `Source/PinWright/Private/Handlers/Asset/AssetDumpCache.cpp`; `docs/wiki-src/niagara.md` gained the `reachable` / `gatedBy` bullets under `### niagara.inspect` and `### niagara.set_module_input` — the two pages this ticket named as silent. Test: `PinWright.niagara.dump.ModuleInputStaticSwitchGate` (`Source/PinWright/Private/Tests/Niagara/TestNiagaraModuleInputReachability.cpp`) places the ticket's own `/Niagara/Modules/Update/SubUV/V2/SubUVAnimation` on a transient system's particle-update stack and asserts `Start Frame Range Override` reads `reachable:false` with `gatedBy.switch == "UseStartFrame"`, `value == false`, `requiredValue == true`, then flips the `UseStartFrame` caller pin (encoded through `NiagaraStaticSwitch::EncodePinDefault`) and asserts the same input flips to `reachable:true` with no `gatedBy`. Counterfactual: revert the `reachable`/`gatedBy` block in `NiagaraDumpBuilder.cpp` and `TryGetBoolField(TEXT("reachable"))` returns false on every entry, so the first assertion fails immediately — which is the reported defect, a green write and a readback that call a dead override configuration. The flip half additionally fails if the field is present but constant.
