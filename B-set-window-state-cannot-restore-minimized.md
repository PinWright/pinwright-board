...
- `#2-minimized-tolerant-selector-opt-in` `IN-REVIEW` developer — "Fixed the selector as an OPT-IN widening, plus the honest-refusal and measured-readback halves. Root cause confirmed in THIS checkout (plugin HEAD `d8f1bc32`, engine `C:/UE_5.8`; the editor was not running, so the reproduction is the static one — sufficient here because the defect is structural, not conditional): `ResolveSelectedWindow` (`DriveEditorChrome.cpp:223`) builds its whole candidate list from `GetAllVisibleWindowsOrdered` and short-circuits `NO_WINDOWS` at `:225` when it is empty. `#1`'s framing that the filter tests the WRONG PROPERTY is correct and was verified down to the platform layer: the engine predicate is `IsVisible() && !IsWindowMinimized()` (`SlateApplication.cpp:3794`, re-applied recursively at `:3803`), and `SWindow::IsVisible()` (`SWindow.cpp:1496`) forwards to the native window's `IsVisible()`, which on Windows returns the Show/Hide flag `bIsVisible` (`WindowsWindow.cpp:915-918`) and stays TRUE while the window is iconic. A minimized window is a fully valid, still-visible `SWindow` dropped SOLELY by the `!IsWindowMinimized()` clause; the `IsVisible()` half is legitimate and was KEPT, so a genuinely hidden window stays untargetable. FIX — exactly the fallback shape `#1` asked for, and the one that answers `B-list-windows-maximized-geometry` `#3`'s index-order objection verbatim: a new pure static `FDriveEditorChrome::AppendMinimizedTolerantWindowCandidates(Visible, Extra, Out)` copies the visible enumeration FIRST and unchanged, then appends each not-already-present extra (deduped by widget identity, not title), so index i of the result is index i of `GetAllVisibleWindowsOrdered` for every i that existed before and NO `window_index` changes meaning. `ResolveSelectedWindow` and `FDriveEditorChrome::ResolveWindow` take a new `bIncludeMinimizedWindows` that DEFAULTS TO FALSE; when true the extras come from the engine's public unfiltered `FSlateApplication::GetTopLevelWindows()` (`SlateApplication.h:1688`, inline `return SlateWindows;` — access verified public) filtered to `IsVisible() && IsWindowMinimized()`. SHARED-SELECTOR CALLERS AUDITED, all seven, one opts in: `BuildElementList` (drive observe/list_elements) NO, `ResolveHandle` (drive click/type/hover) NO, `CaptureWindow` (drive screenshot / Set-of-Mark) NO — all three read or act on a window's CONTENTS, which a minimized window does not have; `editor.frame_graph` (`EditorWindowHandlers.cpp:330`) NO; `editor.resize_window` (`:675`) NO, a minimized window will not visibly resize, the same reason it already refuses a maximized one; `editor.screenshot_window` (`:889`) NO, capturing an iconic window yields nothing; `editor.set_window_state` (`:828`) YES, the sole opt-in, because it acts on the window's own window-manager state rather than its contents. `FDriveEditorChrome::ListWindows` does not route through the resolver and was deliberately NOT widened, so `drive.list_windows` output is unchanged and `#1`'s second half (restoring the `minimized` field) is NOT done here — that half belongs with `B-list-windows-maximized-geometry`, which owns that code and is IN-REVIEW under another agent; only the now-false index-order clause in its `FDriveWindowInfo` comment (`DriveEditorChrome.h:40-45`) was corrected in place to record that the objection is answered. The profiling-verb `mainWindowMinimized` flag of `#1` is also not done. REFUSALS NOW DISTINGUISH THE TWO FAILURES, per the house rule that `NO_WINDOWS` against a window that demonstrably exists is a false statement: `NO_WINDOWS` and `WINDOW_NOT_FOUND` now name which set was searched ('minimized windows were searched too'), so neither is ever a true statement about a narrower list than the caller believes was consulted; and a new `WINDOW_STATE_NOT_CHANGEABLE` covers 'a window MATCHED but its state could not be changed', emitted when the resolved `SWindow` carries no native platform window — which makes `Restore`/`Maximize`/`Minimize` literal no-ops (`SWindow.cpp:1754-1776` each guard on `NativeWindow.IsValid()`) and would otherwise have been reported as a fake success. MEASURED STATE IS NOW PUBLISHED: the response adds `state`, read back FROM THE WINDOW after the call as one of restored/maximized/minimized, alongside the existing `requestedState`, plus `stateMatchesRequest`, plus a `warning` naming both when they disagree (an offscreen host under `-RenderOffscreen` routinely declines a change, and echoing only the request reported that decline as success). `DescribeEditorWindowStateName` tests minimized BEFORE maximized because Windows keeps `::IsZoomed` true on a window it iconifies, so the naive order would report a minimized-from-maximized window as 'maximized' — the exact misreport the readback exists to prevent. USER-SAFETY, both directions asked for: the fix itself closes the unrecoverable trap (minimize is now undoable in-band), and whenever the result IS minimized the response carries a `recovery` field spelling the exact restoring call so the Win32 escape never has to be rediscovered; additionally `state:'minimized'` is REFUSED with `WINDOW_MINIMIZE_REFUSED` while `GIsAutomationTesting` is set — that global spans the whole automation session, not one test (`AutomationTest.cpp:1218`/`:1226`) — because minimizing the main editor window mid-suite is precisely what produced this ticket's 333.33 ms / DrawCalls-0 / frozen-GameThreadTime profile that no threshold could distinguish from a scene regression. Restore and maximize are never gated; the recovery direction must always work. ERROR-CODE TRAP HANDLED: `EditorWindowHandlers.cpp` has ZERO `ErrorCodes::` references (grep-verified) and hand-spells every one of its codes, so both new codes were REGISTERED in `Handlers/ErrorCodes.h` and spelled RAW at the call site — citing a constant there would flip the file to 'adopting' and turn its other sixteen raw codes into hard failures of `RegistryAdoptingFilesUseConstantsOnly`. TESTS, and what they do and do NOT prove — new file `Tests/Drive/TestDriveWindowSelectorMinimizedTolerance.cpp`, two ids; `check_test_ids.py` re-run after adding them: CLEAN (4793 ids, 4793 unique, no dot-prefix collision, no duplicate). `PinWright.drive.editorchrome.MinimizedTolerantSelectorOrder` is PURE over a constructed set of bare `SNew(SWindow)` widgets never added to Slate, never shown, with no native window — it minimizes and restores NOTHING, deliberately, given the suite-host hazard — and proves exactly two things: every pre-existing index keeps its value, and an EMPTY visible list still yields the minimized windows as candidates (the observed whole-editor-minimized condition that used to short-circuit into `NO_WINDOWS`). It proves NOTHING about live minimized windows. `PinWright.editor.set_window_state.RestoresMinimizedWindow` is LIVE but narrow: it creates its OWN uniquely-titled fixture and addresses only that window by title, so it can never target the main editor frame, and its scope guard restores the fixture before destroying it on every path including failing ones. Its always-on half asserts `state` and `stateMatchesRequest` on a normal window — both absent pre-fix, so it FAILS against the pre-fix handler on any host, needing no window-manager cooperation at all. Its minimized half is the true differential (pre-fix the verb answers `WINDOW_NOT_FOUND` 'No visible window title contains …' for a fixture that demonstrably exists), but native minimize is best-effort under `-RenderOffscreen -unattended`: when `Minimize()` does not take, that half CANNOT be asserted and the test emits `PINWRIGHT_ASSERTIONS_SKIPPED` through `Tests/TestSkipReporting.h` with `reason=window-minimize-did-not-take-on-host` rather than returning a quiet green, so `check_suite_log` classifies the run COMPLETED_WITH_SKIPS. The fixture is driven minimized with `SWindow::Minimize()` DIRECTLY, not through the verb, because the verb now refuses that state during a suite. NOT COMPILED AND NOT RUN by this agent, per instruction — the orchestrator builds and runs. DOCS, both sites `#1` part 3 named: `Docs/wiki-src/editor.md` now states this is the one window verb that can target a minimized window, that no selector is needed when the whole editor is minimized, the `state`/`stateMatchesRequest`/`warning`/`recovery` response fields, and both new error codes; `Docs/wiki-src/drive.md:106` no longer redirects minimized-state observation to `editor.set_window_state`'s readback — it now says an empty `windows` array on a live editor means the editor is minimized rather than Slate being down, names the selector-less restore as the recovery, and flags that `set_window_state` is a CONTROL verb that changes the state it reports, not an observation path."
