...
- `#10-guard-test-repaired-after-pump-extraction` `IN-REVIEW` developer — **The test was at fault, not the fix. `#6`'s pump is intact; only the literal it was grepping for moved.** `PinWright.niagara.CompileWait.WaitPumpsAssetCompilation` failed on one assertion — "the wait drives FAssetCompilingManager, which is where a Niagara compile finishes" — because it read `Handlers/Niagara/NiagaraCompileWait.cpp` off disk and required the string `FAssetCompilingManager::Get().ProcessAsyncTasks(` to appear *in that file*. `B-compile-material-blocks-and-mislabels` extracted that call into `Utils/AssetCompilePump.h` as `PinWright::AssetCompile::AdvanceOnGameThread()` so the material wait and this one share one pump, and `NiagaraCompileWait.cpp`'s `AdvanceAsyncCompilationOnGameThread()` now delegates to it (`:12` includes the header, `:42` calls it). Verified nothing was dropped by the delegation: the shared pump is the identical `FAssetCompilingManager::Get().ProcessAsyncTasks(/*bLimitExecutionTime=*/true)`, and the loop still calls the wrapper at the top of the body, before the poll and before the sleep — which is why the three ordering assertions in the *same* test (`PumpIndex < PollIndex`, `PumpIndex < SleepIndex`) still passed while only the file-scoped `Contains` went red. The extraction stands. `grep -rl "FAssetCompilingManager::Get().ProcessAsyncTasks("` over the plugin source now returns `Utils/AssetCompilePump.h`, `Handlers/Animation/AnimSequenceCreate.cpp` (unrelated) and the test file itself (its own direct call).
...
- `#15-direct-compile-wait-did-not-reproduce` `OPEN` reporter — **Non-repro on the direct verb, same editor session as `#14`.** `niagara.compile {assetPath: "/Game/FPS/VFX/NS_Impact_Water", force: true, wait: true, timeoutSeconds: 60}` on a system left `dataInterfaceCheck: "mismatched"` by seven preceding edits (2 x `add_module`, 4 x `set_module_input`, 2 x `set_static_switch`, 1 x `set_curve_keys`, all `compile:false save:false`) returned `{"requested":true,"compiled":true,"completed":true,"waited":true,"waitedMs":10147.07,"timeoutSeconds":60,"timedOut":false,"stillCompiling":[],"outstandingCompilationRequests":false,"status":"completed","durationMs":10409.79}` — a real 10.1 s block that landed, and the following `niagara.validate {level:"strict"}` moved `dataInterfaceCheck` from `mismatched` to `consistent` with `valid:true` and an empty `errors` array. `asset.save {force:true}` then wrote the package (`saveState:"written"`, 1011613 -> 1028843 bytes, mtime moved). So the wave-10 bounded wait does hold on the `niagara.compile` entry point; `#14`'s failure is at `effect.spawn_niagara`'s own internal wait, which is a different call site. Recording the negative so the next fix pass does not re-verify the verb and conclude the ticket is closed — the spawn gate is still unmeasured here.

- `#16-spawn-gate-half-fixed-direct-verb-half-confirmed-closed` `IN-REVIEW` developer — Both halves accounted for, and only one of them needed code.

  **The direct verb is closed and was not touched.** `#15` measured `niagara.compile {force:true, wait:true}` blocking a real 10.147 s, returning `status:"completed"` with `stillCompiling:[]`, and moving `dataInterfaceCheck` from `mismatched` to `consistent` — the wave-10 bounded pumped wait (`Handlers/Niagara/NiagaraCompileWait.cpp:93-145`, 60 s cap) holds at that entry point. Nothing in it was changed here, so `#15` remains the standing evidence for that half.

  **The spawn-gate half `#14` returned this ticket on is fixed.** `Handlers/VFX/EffectHandler.cpp` (~1349) called the shared wait with `bCompileWasRequested=false`, which made `HasWaitWork` (`NiagaraCompileWait.cpp:41-54`) degrade to `HasActiveCompilations()`. For a system whose compile request is queued-but-not-active — the on-demand state UE 5.8 leaves at PostLoad — that is false, so the loop body never ran once: no `AdvanceOnGameThread` pump, no `PollForCompilationComplete(true)` flush, nothing. That is `#14`'s `compileWaited:false` with `compileWaitedMs:0.0009` and `compileTimedOut:false`: not a bounded wait that failed to observe a compile, a bounded wait that was never entered. It now passes `bMayFlushRequestCompile=true` and drains the queued request through the same wait the direct verb uses, which is also what `UNiagaraComponent::Activate` does at the same point (`NiagaraComponent.cpp:1356-1363`).
