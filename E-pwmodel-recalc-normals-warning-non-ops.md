...
- `#2-remedy-rewritten-at-the-drain` `IN-REVIEW` developer — "Rewrote the remedy clause in `DrainWarningsInto` (GeometryScriptDebugSink.h) so the engine's `Consider using 'Set Mesh To Per Vertex Normals' or 'Compute Split Normals' instead.` becomes `This is the expected result for a mesh built without normals and needs no fix; run split_normals if you want hard edges.` — the diagnosis half is still forwarded verbatim, and the match is on the engine's whole sentence so a reworded engine message falls through unrewritten. Fixed at the drain rather than in `PwModelWarningNames` because that layer rewrites leading parameter tokens and cannot reach prose, and at `DrainWarningsInto` rather than at `RecalculateNormals` because five call sites across GeometryOps_Modeling.cpp (`recalculate_normals`, `spherify`, `cylindrify`, harmonic deform) and GeometryOps_Advanced.cpp (`loft`) drain the same message. `split_normals` is safe to name from a caller-agnostic layer only because both front-ends spell it identically (`geometry.split_normals` / `split_normals`). Corrected the falsified comment above the same function — it claimed no GeometryScript function this module calls emits a WarningMessage on UE 5.8 — and pointed the `recalculate_normals` drain comment in GeometryOps_Modeling.cpp at the rewrite. Regression test `PinWright.Model.WarningNames.NormalsFallbackRemedyNamesOnlyRealOps` in Tests/Model/TestPwModelWarningNames.cpp asserts, at the ops layer and end to end through the compiler on the reported `append_buffers`-with-no-`normals=` repro, that the diagnosis survives, that neither blueprint-node title reaches the author, and that every snake_case token the warning emits resolves in PwModelOpTable as a real op (or as a parameter of one the same warning names). NOT COMPILED OR RUN — build and suite are the verification step."
