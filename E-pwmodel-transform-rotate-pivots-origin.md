...
- `#2-docs-and-op-text` `IN-REVIEW` developer — "Documented the part-local-origin pivot and its `distance * sin(angle)` lever arm in `model.authoring` § Transform spaces and on the `transform` op's `describe_ops` text (op description plus the `rotate` and `scale` parameters), naming the generator's own `rotate=` as the in-place alternative; no warning added, and a new measured regression test pins the published number to the geometry"
