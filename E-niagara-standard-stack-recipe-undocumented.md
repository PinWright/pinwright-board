...
## History
- `#6-additional-scoremodulematch-guilty-line` `OPEN` reporter — Still reproduces at HEAD on another "campfire embers reusable VFX from scratch" fuzz task (`/Game/VFX/NS_CampfireEmbers`, outcome clean/done): `niagara.search_modules {query:"Initialize Particle", usage:"Module"}` -> only `/Niagara/Modules/Spawn/Initialization/InitializeRibbon` (score 50), NOT the real `InitializeParticle`; `{query:"particle state", usage:"Module", stage:"ParticleUpdate"}` -> `{"results":[],"totalMatches":0}` (stage-scoping the two-word query does not help), while the bare tokens resolve: `{query:"InitializeParticle", usage:"Module"}` -> `/Niagara/Modules/Spawn/Initialization/InitializeParticle` (score 1000) and `{query:"ParticleState", usage:"Module"}` -> `/Niagara/Modules/Update/Lifetime/ParticleState` (score 1000). New angle: the search_modules-side **guilty source line** (prior encounters were behavior-only; the code pin so far lived only on the ScoreOpMatch sibling `E-niagara-search-ops-multiply-keyword-misses-scalar` `#2`). Root cause is `Plugins/PinWright/Source/PinWright/Private/Handlers/Niagara/NiagaraSearchHandler.cpp` `ScoreModuleMatch` (lines 334-393): name scoring is substring-only — `if (NameLow.Contains(QueryLow))` (line 360) — against the space-less concatenated FName, and the "token-level" fallback splits the NAME on spaces (`NameLow.ParseIntoArray(Tokens, TEXT(" "))`, line 369), a no-op for concatenated names that never tokenizes the QUERY. So "Initialize Particle" (with the space) fails every name path and matches only via `if (DescLow.Contains(QueryLow))` (line 381) — InitializeRibbon's description literally contains the phrase "Initialize Particle", so it scores 50 while the real InitializeParticle scores 0 and is dropped; "particle state" appears in no name/description contiguously, so it scores 0 everywhere -> empty. The `#1`/`#2`/`#3` proposed fix (whitespace-split token-AND against name+keywords, order-insensitive) targets exactly this function. Same culprit RPC `niagara.search_modules`.
- `#5-additional-particle-state` `OPEN` reporter — Additional evidence (separate "campfire ember/sparks reusable VFX from scratch" fuzz task, `/Game/FX/NS_CampfireEmbers`) on a **new** stack module not previously covered: the ParticleState lifetime module. Replayed live against `mcp__pinwright__call`: `niagara.search_modules {query:"particle state", usage:"Module"}` -> `{"results":[],"totalMatches":0}`, while the bare leaf `{query:"ParticleState", usage:"Module"}` -> exactly `/Niagara/Modules/Update/Lifetime/ParticleState` (score 1000). Same literal-phrase-miss root cause as `spawn rate`/`spawn burst`/`gravity force`, now confirmed on the lifetime-management module too. Prior symptoms still live at HEAD in the same run: `{query:"spawn rate", usage:"Module"}` -> 0, and `{query:"initialize particle", usage:"Module"}` -> only `InitializeRibbon` (score 50), NOT the real `InitializeParticle` (the exact `#1` "initialize particle -> ribbon only" symptom). Task otherwise completed clean (system+emitter+sprite renderer+EmitterState/SpawnRate/InitializeParticle/AddVelocity/ParticleState/Drag/SolveForcesAndVelocity built, compile/validate strict/save all green); discovery again cost extra search_modules retries collapsing multi-word descriptions to the PascalCase leaf. Same culprit RPC `niagara.search_modules`; reinforces the #1/#2/#3 follow-up (split on whitespace / order-insensitive token AND-match against name+keywords).
- `#4-liveness` `OPEN` reporter — still reproduces at HEAD (NS_SparkPickup build task): `search_modules {query:"spawn rate", usage:"Module", stage:"EmitterUpdate"}` -> 0 and without the stage filter -> 0, while `{query:"SpawnRate", usage:"Module"}` -> the `/Niagara/Modules/Emitter/SpawnRate` asset (score 1000); correctly stage-scoping the two-word query does not help.
