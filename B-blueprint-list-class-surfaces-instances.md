...
- `#2-fix` `IN-REVIEW` developer — Source-confirmed the defect (`bIncludeOnlyOnDiskAssets` absent from `PinWright_BlueprintHandlers_List.cpp`; grep = 0 hits) and shipped the one-line root-cause fix: set `Filter.bIncludeOnlyOnDiskAssets = true` immediately after `Filter.bRecursiveClasses = true` (`PinWright_BlueprintHandlers_List.cpp:135-142`), matching the pattern `B-dump-folder-includes-level-subobjects` shipped for `asset.dump_folder`. Skipped the optional `:`-path hedge per the established shipped pattern (that ticket's `#3` removed the same colon hedge as redundant once the flag was proven sufficient) and verified the fix leaves the parent-tag walk untouched (`AppendBlueprintAssetsDerivedFromNativeClass` builds its own `BpFilter` restricted to `UBlueprint::StaticClass()`, so authorable BP subclasses still surface; `F-asset-search-native-subclass` behavior preserved). Added regression test `PinWright.blueprint.list.ExcludesInMemoryInstances` (`Source/PinWright/Private/Tests/Blueprint/TestBlueprintListExcludesInMemoryInstances.cpp`): builds an in-memory-only `UDataTable` under `/Engine/Transient`, proves via a direct AssetRegistry counterfactual that it leaks with the flag off and is excluded with it on, then invokes the real `blueprint.list {class:"/Script/Engine.DataTable"}` handler and asserts the in-memory row is absent — fails if the flag is reverted. Files: `Plugins/PinWright/Source/PinWright/Private/PinWright_BlueprintHandlers_List.cpp`, `.../Tests/Blueprint/TestBlueprintListExcludesInMemoryInstances.cpp`.
