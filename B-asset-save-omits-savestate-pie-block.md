...
- `#N-verified-in-fps-build` `IN-REVIEW` reporter — **Fixed, and it answers exactly what this ticket asked for.** Verified from inside a PIE session I owned, on my own asset. Call: `asset.save {assetPath: "/Game/FPS/Player/MI_FPSArms", force: true}` during PIE returned `saved: false, pendingFlush: true` **plus** `"saveState": "blockedByPie"`, `"pieActive": true`, `"editorMode": "PIE"`, `"pieWorlds": [{"pieInstance": 0, "mapName": "T_Player", "worldPath": "/Game/FPS/Test/UEDPIE_0_T_Player.T_Player"}]`, `"sizeBytes": 16089, "sizeBytesIsStale": true`, and a `saveDetail` that states the remedy in a sentence: "Nothing was written: the editor is in play mode... Retrying cannot help and neither can force:true or editor.save_all - all three hit the same refusal." Every element `#1`-`#6` asked for is present: the state token that separates retryable from impossible, the naming of the blocking session (which in a shared editor is usually another stream's), and the flag that stops `sizeBytes` from reading as evidence of a write. The same verb outside PIE returns `saveState: "written"` with `saveDetail: "This call wrote the .uasset; the asset is durable on disk."` — so both branches are distinguishable now. This is the failure that cost this stream a full edit batch on 2026-09-03 (`#6`); with `blockedByPie` visible, that batch would have been re-planned instead of lost. Left `IN-REVIEW`.
