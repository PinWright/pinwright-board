...
- `#4-verified-in-fps-build` `IN-REVIEW` AI-stream - Direct route retried after the 2026-09-05 plugin pull; **works**. `blueprint.add_function {blueprintPath:'/Game/FPS/AI/BPI_AIWeapon', functionName:'GetMagAmmoIface', outputs:[{name:'Ammo', type:'int'}], isPublic:true}` succeeded, then `compile_bpir {blueprintPath:'/Game/FPS/AI/BP_AIWeaponStub', mode:'replace'}` with `entry override GetMagAmmoIface() -> (int Ammo) { return (Ammo: 27) }` compiled clean (`compiled:true, status:'UpToDate', errors:[]`). `decompile_function` reads it back as `entry override GetMagAmmoIface() -> (int Ammo) { return 27 }` - a real interface-owned function graph with its output wired, not the `K2Node_Event` that could not implement the signature. The original failure (outputs counted as inputs, and `add_function {override:true}` reporting success for an unimplementable event) does not reproduce. One cosmetic leftover: the upsert kept the graph's original terminator, so `decompile_function` warns `Orphaned node not reachable from any entry point: K2Node_FunctionResult 'Return Node'` - harmless here, but it is the same orphan-reporting gap other tickets track. On disk: `Content/FPS/AI/BP_AIWeaponStub.uasset` 312719 bytes, saved with `asset.save {force:true}` reporting `saveState:'written'`. Note: `BPI_AIWeapon` and `BP_AIWeaponStub` are dead assets in this stream (the AI moved to `BP_WeaponBase`); they were used only as a clean harness for this retry. Left IN-REVIEW for the tester.
