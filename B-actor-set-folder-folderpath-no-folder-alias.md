...
- `#2-folder-alias-declared` `IN-REVIEW` developer — "Replaced `RPC_PARAM_REQ('folderPath', ...)` on `actor.set_folder` with `ParamAliasUtils::MakeAliasParamSpec(TEXT('folderPath'), ..., /*bRequired=*/true, {folderPath, folder})` in `Handlers/Actor/ActorFolderHandler.cpp`, so `folder` is a schema-registered alias the dispatcher's `ValidateHandlerParams` accepts for the required slot rather than rejecting as MISSING_REQUIRED_PARAM / UNKNOWN_PARAMS. The two accepted spellings live in one file-local `ActorSetFolderPathKeys()` list shared by the declaration and the body, which now reads the value with `Ctx.GetStringFirstOf` (the `RequireString('folderPath')` call would have failed on a `folder`-only payload that cleared the gate) and errors MISSING_PARAM naming both spellings when neither is present. Also echoed `folder` alongside `folderPath` at top level of the response so the key a caller reads back is the key the next call accepts, matching the per-row `updated[].folder` key that was already emitted. Regression test `PinWright.actor.set_folder.FolderAliasRoundTrip` in `Tests/Actor/TestActorErgonomicsHandlers.cpp` drives both calls through the real `FRpcDispatcher` (InvokeHandlerWithCapture bypasses the param gate where the defect lived): call 1 sets the folder with `folderPath` and asserts the top-level `folder` echo, call 2 feeds that key back with a different value and asserts success, an empty error code, and that the actor actually moved. Not compiled or run — build/test loop pending."
