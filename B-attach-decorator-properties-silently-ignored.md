...
## History
- `#1-verified-in-fps-build` `IN-REVIEW` AI-stream - Direct route retried after the 2026-09-05 plugin pull; **works, and now fails loudly**. First call carried a deliberately wrong enum: `behavior_tree.attach_decorator {assetPath:'/Game/FPS/AI/BT_Enemy', parentNodeId:'BehaviorTreeGraphNode_Task_10', decoratorClass:'BTDecorator_Blackboard', properties:{BlackboardKey:'bHasLOS', FlowAbortMode:'Both', NotifyObserver:'OnValueChange'}}`. It was **refused** with `INVALID_PROPERTY`, `droppedFields:[{name:'NotifyObserver', reason:"Invalid enum value 'OnValueChange' for enum 'EBTBlackboardRestart'"}]`, `appliedProperties:['BlackboardKey','FlowAbortMode']`, `rolledBack:true` - and no decorator was attached. Corrected to `NotifyObserver:'ValueChange'` and the call succeeded with `appliedProperties:['BlackboardKey','FlowAbortMode','NotifyObserver']`. Read-back confirms the key landed: Python on the runtime node shows `BTDecorator_Blackboard_9` with `selected_key_name = bHasLOS`, and `behavior_tree.decompile` places it on the intended parent - the Flank branch's `BTT_PeekFire @(250, 520)` - as `(BlackboardKey: bHasLOS, FlowAbortMode: "Self", NotifyObserver: "ValueChange")`. The reported failure (key silently defaulting to `SelfActor`, leaving the branch gated on an always-set key) does not reproduce. One observation, not a defect: `FlowAbortMode` was requested as `Both` and stored as `Self` - UE clamps abort modes for a decorator attached to a task, so the stored value is correct engine behaviour rather than a dropped property. Worth a wiki line, since `appliedProperties` lists it as applied and a caller checking only that list would not notice the clamp. On disk: `Content/FPS/AI/BT_Enemy.uasset` 50861 bytes, saved with `asset.save {force:true}` reporting `saveState:'written'`. Left IN-REVIEW for the tester.
