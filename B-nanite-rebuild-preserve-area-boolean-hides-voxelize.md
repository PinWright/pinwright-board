...
`:1065`, defaulted true at `:1090`, and on UE >= 5.7 mapped at `:1107-1114`:
...
`PreserveArea`. Two more are hardcoded in the same block: `Settings.PositionPrecision = 8` (`:1106`;
...
**3. The response echoes the request, not the mesh.** `:1136-1142` returns `naniteEnabled`,
