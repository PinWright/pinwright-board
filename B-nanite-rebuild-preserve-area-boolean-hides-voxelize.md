...
- `#2-enum-replaces-boolean-and-response-reads-back` `IN-REVIEW` developer — Every mechanism claim in the report reproduced against source. `ENaniteShapePreservation` (`EngineTypes.h:3266-3274` on 5.8) has three values, `FMeshNaniteSettings::ShapePreservation` defaults to `None` (`:3315`), `PositionPrecision` defaults to `MIN_int32` (`:3319`), and `Cluster.cpp:1054` does test `FoliageOverOcclusionBias > 0` before the `else if` PreserveArea branch at `:1065-1067`. One correction: the bias suppresses **PreserveArea only**. `Voxelize` is consumed on a different path (`Cluster.cpp:457` `bAllowVoxels`, `ClusterDAG.cpp:219,1075,1453`, `NaniteAssemblyBuild.cpp:49`) that the bias branch does not gate, so a warning phrased as "shapePreservation was also requested" would be wrong for voxelize; the shipped warning names preserve_area only. Fix in `Handlers/Asset/AssetWorkflowHandler.cpp`: added `shapePreservation` (`none` / `preserve_area` / `voxelize`, name-resolved with an error listing the valid values, `ERR_INVALID_ARGUMENT`), kept `preserveArea` as a deprecated alias, and **stopped defaulting both** — omitting them now leaves the asset's stored technique. `positionPrecision` became a parameter instead of the hardcoded `8`, and `GenerateFallback` / `FallbackPercentTriangles` are written only when `fallbackPercent` is supplied. The response is now read back off `GetNaniteSettings()` after the write (`shapePreservation`, `preserveArea`, `positionPrecision`, `positionPrecisionAuto`, `trianglePercent`, `fallbackPercent`, `generateFallback`) and reports `foliageOverOcclusionBias` off `GetRayTracingProxySettings()`, with a `warnings` entry naming `Cluster.cpp` when a non-zero bias will suppress a stored `preserve_area`. Response compatibility: additive except that `preserveArea`/`trianglePercent`/`fallbackPercent` are now readbacks rather than echoes — same names, same types, and identical values whenever the caller supplied them. Version range: the enum is **5.7+** (verified by grepping all six installed trees — 5.3-5.6 carry only `bPreserveArea`, and `GenerateFallback` itself only arrives in 5.6), so the fix reuses the handler's existing `#if UE_VERSION_NEWER_THAN_OR_EQUAL(5, 7, 0)` split rather than inventing a guard; the pre-5.7 branch refuses `voxelize` with `Ctx.SendUnsupportedEngineVersion("5.7", ...)` instead of silently downgrading it. `GetRayTracingProxySettings()` is also 5.7+ (5.6 has the struct but only a public member) and sits inside that same guard, so no row is owed to `Docs/engine-version-support.md`. Regression test `PinWright.asset.nanite_rebuild_mesh.ShapePreservationSelectsEngineMode` (`Tests/Assets/TestNaniteRebuildShapePreservation.cpp`) is behavioral: it drives each of the three values and reads `FMeshNaniteSettings::ShapePreservation` back off the asset, asserts an omitted call leaves a seeded `Voxelize` + `PositionPrecision=3` untouched, and covers the deprecated boolean and the rejection path — it fails against the old body on the first case (`voxelize` ignored, `preserveArea` defaulting true) and on the omission case, neither of which a response-only assertion could see. **Not RPC-verified and not compiled here:** no editor was running, no mesh was rebuilt, and the wave's build/suite run happens after this handoff. Ticket items deliberately not taken: writing `FoliageOverOcclusionBias` (a different settings struct and a different verb's business — only the readback and the warning landed) and the correction to `B-render-nanite-rebuild-mesh-no-completion-signal` / `E-render-nanite-rebuild-async-poll-undocumented`, which is recorded in this ticket's body for a reviewer to lift. Noted while reading: `Docs/rpc-hard-removal-rejected-candidates.md:173` already states the two verbs correctly, so the conflation is confined to those two board files.
