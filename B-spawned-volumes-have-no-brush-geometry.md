...
- `#4-lifted-brush-builder-to-both-spawn-paths` `IN-REVIEW` developer — **Fixed by lifting the existing builder, not by copying it, exactly as `#1` asked.** New shared header `Source/PinWright/Private/Handlers/Volume/VolumeBrushGeometry.h` holds `BuildBoxBrushGeometry` (and the three `Create*BrushForVolume` wrappers) moved **verbatim** out of `VolumeHandler.cpp`'s `VolumeHelpers` namespace and made `inline` in a named namespace (the plugin's Unity-safe convention, per `EnvironmentDirtyUtils.h`). `VolumeHandler.cpp` now re-exports them with four `using VolumeBrushGeometry::…;` declarations at the old site, so all three existing call sites (`SpawnVolumeActor`, `set_volume_extent`, `set_volume_bounds`) compile unchanged and the `set_volume_extent` work in flight for `B-set-volume-extent-ignores-existing-actor-scale` is untouched. **`foliage.create_procedural`:** the `SetActorScale3D(Size / 200.0f)` write and its "default extent of 100 units" comment are **deleted** and replaced with `VolumeBrushGeometry::BuildBoxBrushGeometry(Volume, Size)` — `Size` is FULL box size, which is what `UCubeBuilder::X/Y/Z` mean (`EditorBrushBuilder.cpp:395-417`, default 200 = 100 half-extent), so the volume comes out at `location ± Size/2` at identity scale. This closes `#3`'s "necessary and not sufficient" point in one edit: the scale that `#3` proved would multiply the new geometry ~100x is the very line removed, so a volume created by the fixed verb carries no scale to normalize. **`actor.spawn`:** a new `else if (AVolume *VolumeActor = Cast<AVolume>(Spawned))` arm in `SpawnHandler.cpp`'s post-spawn chain builds the engine's own default box (`DefaultBoxSize = 200.0f`, mirroring `UActorFactoryBoxVolume::PostSpawnActor`, `ActorFactory.cpp:1905-1915`, which builds with a default-constructed `UCubeBuilder`); the request's `scale` is applied first and multiplies it, matching how a brush actor behaves everywhere else. Chose BUILD over `#1`'s alternative of REFUSING the class: refusing would break every `AVolume` subclass `actor.spawn` accepts today, and `F-generic-volume-creator-with-brush-geometry`'s own rebuttal is specifically that an `actor.spawn` alias "would reproduce a Critical defect" — building geometry removes that objection rather than papering over it. **Gate is `AVolume`, not `ABrush` as `#1` proposed, and that is deliberate:** the engine's own helper is `UActorFactory::CreateBrushForVolumeActor(AVolume*)` and `csgPrepMovingBrush` is the MOVING-brush path; a non-volume `ABrush` (a CSG level brush) wants a level CSG rebuild, not this. Both measured classes (`APCGVolume`, `AProceduralFoliageVolume`) are `AVolume`. **No new error code** — nothing new is refused, so `Handlers/ErrorCodes.h` is untouched. **Regression test, behavioural not existential:** new `Source/PinWright/Private/Tests/World/TestSpawnedVolumeBrushGeometry.cpp`, two tests — `PinWright.actor.spawn.VolumeSpawnBuildsBrushGeometry` (spawns a `BlockingVolume` through the real handler) and `PinWright.foliage.create_procedural.VolumeBuildsBrushGeometry` (dispatcher route, `foliageTypes: []` so no assets are generated and no scatter runs). Each asserts `AVolume::EncompassesPoint` is TRUE at the volume's own centre and at 90% of the way to its corner, and FALSE two half-extents out — which is the assertion a zero-extent phantom cannot pass: with a null brush there are no convex hulls, `FBodyInstance::GetSquaredDistanceToBody` returns false with `bFoundValidBody` unset (`PhysInterface_Chaos.cpp:1189-1264`), and `EncompassesPoint` answers false for the centre point (`Volume.cpp:84-116`). A test asserting only that the actor exists passes today, which is why none is written. The foliage test additionally pins `GetActorScale3D() == (1,1,1)` and `GetBounds().BoxExtent == Size/2`, so a re-introduced scale write fails for the right reason. Both ids checked against the dot-prefix-swallowing trap: neither `PinWright.actor.spawn` nor `PinWright.foliage.create_procedural` exists as a leaf id, and neither new id is a prefix of an existing one. **Docs:** one paragraph added to `Docs/wiki-src/actor.md` under `### actor.spawn` stating that volume classes now get the default 200 uu box and that `scale` multiplies it, and one line under `### foliage.create_procedural` in `Docs/wiki-src/foliage.md` stating `size` is FULL extent centred on `location` at identity scale. **NOT DONE, and a verifier must check these rather than assume them.** (1) **Not compiled and not run** — the wave lead builds and runs the suite after the wave, per the task's standing instruction; every claim above is a source reading. (2) `#1`'s open question is still open: whether a correctly built brush makes `instances_spawned` go positive was NOT verified, and `B-create-procedural-ignores-scale-and-normal-fields` `#3` documents a second independent placement defect in the same verb. (3) **A side effect on an existing test that a verifier should watch:** `PinWright.foliage.create_procedural.ReportsInstancesSpawned` (`Tests/World/TestEnvironmentHandlers.cpp:1666`) drives the verb at the world ORIGIN with a real cube foliage type and cleans up only the generated `/Game/ProceduralFoliage` packages — not placed instances. Before this fix its volume had no geometry and placed nothing; with geometry it may now scatter real foliage into the host map, and instances landing on a PRE-EXISTING `AInstancedFoliageActor` survive `FScopedEditorWorldActorGuard`. Left unedited on purpose (out of this ticket's scope, and the file is in flight), but it is a live host-content-leak risk in the `B-tests-leak-host-content` family and wants its own ticket. (4) **Three more `AVolume` spawn sites were found by source read and deliberately NOT fixed here.** `lighting.create_lightmass_importance_volume` (`Handlers/Environment/LightingHandler.cpp:773-779`) is this exact defect verbatim, `SetActorScale3D(Size / 200.0f)` included, on an `ALightmassImportanceVolume` — a fourth instance, unmeasured, and it wants its own ticket rather than a silent ride on this one. `PostProcessVolumeUtils.h:61` spawns an `APostProcessVolume` but sets `bUnbound = true` immediately, so bounds are irrelevant there and it is NOT affected. Relation to `F-generic-volume-creator-with-brush-geometry` (Medium): this fix does **not** make it redundant and is **not** a subset of it — that ticket asks for a `volume.create {volumeClass, shape, extent, properties}` verb; this one only stops two existing verbs from lying. But it **does demolish that ticket's central rebuttal**, which is entirely "`actor.spawn` of an `ABrush` subclass produces a zero-extent, collisionless phantom": after this, `actor.spawn` gives any `AVolume` subclass real geometry, so the residual gap is only the shape/extent/properties ergonomics (extent already reachable via `volume.set_volume_extent`). That ticket should be re-argued or re-rated on the narrower ask.
- `#5-verified-fixed-on-the-1332-build` `DONE` verifier — 2026-08-30, **13:32 build** (the one carrying waves 2 and 3, plugin `d8f1bc32`; the fix itself landed one wave earlier in `2ba21649`). Both entry points re-measured live in a running editor, and the deciding number in every case is `actor.get_bounding_box` — a **different verb in a different namespace** from the one under test, and the same readback `#1` used to record `extent:[0,0,0]`, so before and after are the same measurement. **(1) `actor.spawn {className:"PCGVolume"}`** → `extent [100,100,100]` at `origin [0,0,200000]`. `#1` measured `[0,0,0]` on this exact class and call shape. That is the engine's default 200 uu box read as a half-extent, which is what `#4` said it would build. **(2) `foliage.create_procedural`** with `bounds.size {1000,3000,6000}` → `extent [500,1500,3000]`, i.e. exactly `size/2`, and `actor.get_transform` reports scale `[1,1,1]`. **That identity-scale reading is the independent close of `#3`'s "necessary and not sufficient" objection**: `#3` proved that building the brush while `FoliageHandler.cpp:1499`'s `SetActorScale3D(Size/200)` still ran would give a volume ~100x too large, and a volume that reports `size/2` bounds at scale 1 cannot have had that line execute. Measured on a fresh spawn, not inferred from the deleted line. **(3) The `set_volume_extent` obstruction `#1` and `#2`/`#3` chase is gone as well**, checked here only because it is the workaround this ticket prescribes: on a volume deliberately pre-loaded with scale `(5,5,5)` via `actor.set_transform`, `volume.set_volume_extent {300,400,200}` returned `newExtent {300,400,200}` with `clearedScale {5,5,5}`, and `actor.get_bounding_box` independently confirmed `[300,400,200]` — not the `(1500,2000,1000)` the double-apply would have produced. That belongs to `B-set-volume-extent-ignores-existing-actor-scale` and its status is **not** changed from here.

  **Closed on the brush ONLY, and one criterion is explicitly forbidden to a future tester.** `#1`'s open question — whether real geometry makes `instances_spawned` go positive — is **not** answered by this close and must never be used as the criterion, because the field is structurally incapable of answering it. `foliage.create_procedural` counts through `FFoliageInfo::GetPlacedInstanceCount()` (`FoliageHandler.cpp:2367-2368`, delta at `:2436-2437`, emitted at `:2481`), and that engine predicate counts an instance only when `!Instances[i].ProceduralGuid.IsValid()` (`C:/UE_5.8/Engine/Source/Runtime/Foliage/Private/InstancedFoliage.cpp:2591-2601`) — i.e. exactly the hand-painted complement of what a procedural run produces. This run returned `instances_spawned: 0` alongside a correctly built volume, consistent with that. Owned by `B-create-procedural-spawned-count-always-zero` (OPEN, High); read it before judging that verb's output by that field.

  **Not re-checked here, and left standing for whoever picks them up:** `#4`'s side-effect (3), the host-content leak risk in `PinWright.foliage.create_procedural.ReportsInstancesSpawned` — a volume with real geometry may now scatter into the host map, and this session did not run the suite; and `#4`'s item (4), `lighting.create_lightmass_importance_volume` (`LightingHandler.cpp:773-779`), which carries this defect verbatim and now has its own ticket, `B-lightmass-volume-no-brush-geometry`. Neither was measured. `F-generic-volume-creator-with-brush-geometry` still wants the re-argument `#4` describes; that file is deliberately untouched.
