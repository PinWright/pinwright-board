...
- `#2-typed-level-viewport-resolution` `IN-REVIEW` developer — Rewrote the shared helper `Source/PinWright/Private/Handlers/Editor/EditorHandlerUtils.h` `ResolveActiveLevelViewportClient()` onto the typed route `FLevelEditorModule::GetFirstActiveViewport()` -> `IAssetViewport::GetAssetViewportClient()` (the pattern `B-set-game-view-keeps-editor-billboards` already established), so it never touches `FViewport::GetClient()` and keeps returning the editor client through PIE (`SLevelViewport::StartPlayInEditorSession` parks the editor viewport in `InactiveViewport` and never replaces `LevelViewportClient`); it returns null when no level viewport is open and every caller already handles null. Added `EditorHandlerUtils::IsActiveViewportPlayInEditor()`, an explicit `FViewport::IsPlayInEditorViewport()` discriminator, since the client pointer itself carries no usable type information. Re-pointed all five production sites — `Handlers/Environment/EnvironmentHandler.cpp` (the crash), `Handlers/Editor/ViewportHandler.cpp` `ForceRedrawActiveViewport()` **and** `editor.set_camera`'s fallback, which carried the same defect written as a C-style `(FEditorViewportClient*)` cast and was not in the original report, `Handlers/Actor/ActorNudgeHandler.cpp` (its two error branches collapse into one, the second having been dead code) — plus the two test sites `Tests/EditorOps/SetGameViewTogglesTest.cpp` and `Tests/EditorOps/TestEditorHandlers.cpp`. `system.inspect.get_viewport_info` now reports `pie` (bool) and `activeViewport` (`"pieGameViewport"` / `"levelEditorViewport"`) beside the geometry and reads the camera only from the level-editor client, so during PIE the dimensions describe the play window while the camera fields describe the editor camera and the response says so; method description and `docs/wiki-src/system.inspect.md` updated for the new fields. Regression tests in `Tests/EditorOps/TestViewportClientResolution.cpp`: `PinWright.editor.viewport_client.NoRawViewportClientDowncasts` scans every `.cpp`/`.h` under the plugin's `Source` (comments and raw strings neutralized) for `static_cast`/C-style casts to `FEditorViewportClient*`/`FLevelEditorViewportClient*` and requires zero — counterfactual: reinstate the cast in `EditorHandlerUtils.h` (or any of the other six removed sites) and the scan reports that file and line and the count assertion fails. `PinWright.system.inspect.get_viewport_info.CameraComesFromTheLevelViewportClient` forces `FViewport::SetPlayInEditorViewport()` on and off around the verb (restored on the same stack) and asserts the discriminator follows the flag, the resolver does not, and the response carries `pie`/`activeViewport`/editor-camera fields — counterfactual: revert the handler hunk and the pre-fix body emits no `pie` field at all, so `response carries pie` fails. Stated limitation, not hidden: with no live PIE session the active viewport's client and the level client are the same object, so that test's camera-equality assertions are consistency checks rather than crash reproductions; the crash itself needs a PIE session a synchronous automation test cannot stand up, which is why the durable half is the source contract. `reinterpret_cast` is deliberately outside the contract — the one site (`Tests/Render/TestCaptureSubjectResolver.cpp`'s sentinel) fabricates a pointer that is only ever compared against null.
- `#3-review-followups` `IN-REVIEW` developer — Review follow-ups. (a) `Tests/EditorOps/SetGameViewTogglesTest.cpp`: the mirrored LevelEditor-module walk and the resolver "fallback" beneath it had become the same code, so the second was unreachable; collapsed to one `EditorHandlerUtils::ResolveActiveLevelViewportClient()` call and dropped the three includes (`IAssetViewport.h`, `LevelEditor.h`, `Modules/ModuleManager.h`) that only the deleted walk used. (b) `PinWright.editor.viewport_client.NoRawViewportClientDowncasts` patterns widened to allow an optional `const` in both the `static_cast<...>` and C-style forms; re-swept, still zero matches outside the scanner file. Documented at the pattern table that a cast written through a `using` alias of the type evades the scan by design — it is a tripwire on the idiom that actually recurred, not a type checker. (c) **Correction to the review note rather than to `#2`:** the `reinterpret_cast` sentinel is `SentinelViewportClient()` at `Source/PinWright/Private/Tests/Render/TestCaptureSubjectResolver.cpp:119-122`, not `Tests/TestUtils.h`. `grep -rn SentinelViewportClient Source/` returns that file only (definition `:119`, sole use `:806`), and `TestUtils.h:110-130` is `JsonArrayFindObjectByStringField`. The line range `119-122` in the note is right; the filename is not, so `#2`'s citation stands as written. (d) Behaviour change worth naming, benign: `ForceRedrawActiveViewport()` (`Handlers/Editor/ViewportHandler.cpp`) now invalidates and draws the level-editor client's own viewport, which during PIE is the one `SLevelViewport` parked in `InactiveViewport` — a wasted draw of an offscreen viewport in place of the previous undefined call through the game client, and outside PIE it is the same viewport as before. `ForceRedrawViewportClient` already null-guards `ViewportClient->Viewport`, so a client whose viewport pointer was cleared is a no-op rather than a fault. The PIE game viewport is no longer force-drawn by this path; it renders itself every frame, so nothing depended on that.
