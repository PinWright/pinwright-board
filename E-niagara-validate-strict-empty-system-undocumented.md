...
- `#4-docs-strict-validate-levels` `IN-REVIEW` developer — Implemented the docs fix. Added a `### niagara.validate` H3 overlay section to `Docs/wiki-src/niagara.md` documenting the `level:strict` severity escalation: (a) strict promotes exactly `NO_EMITTERS`/`DISABLED_EMITTER`/`NO_RENDERERS` from warning to hard error (top-level `valid` flips false) while basic keeps them warnings — verified against `NiagaraInspectHandler.cpp` `NormalizeValidationSeverity` :101-107 (the ticket body's :85-95 is stale; History #3 already had the right lines); (b) these fire by design on freshly-authored assets — a `niagara.create_system` system always strict-errors `NO_EMITTERS` (raised as warning at `NiagaraDumpBuilder.cpp:1561`, not the body's stale :1500-1508), a blank `create_emitter` emitter always strict-errors `NO_RENDERERS` (:1581/:1646), a disabled handle errors `DISABLED_EMITTER` (`NiagaraInspectHandler.cpp:184`) — so stay on the default `basic` when validating an in-progress/empty system for *other* issues (e.g. dangling refs after a rename); (c) top-level `errors`/`valid` are authoritative over the raw pre-normalization `compile.issues` severities (History #3's warning-vs-error layering nuance). Behavior is unchanged and stays locked by `FNiagaraValidateStrictNoEmittersErrorTest`. Regression test added: `PinWright.infra.wiki_handler.MethodPage.NiagaraValidateStrictLevel` (`Source/PinWright/Private/Tests/Infra/TestNiagaraValidateStrictLevelDocs.cpp`) renders the live `niagara.validate` method page via `WikiHandler::RenderPage` and asserts the three code names + the warning→error promotion + the `create_system` steer + the `compile.issues`/authoritative nuance all survive (every marker is overlay-exclusive; reverting the H3 makes `WikiOverlay::LoadMethodSection` return empty and the test fails). Scoped to the strict-escalation angle only; the `COMPILE_STATE_UNINITIALIZED` note stays with the distinct sibling `E-niagara-validate-compile-state-uninitialized-undocumented`. Files: `Docs/wiki-src/niagara.md`, `Source/PinWright/Private/Tests/Infra/TestNiagaraValidateStrictLevelDocs.cpp`.
