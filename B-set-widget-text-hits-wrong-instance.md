...
encounters: 2
lastSeen: 2026-07-11T21:32:56.6622039+03:00
---
...
## History
- `#4-liveness` `IN-REVIEW` reporter — Still observed at tested HEAD (audit reconfirm, ui.screenshot task): with no HUD cleared, `ui.create_hud {widgetPath:"/Game/Global/Blueprints/WBP_PlayerHUD.WBP_PlayerHUD"}` returned `{"widgetName":"WBP_PlayerHUD_C_1"}`, then `ui.set_widget_text {key:"InteractionText", ...}` returned success echoing `"owning_user_widget":"WBP_PlayerHUD_C_0"` — the auto-spawned instance, not the created/painted `_C_1`. Same multi-instance-PIE first-match ordering as `#3`; the `#2` fix remains incomplete. No `instance_name`/host selector on the `ui.set_widget_*` surface to redirect the write.
