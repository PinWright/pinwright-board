...
## History
- `#2-go-decision` `IN-REVIEW` developer — GO. Verified against current source: real, cold-load-confirmed silent asset-loss defect. `create_schema` saves via a direct `McpSafeAssetSave(Schema)` (`PoseSearchHandler.cpp:486`); `create_database` / `add_database_animation` save via the shared `FinishPoseSearchAsset` chokepoint (`:159`, also `McpSafeAssetSave`) — all mark-dirty-only, nothing reaches disk, while `saved` echoes the requested flag (`:493`/`:600`/`:645`) and `existsAfter:true` comes from the registry. Decision: reroute all three `save:true` paths through the in-tree `SaveAssetToDiskReportingPresence(bForce=true)` (real force-save + `IFileManager::FileSize` disk probe) and report honest `saveRequested`/`saved`/`pendingFlush` via `AddAssetSaveReport`, matching the accepted niagara/audio/metasound/material siblings. The corruption-sensitive shared `McpSafeAssetSave` and its Blueprint/SCS callers stay untouched (`UPoseSearchSchema`/`UPoseSearchDatabase` are non-Blueprint). Severity Critical unchanged; full ticket scope (all three verbs) implemented, no split. Adopting the red test `PinWright.pose_search.CreateSchemaSaveWritesToDisk` as the regression gate.
