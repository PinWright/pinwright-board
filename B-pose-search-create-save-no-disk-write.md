...
## History
- `#3-test-phase-fix` `IN-REVIEW` developer — Full-suite verification (the #2 note's green was a narrow `PinWright.pose_search`-only run) surfaced a fatal engine assert on a background worker that killed the run mid-suite: the pre-existing `SchemaDatabaseAuthoringPipeline` fixture bound a degenerate in-code `UAnimSequence` (no data model → zero PlatformHash) to a database, and the editor's async PoseSearch index build (`FAsyncPoseSearchDatabasesManagement`, scheduled off the edit-modification delegates, independent of save) hard-`check()`ed the uncooked/zero-hash sequence in `PoseSearchAssetSampler.cpp:438`. Root-caused to the FIXTURE, not production (save:false path is byte-identical pre/post-fix; real animations are already samplable): fixed by having `CreatePoseSearchTestSequence` (`Tests/Gameplay/TestPoseSearchHandlers.cpp`) author one keyed `root` bone track via `IAnimationDataController` so `CloseBracket` builds real compressed data (non-zero hash) and the indexer accepts it. Also cleaned stale untracked crash-flushed `.uasset` artifacts left under `Content/` by the first (pre-fix) crash — they made `CleanupTestAsset`'s `IsOnDisk` gate reload a crash-flushed GAS blueprint and access-violate in `RegenerateBlueprintClass`; a clean baseline never persists those. Shipped mechanism unchanged from #2 (save reroute through `SaveAssetToDiskReportingPresence(bForce=true)` + honest `AddAssetSaveReport`); all #2 adversarial-review fixes (non-fatal `TryConvertLongPackageNameToFilename` in `CleanupTestAsset`, transaction-scoped-before-save in all three verbs, `save` param wording, added `CreateDatabaseSaveWritesToDisk` test) verified. Full PinWright suite 3637/3637 green, 0 crashes, exit code 0; all three `pose_search` tests pass — `CreateSchemaSaveWritesToDisk` asserts the schema `.uasset` on disk.
