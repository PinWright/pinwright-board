...
no workaround from inside the tool surface for *these* cvars: a caller cannot **detect** the pin,
let alone undo it — `Unset` is not exposed, and re-setting at a lower priority is exactly what
`CanChange` refuses.

Stated precisely, because the obvious stronger version is false: PinWright **can** read a cvar's
SetBy, in exactly one place. `Handlers/Render/PreviewViewportCaptureUtils.cpp:1397-1398` calls
`::GetConsoleVariableSetByName(CVar->GetFlags() & ECVF_SetByMask)` and publishes it as `setBy` at
`:2803` — scoped to the forced-show-flag survey inside a capture response, which will never carry
`r.MaxAnisotropy` or `r.Streaming.PoolSize`. So the capability exists and this path cannot reach it.
The general readback gap is `F-console-search-setby-priority`'s, and closing that would make this
pin *detectable* but still not undoable; it is a complement to the fix here, not a substitute.
...
- **`F-console-search-setby-priority`** — the general "no verb reports a cvar's SetBy" gap, which is
  what makes this pin undetectable in-band. Complement, not substitute: closing it would let a caller
  *see* the pin; only the fix here stops it being created, and neither exposes a way to undo one.
...
## History
- `#1-typed-verbs-pin-above-the-panel` `OPEN` reporter — Third route to the scalability pin the two console tickets cover, and the one that needs no console string. `performance.configure_texture_streaming` (`Handlers/Debug/PerformanceHandler.cpp:558`) and `performance.apply_baseline_settings` (`:939`) write scalability cvars through a bare `CVar->Set(x)` — `:572` and the `SetCVar` lambda's `:953` — and the templated helper at `IConsoleManager.h:766` defaults its flags to `ECVF_SetByCode = 0x0E000000` (`:183`), six levels above the `ECVF_SetByScalability = 0x02000000` (`:159`) that `Scalability::SetQualityLevels` and therefore the editor's own Settings > Engine Scalability Settings panel write at, so `FConsoleVariableBase::CanChange` (`ConsoleManager.cpp:275`, `bRet = NewPri >= OldPri` at `:280`) discards every later panel change to the owning group for the session, logging a Warning via the `else` at `:305-309`. **Corrected the relayed count while re-deriving: seven `ECVF_Scalability` cvars across the two verbs, not six.** `apply_baseline_settings` writes six, not five — the scope note on `B-console-member-cvar-pin-freezes-scalability` matched against `BaseScalability.ini` and so missed `r.VSync`, which carries `ECVF_Scalability` on its declaration (`ConsoleManager.cpp:4330`, flags `:4334`) with **no ini row** — precisely the `r.ScreenPercentage` case that same ticket identified when it wrote *"Refuse on the flag, not on a hardcoded list."* Every membership re-derived from the declaration rather than the ini: `r.MotionBlurQuality` (`:4126`/`:4130`), `r.DepthOfFieldQuality` (`:4173`/`:4181`), `r.BloomQuality` (`:4082`/`:4091`), `r.ShadowQuality` (`:4119`/`:4123`), `r.MaxAnisotropy` (`:4361`/`:4364`), `r.Streaming.PoolSize` (`TextureStreamingHelpers.cpp:120`/`:123`); and the two non-members named so a fixer does not over-scope — `r.TextureStreaming` is `ECVF_Default` (`TextureStreamingHelpers.cpp:105`/`:110`) and `r.AllowHDR` is `ECVF_ReadOnly` (`RenderCore.cpp:322`/`:327`). **The aggravator:** `apply_baseline_settings` publishes `scalabilityGroupsChanged: false` at `:996` — added by the comment at `:992-995` to be *"machine-checkable"* — which is literally true (no `sg.*` value moved) and materially false (six scalability cvars across three groups are now pinned above the panel), so the response actively asserts the absence of the side effect it just caused. **The guard cannot reach this path:** `ScalabilityConsoleGuard::IsScalabilityGroupLine` (`ScalabilityConsoleGuard.h:38-41`) is a prefix test on a command-line string, included only by `SystemControlHandler.cpp:6` (`:611`) and `EditorCommandHandler.cpp:13` (`:302`); `grep ScalabilityConsoleGuard PerformanceHandler.cpp` returns zero hits and there is no string here to test, so even the flag-based widening the sibling ticket asks for does not reach it. Fix asked in three parts, with the correct pattern already in the tree: `FScopedViewDistanceScale` (`PreviewViewportCaptureUtils.cpp:161-162`, restore `:174`, rationale `:139-142`) reads `GetFlags() & ECVF_SetByMask` and writes back at that same priority — lift it to a shared helper and sweep the file, since `:1011`/`:1043`/`:1049` carry the same bare-`Set` shape; correct or remove `scalabilityGroupsChanged`; and re-read the cvar after the set so `appliedCVars` (`:991`, built from the requested value at `:954`) stops reporting a discarded write as applied. **Stated as not observed:** the relayed claim that this is what froze the Scalability panel today does **not** hold for this door — the live log (`Saved/Logs/EAContentExamples58.log`, opened 17:15:53) contains zero `"was ignored as it is lower priority"` lines and zero calls to either verb this session, so today's pin was through the console door and belongs to the sibling ticket. Source-derived, **not RPC-verified, and deliberately not reproduced**: reproducing it means pinning six cvars in an editor several agents share, i.e. inflicting the defect to prove it; the live check is written out for a disposable editor. Dedup: searched `apply_baseline_settings`, `configure_texture_streaming`, `ECVF_SetByCode`, `SetByScalability`, `scalabilityGroupsChanged`, `r.MaxAnisotropy`, `r.Streaming.PoolSize` and every `*scalability*`/`*console*` filename; the two console tickets, `E-baseline-no-sg-setall` and the three other `ECVF_SetByCode` tickets are distinguished in § *Distinct from*, and nothing on the board says a typed verb creates this pin. Filed as its own ticket exactly as `B-console-member-cvar-pin-freezes-scalability` § *Scope note* asked, with **that ticket's `encounters` and `lastSeen` left untouched** — three doors, three files, one defect, and merging them would hide that the fix sites differ. Severity High on two independent counts of the band: a silent session-durable degradation of the user's editor with nothing in the response naming it, and a machine-readable field asserting its absence. Critical declined — no crash, nothing written to a package, and the damaged artefact is session state a restart recovers. Medium declined — no workaround exists inside the tool surface for these cvars: a caller cannot detect the pin, and `Unset` is not exposed while re-setting lower is what `CanChange` refuses. Stated precisely rather than in its stronger false form: PinWright **can** read a cvar's SetBy, in exactly one place — `PreviewViewportCaptureUtils.cpp:1397-1398`, published as `setBy` at `:2803`, scoped to the forced-show-flag survey inside a capture response, which will never carry `r.MaxAnisotropy`; the general gap is `F-console-search-setby-priority`'s and is a complement to this fix, not a substitute. Reach declined in both directions: neither verb is every-session, but neither is a rare edge path — they are the path the guard's own refusal text steers callers onto.
- `#2-write-at-the-cvars-existing-priority` `IN-REVIEW` developer — "Implemented Ask 1, 2 and 3, and swept the file as § *Fix* asked. **The default was verified before the fix rather than taken from the ticket:** `IConsoleManager.h:766` really is `template <typename T> void Set(T Value, EConsoleVariableFlags Flags = ECVF_SetByCode, FName Tag = NAME_None)`, so a bare `CVar->Set(x)` writes at `ECVF_SetByCode = 0x0E000000` (`:183`), and `FConsoleVariableBase::CanChange` really is `bool bRet = NewPri >= OldPri;` at `ConsoleManager.cpp:280`. **New shared header `Handlers/CVarPriorityPreservingSet.h`** — `ReadCurrentSetByPriority(CVar)` returning `GetFlags() & ECVF_SetByMask`, plus `SetPreservingPriority` int32/float overloads — lifting the `FScopedViewDistanceScale` pattern (`PreviewViewportCaptureUtils.cpp:161-162`) out of the one place that had it rather than copying it a third time. **I did NOT write at a fixed `ECVF_SetByScalability`, and the reason is engine evidence rather than preference.** Two independent failures: (i) a Scalability-priority write is DISCARDED whenever the variable already sits higher, and `r.VSync` — one of the seven — is written at `ECVF_SetByGameSetting = 0x03000000` by `UGameUserSettings::ApplyNonResolutionSettings` (`GameUserSettings.cpp:529`), so on any host that has applied game user settings the verb would have silently stopped working, which is precisely the mirror-image failure Ask 3 exists to report; (ii) `ECVF_SetByScalability` is reserved for scalability-flagged variables — `ConfigUtilities.cpp:337-346` `ensureMsgf`s on a Scalability-priority write to a cvar carrying neither flag — and these verbs also write `r.TextureStreaming` and `r.MeshDrawCommands.*`. Writing at the variable's own priority is accepted unconditionally (the guard is `>=`, not `>`), never raises, never lowers, so the write always lands and never outranks anything it did not already outrank. `IConsoleVariable::SetWithCurrentPriority` was considered and rejected: it exists on 5.3-5.8 but its overload shape changes between 5.6 and 5.7, and the explicit read-then-write is identical on every version. **Swept all twelve bare `CVar->Set()` sites in `PerformanceHandler.cpp`, not only the two the ticket names**, because per-site flag judgement is exactly the ini-list reasoning the sibling ticket warns against. Seven of the twelve write `ECVF_Scalability` cvars and five of those are OUTSIDE the two verbs on the ticket, re-derived from each declaration: `performance.set_resolution_scale` -> `r.ScreenPercentage` (`LegacyScreenPercentageDriver.cpp:38`, flags `:43`); `performance.set_vsync` -> `r.VSync` (`ConsoleManager.cpp:4330`/`:4334`); `performance.configure_nanite` -> `r.Nanite` (`StaticMeshSceneProxy.cpp:110`/`:117`); `performance.configure_lod` -> `r.MipMapLODBias` (`Texture2D.cpp:81`/`:84`) and `r.ForceLOD` (`RenderCore.cpp:223`/`:226`). The other five sites (`r.TextureStreaming`, `r.GPUStatsEnabled`, `r.MeshDrawCommands.*`, the occlusion trio) are not scalability-flagged and go through the same helper anyway, so the file has one write rule rather than twelve judgements. **Ask 2:** `scalabilityGroupsChanged: false` is KEPT (it is literally true and is an existing contract with no test asserting it) and `scalabilityCVarsPinned: false` is added beside it — the field that answers what a caller reading the first one was actually asking, and false now as a structural consequence rather than a claim. **Ask 3:** `appliedCVars` entries are built from `CVar->GetInt()` read back AFTER the write instead of from the requested value, so a clamped value or a write `CanChange` discarded is reported as it ended up. **Tests** — new `Tests/EditorOps/TestPerformanceCVarPriority.cpp`, 3 ids. `core.cvar_priority_preserving_set.KeepsExistingSetBy` pins the helper on `r.MaxAnisotropy` (write a different value, assert the SetBy is unchanged, restore, assert both value and priority back, plus the null-pointer case). `performance.configure_texture_streaming.PreservesCVarSetByPriority` and `performance.apply_baseline_settings.PreservesCVarSetByPriority` drive the real verbs and assert per cvar that the SetBy after equals the SetBy before — which is what fails before this fix, on all six scalability members of `apply_baseline_settings` — plus, separately, that a cvar which started at or below `ECVF_SetByScalability` is still there afterwards, which is the user-facing property rather than the mechanism. The baseline test also asserts `scalabilityCVarsPinned` is present and false and that every `appliedCVars` entry matches the live cvar. **Test hygiene, since this test really does write real scalability cvars in a shared editor:** every cvar is snapshotted (value and SetBy) and restored; the restore is issued at the cvar's CURRENT priority rather than the snapshotted one so it lands even on a reverted build where the verb has just stamped `ECVF_SetByCode`; and the texture-streaming case passes each cvar the value it already holds, so on a fixed build that call is a complete no-op and only the priority stamp is under test. What no test can undo is the priority stamp a reverted build leaves — `Unset` is not reachable for this and re-setting lower is what `CanChange` refuses — and that residue is the defect, not something the test introduces. **Docs** in the same commit: a `## CVar writes here cannot outlive the call` section on `Docs/wiki-src/performance.md` (above the first `###`, so it renders on the namespace page) stating the rule, the reason a fixed Scalability priority is wrong, and the consequence for callers — a `performance.*` write is not sticky; plus new `### performance.apply_baseline_settings` and `### performance.configure_texture_streaming` sections, and both registered summaries rewritten. **Follow-up, deliberately NOT fixed here, because this is a class and the instruction was to fix the scalability-flagged ones in scope and record the rest rather than widen indefinitely.** Four more sites outside `PerformanceHandler.cpp` write a scalability-flagged cvar above the panel's priority, in descending order of harm: (1) `Handlers/Render/RenderHandler.cpp:1943` — `render.lumen_update_scene` runs `r.LumenScene.SurfaceCache.Reset 1` through `GEngine->Exec`, i.e. at `ECVF_SetByConsole`, the HIGHEST priority, on a cvar carrying `ECVF_Scalability` (`LumenSceneRendering.cpp:90`, flags `:93`); (2) `Handlers/Editor/EditorCommandHandler.cpp:675-685` — `editor.set_preferences` writes ARBITRARY caller-named cvars with a bare `Set()`, so it is unbounded, it is what `editor.md` recommends for setting several cvars at once, and no console-string guard can reach it (a warning was added to that doc line here); (3) `Handlers/Environment/LightingHandler.cpp:1385` — `r.Shadow.Virtual.Enable`, `ECVF_Scalability` (`VirtualShadowMapArray.cpp:161`, flags `:170`), bare `Set()`; (4) `Handlers/Environment/PostProcessHandler.cpp:397` — `r.ScreenPercentage` at an EXPLICIT `ECVF_SetByCode`, so a grep for bare `Set()` misses it. (3) was left alone on purpose: its `:1376-1382` comment documents the `ECVF_SetByCode` choice as part of `B-lighting-writes-vetoed-by-scalability-cvars`, which is IN-REVIEW, and changing it would also change that verb's veto semantics under a reviewer's feet. Three sites are NOT scalability-flagged and are noise for this class, named so nobody re-derives them: `GIMethodCVarHelper.h` (`r.DynamicGlobalIlluminationMethod` / `r.ReflectionMethod`, `ECVF_RenderThreadSafe` only, `IndirectLightRendering.cpp:93`/`:98` and `:104`/`:108`), `DynamicMeshCountProbe.cpp:161,170` (`geometry.DynamicMesh.TextBasedDupeTriThreshold`, no flags argument at `UDynamicMesh.cpp:33-35`, and already scoped/restored), and `PostProcessHandler.cpp:386` (`r.AntiAliasingMethod`, `ECVF_RenderThreadSafe`, `SceneView.cpp:237`/`:246`). `RenderingProjectSettingsHandler.cpp:248` writes `r.Lumen.DiffuseIndirect.Quality` with a bare `Set()`; its declaration was not located in the 5.8 Runtime tree, so its flags are UNVERIFIED rather than assumed clear. **Not compiled and not run** — the orchestrator builds and runs after all agents return. `check_test_ids.py` and `check_test_skips.py` both CLEAN."
