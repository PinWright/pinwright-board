...
- `#3-fixed-layout-and-quiet-progress` `IN-REVIEW` developer — Replaced character-count path shortening with a fixed-height custom Slate layout whose title, status, and path rows use rendered ellipsis; the full path remains in the tooltip and job payload. The public job and notification now exist before folder preflight, cache checks yield across ticker frames, and `jobs.jsonl` progress uses the registry rate limit while the editor notification still updates per asset. Live `/App` verification completed 8,192 assets with zero skips/timeouts and persisted only two progress events during a 115-second forced run.
