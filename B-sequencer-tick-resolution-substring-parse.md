...
## History
- `#2-in-review` `IN-REVIEW` developer — GO. Fixed the substring-parse defect in `SequenceHandler.cpp` `sequencer.set_tick_resolution`: dropped the `Contains("24000")`/`Contains("60000")` fast-paths (`:2355-2358`) so resolution strings parse through the rational/numeric branches (`240000`->240000/1, `600000`->600000/1, `240000/1`->240000/1 — all previously clamped 10x low); reject a non-empty unparseable resolution with `INVALID_ARGUMENT`; and echo the applied `tickResolution` as `{numerator,denominator}` (mirroring `set_display_rate`/`get_properties`) so the write is verifiable instead of a bare `{}`. Severity Medium unchanged; single-method scope, no follow-ons. Regression test: adopted the red test `PinWright.Sequencer.SetTickResolution.NumericStringNotSubstringClamped` (strengthened to also assert the echo) — observed failing pre-fix, now green (stored 240000/1 for both the numeric and rational forms). Plugin compiles clean. Files: `Handlers/Sequencer/SequenceHandler.cpp`, `Tests/Sequencer/TestSequencerTickResolutionSubstringParse.cpp`.
