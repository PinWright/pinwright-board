...
severity: Low
category: ergonomic
tags: [docs, gas, get_gas_info, gameplay-ability, cooldown, cost, ability-tags, targeting, set_ability_targeting, readback, inspect-after-mutate, wiki]
encounters: 2
lastSeen: 2026-07-02T08:48:18.5809919+03:00
---
...
## History
- `#4-additional-targeting-fields` `IN-REVIEW` reporter — Struggle-audit of a second task on this same branch (Flame-Breath drake ability kit, seed `gas.set_ability_targeting`; the judge filed the sibling `F-gas-ability-activation-tags-unauthorable`). Same-method/same-branch additional angle: beyond cooldown/cost/tags, the GameplayAbility branch of `get_gas_info` ALSO omits the **targeting** fields that `gas.set_ability_targeting` stamps onto the ability CDO (`TargetingType`, `TargetingRange`, `TargetingAngle`, `bRequiresLineOfSight`). The story's named final readback ("confirm the cone targeting — type/reach/angle/line-of-sight — actually landed on the asset") was NOT satisfiable via `get_gas_info`: it returned instancing/net/cooldown/cost/tags but no targeting, so the agent had to make an extra `asset.dump` call and Read the written `properties.json` off disk (TargetingType="cone", TargetingRange=1200, TargetingAngle=90, bRequiresLineOfSight=true) to verify — an extra round-trip for a single readback. NOTE: the `#2`/`#3` IN-REVIEW widening added `cooldownEffect`/`costEffect`/`abilityTags` only — it does NOT emit the targeting fields, so the ability-branch widening is still incomplete; the same branch (near the instancing/netExecution stamps in `GASHandler.cpp`) should also read and emit the four targeting variables so a single `get_gas_info` confirms targeting alongside cooldown/cost/tags. Do not close this ticket as fixed until the targeting fields are also emitted. Evidence: agent friction (verbatim) "gas.get_gas_info does NOT surface targeting fields for a GameplayAbility"; the iteration's own success check pre-acknowledged the gap. encounters 1 -> 2.
