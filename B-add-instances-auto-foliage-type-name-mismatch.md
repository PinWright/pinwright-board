...
encounters: 4
lastSeen: 2026-08-29T20:35:00+03:00
---
...
- `#4-population-count-for-the-silent-save` `IN-REVIEW` reporter — **Fourth encounter, adding a
  population count to consequence (2) rather than a new consequence. Status deliberately NOT changed —
  I was not asked to verify `#3`'s fix and am not acting as tester.** Observed during a bulk
  performance-tuning pass over the vegetation level's foliage types (host `EAContentExamples58`, UE 5.8;
  `Docs/map/vegetation-performance.md` § *Two Python write traps*, script `dev/perf/pw_foliage_tune.py`):
  `unreal.EditorAssetLibrary.save_asset("/Game/Foliage/Auto_<Mesh>")` silently failed — no exception, no
  false return worth acting on — for **17 of the 33** foliage types written in one pass, i.e. every one
  of them that had been auto-created rather than authored with `foliage.add_type`. The other 16 saved
  normally. `unreal.EditorAssetLibrary.save_loaded_asset(obj, only_if_is_dirty=False)` on the loaded
  object worked for all 17. This is the same mechanism `#1` already identified and this ticket already
  cites — `EditorScriptingHelpers::ConvertAnyPathToObjectPath` infers the object name from the package
  short name, so the handle normalises to `Auto_X.Auto_X` and resolves to nothing — so nothing new is
  claimed about the cause. What is new is the **ratio**: the failure is not an occasional path-shape
  slip a caller notices, it is deterministic and silent across every auto-created type in a batch, and
  the two halves of a mixed batch behave differently with no signal distinguishing them.
  **One concrete gap in `#3`'s regression coverage, offered for the tester.** `#3` renames both halves
  to `Auto_X.Auto_X`, which should make the bare package handle resolve through
  `ConvertAnyPathToObjectPath` and close this. But its test
  `PinWright.foliage.add_instances.AutoCreatedTypeHandleRoundTrips` drives the published handle back
  through `asset.exists` and `foliage.get_instances` in both forms — and **not through the save route**.
  Consequence (2) is the one this encounter measures at 17/33, and it is the one consequence the new
  test does not exercise. Adding an `asset.save` (or `EditorAssetLibrary.save_asset`) assertion on the
  bare package handle would cover it, and would also touch the "Second half, separable" persistence
  defect that `#3` explicitly leaves open. **No severity change proposed:** High already, on the same
  impact class, and a population count is a `encounters` input, never a severity input. `encounters`
  3 -> 4.
- `#5-persistence-half-carved-into-its-own-ticket` `IN-REVIEW` reporter — **Status deliberately NOT changed; no code touched.** The "second, separable half" this ticket has carried since `#1` — `McpSafeAssetSave` marks dirty and writes nothing, and the verbs publish a hardcoded `existsAfter:true` with no disk probe — is now **`B-foliage-auto-type-no-disk-write`** (OPEN, High). Carved out rather than left here because this ticket is `IN-REVIEW` on `#3`'s naming fix: a tester verifying that fix flips this to `DONE` on the half that *was* fixed, and the fix picker works only `OPEN` tickets, so the residue would have been retired without ever being scheduled. Re-verified against the current working tree (post-`#3`, with the `Auto_X.Auto_X` rename in): `PinWrightResolveOrCreateAutoFoliageTypeForMesh` still ends at `McpSafeAssetSave(AutoFT)` (`FoliageHandler.cpp:150`), `grep -c` over the whole file for both honest-reporting helpers (`SaveAssetToDiskReportingPresence`, `AddMarkDirtySaveReport`) returns **0** against 89 and 46 call sites elsewhere in `Source/`, and `existsAfter` is a bare literal at four sites — `:1081` paint, `:1237` remove, `:1479` get_instances, `:1928` add_instances. Line numbers in `#1` and `#3` for that half are stale by ~660 lines after `#3`'s helper insertion; the new ticket carries re-derived ones. **Scope for the tester of `#3` is now exactly the naming half and the `get_instances` refusal** — do not hold `DONE` on persistence. `encounters` unchanged: this is a re-scope of an already-recorded observation, not a new sighting.
