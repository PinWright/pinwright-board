...
- `#3-liveness` `OPEN` reporter — still reproduces at HEAD on `ExampleProjectWelcome`/`PersistentLevel` (`get_scene_stats` = 227 actors; `find_by_class{className:"DynamicMeshActor"}` = 53340 chars, spills to `Saved/PinWright/HttpResponses/*.json`).
- `#4-liveness` `OPEN` reporter — still reproduces at HEAD (`get_scene_stats` = 227 actors; `find_by_class{className:"DynamicMeshActor"}` = 53340 chars, spills to `Saved/PinWright/HttpResponses/*.json`; `find_by_class{className:"PlayerStart"}` = 1 row inline).
