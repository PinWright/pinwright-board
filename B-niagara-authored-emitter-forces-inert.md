...
- `#7-verified-fixed` `DONE` verifier — 2026-08-28. Plugin rebuilt from a clean tree at `b79ba53e` and verified against disk, not against the build's own success message: `UnrealEditor-PinWright.dll` 39,898,624 -> 40,644,096 bytes at 2026-08-28 08:11:48, `UnrealEditor-PinWrightGeometry.dll` 4,983,296 -> 5,113,344, canonical link with no `-000N` artifacts in `UnrealEditor.modules`. Editor restarted on that DLL and the ticket's own repro re-run. Root cause from #2 is fixed and is now measurable in the response. On the rebuilt DLL, `niagara.create_system {name:"NS_VerifyWire", savePath:"/Game/PinWrightScratch"}` then `niagara.add_emitter {emitterPath:"/Game/Atlantis/VFX/NE_Fish_A"}` returns `emittersInvokedBySystemGraph: 1` and `emitterNodesRebuilt: 2` - the SystemSpawn / SystemUpdate `UNiagaraNodeEmitter` pair that #2 proved the plugin had no code path to create. `dataInterfaceCheck: "consistent"`. Not re-measured end to end: whether particles now actually simulate was not re-shot, since that needs a level capture and this pass was scoped to plugin verification. The graph-wiring defect itself is closed.
