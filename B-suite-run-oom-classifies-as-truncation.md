...
severity: High
category: bug
tags: [automation-suite, memory, oom, garbage-collection, check-suite-log, verdict, ci, job-object, cpu-priority]
encounters: 2
costly: 0
lastSeen: 2026-09-10T05:30:00Z
---
...
- `#3-the-same-job-now-carries-the-cpu-priority` `IN-REVIEW` developer — Additional evidence, not a regression: the containment this ticket built was the right place to hang a second limit. A build or a suite run still took the whole box with it — UBT saturates every core through `cl.exe`, the suite adds the editor plus its `ShaderCompileWorker` children, all at normal priority — so the user asked for both to run below normal. Rather than a wrapper or a `SetPriorityClass` poke, `JOB_OBJECT_LIMIT_PRIORITY_CLASS` went onto the job this ticket already creates: it applies to every process in the job including children, and a process in the job cannot raise itself back above it. That is what makes it cover the cases a per-process call misses — UBT spawns compile actions through `EpicGames.Core.ManagedProcess`, which passes `NORMAL_PRIORITY_CLASS` in its creation flags (`ManagedProcess.cs:659-679`), and the job limit overrides it; nothing in the path breaks away (`ManagedProcessGroup` sets only `JOB_OBJECT_LIMIT_BREAKAWAY_OK` on its own nested job, `ManagedProcess.cs:144-155`, and UnrealBuildAccelerator passes no `CREATE_BREAKAWAY_FROM_JOB` at all). UBA's native `ProcessPriority` already defaults to BelowNormal on symmetric CPUs (`UnrealBuildAcceleratorConfig.cs:402`) but covers only UBA-executed actions, not UBT itself or a non-UBA executor, so the job stays the mechanism. **BelowNormal and not Idle**: an Idle-class job is starved whenever anything else wants CPU, which inflates wall time instead of merely yielding. Shipped in `27df561d` on `origin/master`: the Job Object interop moved out of `scripts/Run-SuiteCapped.ps1` into a dot-sourced `scripts/CappedJob.ps1` shared with a new `scripts/Run-Capped.ps1` (arbitrary command — `Build.bat`, a commandlet, an unattended editor — in the same job, one `PINWRIGHT_JOB_RESULT` line, exit 2 on TIMEOUT/MEMORY_CAP_HIT, `.bat` routed through `cmd.exe` because `CreateProcess` cannot execute one); `Run-SuiteCapped.ps1` keeps its byte-identical suite argv and gains `-PriorityClass` (default BelowNormal) plus `priority=` on the result line after `peak_gb=`; all three launch sites repointed (`mcp-test-loop` compile **and** suite, the version-matrix build leg and T1 suite, the CI build and suite steps). **Measured on this host** (UE 5.8, 63.18 GiB RAM, cap 37.91 GiB, tree `26d2b96f` plus the change): a `PDSEditor` build through the launcher sampled `dotnet` (UBT) and `cl.exe` at `PriorityClass BelowNormal` mid-build with `Result: Succeeded`; the full suite ran 5258 tests, 5257 pass, 1 fail, 95 skips — **identical to `#1`'s `TestGcEvery=25` figures**, and the one failure is the same pre-existing `PinWright.audio.authoring.set_sound_wave_properties.PublishesOrdinaryProperties` — with the editor and all five `ShaderCompileWorker` children sampled BelowNormal, `capSeenByEditor=True`, 0 OOM lines, 0 watermark markers, peak **14.66 GiB** (against 15.02) and wall **10m55s** against 10m51s at normal priority. **The whole price of the change is 0.6% of wall time**, which is the number to quote when someone proposes reverting it for speed. Docs carry the preference rule: the capped launcher is how a compile, a suite run and any unattended editor launch start, while an interactive session a human is working in stays uncapped at normal priority. Not proven: `MEMORY_CAP_HIT` on `Run-Capped.ps1` has never fired for real — its detection is the suite script's minus the two UE log-string probes, since a generic command has no OOM banner.
