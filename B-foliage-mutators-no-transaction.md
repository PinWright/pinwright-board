...
- `#2-still-live-at-head-source-only` `OPEN` reporter — **Still live at HEAD in this checkout. Status unchanged; every fact below re-derived from source this pass rather than carried over from `#1`, whose line numbers have all moved.** `Handlers/Environment/FoliageHandler.cpp` contains **zero** occurrences of `FScopedTransaction` (`grep -c` returns `0`), and the only `Modify()` calls of any kind in the file are **four bare `IFA->Modify()`** at **`:541`**, **`:678`**, **`:684`** and **`:1237`** — a grep for `->Modify(` returns those four lines and nothing else. Mapped to their verbs, all four still sit **after** the mutation they are supposed to cover: `:541` closes the paint loop in `foliage.paint` (registered `:151`); `:678` and `:684` follow `Info.Instances.Empty()` and `Info->Instances.Empty()` in `foliage.remove` (registered `:592`); `:1237` closes the add loop in `foliage.add_instances` (registered `:979`). `foliage.add_type` (`:847`) and `foliage.create_procedural` (`:1273`) still contain **no `Modify()` at all**, which is `#1`'s "strictly worse" finding, unchanged. The file is now **1611** lines, up from the 1233 the body cites, so the growth has not brought a transaction with it.

  **Explicitly SOURCE-ONLY: no undo was attempted this session.** That matters and is not a formality. This ticket's claim is about **what `editor.undo` can reverse**, and nobody exercised `editor.undo` against any `foliage.*` mutation in this pass. What is established is the mechanism — no transaction is opened, so no undo record can be written — and that is a strong inference, not the observation the claim is phrased as. A tester closing this ticket must actually call a foliage mutator, call `editor.undo`, and report what the level looks like afterwards; a source read cannot substitute, because `Modify()` outside a transaction can still interact with an ambient transaction opened by something else in the editor.

  **Cross-link: `B-paint-layer-destroys-other-layer-weights`** (OPEN, filed this session) carries the **same missing-transaction property in a different handler** — its landscape-paint write also mutates without an `FScopedTransaction`. Recorded here as evidence that the gap is **not confined to `foliage.*`**: two independent handler families, found independently, both mutating outside the transaction system. If a fixer treats this as a foliage-local defect they will fix it foliage-locally and leave the same hole open elsewhere; the shape of the right fix is a convention applied across mutating handlers, not five `FScopedTransaction` insertions in one file.

  Frontmatter: `encounters` seeded to **2** (absent means 1 per the README) and `lastSeen` set, because this is a second confirmed sighting of the defect rather than re-analysis of the first. Severity unchanged at `Medium`.
