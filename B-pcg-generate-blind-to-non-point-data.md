...
- `#2-datatypes-rows-and-omitted-pointcount` `IN-REVIEW` developer — Shape fix in `Source/PinWrightPCG/Private/Handlers/PCG/PCGGenerateReadback.h` plus the doc corrections, both in one change. **Enumeration.** The single-cast counting loop became `SummarizeGeneratedData`, one pass that records `Data->GetClass()->GetPathName()` at the exact site the `Cast<UPCGBasePointData>` already held the class, and also resolves PCG's own type via `StaticEnum<EPCGDataType>()->GetIndexByValue(Data->GetDataType())` — so a row reads `{class: "/Script/PCG.PCGParamData", pcgDataType: "Param", count: 3}`. `pcgDataType` is **omitted, not guessed**, when the reflected bitmask enum does not name the value (`EPCGDataType` is `UENUM(meta=(Bitflags))`, `PCGCommon.h:235`, so combinations such as `Concrete`/`Spatial` have no single name); the ticket's measured type resolves to `Other` (`UPVData::GetDataType()`, `PVData.h`). `CountGeneratedPoints` is retained and now delegates to the same pass, so the existing `PointCountReadback` test still exercises the shipping symbol. Rows carry a `(null data)` key for a `TaggedData` entry with no data object, on the `(no static mesh)` precedent, so the rows always sum to `dataCount`. Payload gains `dataTypes[]` and `distinctDataTypeCount` beside `dataCount`; no row cap, per the ticket's own argument that distinct classes bound it. **Omission, which is the load-bearing half.** `FGenerationReadback` gains `PointDataCount` (output data objects the counter could actually read), written into the payload unconditionally whenever `graphOutputAvailable` is true, and **`pointCount` is now emitted only when `pointDataCount > 0`**. A graph whose output holds no point data therefore has no `pointCount` field at all rather than a confident `0` — "produced no points" and "produced something that is not points" stop being the same receipt — and a `warnings[]` entry says so explicitly, names the classes it could not count inside, and steers the caller to `dataTypes`/`dataCount` rather than to `pointCount` or `instanceCount`. **Doc defect, `Docs/wiki-src/pcg.md`.** The three cited lines had already been renumbered by `B-pcg-generate-instancecount-blind-to-species`; corrected in place at their current positions. The `## Running a graph` steer (`:42`, namespace page) no longer nominates a single field and routes by what the graph produces. The `### pcg.generate` bullet that claimed `instanceCount` is *"non-zero exactly when the graph put geometry in the world"* is replaced: it is non-zero exactly when the graph spawned instances it still owns, and is **structurally zero** for a graph that spawns neither instances nor actors. A new `dataTypes[]` bullet is what a non-spawner generation is now judged by, and the `pointCount` bullet states both omission reasons and drops "check `instanceCount`" as the universal fallback. Completion-result field list updated. **Regression test** `PinWright.pcg.generate.NonPointOutputIsDistinguishableFromNothing` in `Source/PinWrightPCG/Private/Tests/PCG/TestPCGGenerateHandler.cpp`: two real `UPCGComponent`s on two actors in the editor world — one whose private `GeneratedGraphOutput` UPROPERTY is written by reflection with three `UPCGParamData` plus one empty tagged slot (`UPCGParamData` stands in for `UPVData`, which needs an experimental plugin; the counting path has exactly one cast so every non-point type behaves identically), one that produced nothing — driven through the real `ReadGeneration` and `BuildGenerationPayload`. It asserts the ticket's exact impossible comparison: both receipts report `instanceCount: 0` and are identical in every shared number, yet the working one omits `pointCount`, reports `pointDataCount: 0`, and names its output in `dataTypes[]` while the other has nothing to name. Differential three ways — pre-fix `pointCount` was written unconditionally (omission assertion fails), `dataTypes` did not exist (naming assertion fails), and the test names `PinWrightPCG::NullDataKey` and `FGenerationReadback::PointDataCount` so a revert fails compilation. Doc assertions are split across the namespace and method pages because rendering stops at the first `###`. One existing fixture updated, not widened: `PayloadReportsRealPointCount` now sets `PointDataCount = 2` to declare that its `PointCount = 8` was a measurement; its assertions are unchanged. Not built or run here — orchestrator owns the build and suite.
