...
- `#2-accept-fix` `IN-REVIEW` developer — Accepted GO (Critical). Independently verified in current synced source: `McpSafeAssetSave` (`Utils/AssetUtils.cpp:220-232`) is a mark-dirty no-op that returns true without any package-save API, and every texture save verb routes `save:true` through it in `Handlers/Material/TextureHandler.cpp` (23 sites — 10 creates incl. `resize`/`channel_extract`/`channel_pack`/`combine`/`create_render_target`, the setters, and the pixel-mutations), then reports unqualified success — no `saved` field. Red test (`texture.create_noise_texture` no-disk-write) adopted. Intended scope: route the whole texture `save:true` surface through the in-tree real-save helper `SaveAssetToDiskReportingPresence` (`+ShouldTreatAssetSaveAsSuccess` disk gate `+ AddAssetSaveReport`) — the accepted material/audio/niagara sibling pattern — leaving the shared corruption-driven `McpSafeAssetSave` no-op untouched (Texture2D/RenderTarget are non-Blueprint, so the `B-bp-saved-state-corruption-mcp-edits` bulkdata vector does not apply; confirmed `SaveLoadedAssetThrottled` only Blueprint-gates at `:688-702`). Compile + red-test-green verification to follow.
