...
- `#6-message-log-occluder-again` `OPEN` reporter - Reproduced the `#4` cause on UE 5.8, host `unreal-fpv-new`, plugin `8748c637`, fresh editor started by `editor_start {map}`: after `editor.play` the editor again opened a `Message Log` top-level window at (0,0,1373x813). `drive.click` on `W_ForcedLogoutPopup_C_0/OkButton` and `W_LoginOverlay_C_1/W_Login/BackButton` returned `no_change_within_budget` with empty diffs; `drive.list_windows` showed the occluder, and closing its tab via `drive.click {surface: editor_chrome}` fixed every later click. The occluder appears on each PIE start in this host, so the missing `TARGET_OCCLUDED` check is hit every session. Cheap this time (the ticket named the cause; about 5 calls).
- `#7-message-log-reopens-per-pie-session` `OPEN` reporter - Reproduced the `#4` cause again, UE 5.8, host `unreal-fpv-new`, plugin `8748c637`, PIE in the level viewport on `L_Core` (school-computer verification pass). The `Message Log` top-level window reappeared at (0,0,1376x816) on two separate `editor.play` starts, each time after the previous session had logged `PIE:` message-log warnings. While it was up, `drive.click` on `W_SchoolNameLogin/AdminLinkButton/SCommonButton` (twice) and on `W_SchoolNameLogin/PasswordLoginButton/SCommonButton` returned `no_change_within_budget` with empty diffs; a Python `CommonButtonBase.call_method('HandleButtonClicked')` on the same button fired the handler, which briefly looked like a product bug in the button. After `drive.list_windows` and closing the tab via `drive.click {surface:"editor_chrome", window_index:1, handle:".../SDockTab[0]/.../SButton[2]"}` the same clicks worked first time (`LogWindows: LaunchURL http://localhost:8080`). Costly: about ten extra calls and a detour into the widget tree and CommonUI source before checking for the occluder. A click refused with a named occluding window (or `editor.play` closing the Message Log it opens) would have saved it.
- `#8-route-gate-and-target-occluded` `IN-REVIEW` developer - **Two root causes, one fix site.** Both are about where Slate routes the injected pointer event. Coordinates and capture are not involved. `FSlateApplication::ProcessMouseButtonDownEvent` (and the move and wheel paths) hit-tests through `LocateWindowUnderMouse`. That call asks the platform for the window under the cursor first and hit-tests only inside that window, with no fallback (`SlateApplication.cpp:1151-1158`). **(1) Occluding top-level window** (`#4`/`#6`/`#7`): the covering window wins the routing and swallows the click. **(2) Linux: the platform's window-under-cursor is stale right after the warp.** `FLinuxApplication::CurrentUnderCursorWindow` changes only when SDL's `WINDOW_MOUSE_ENTER`/`LEAVE` event is pumped (`LinuxApplication.cpp:952-1006`). That happens on a later frame, but `drive.click` warps and clicks in the same call. So the first click into a different top-level window was hit-tested in the window the pointer had just left and landed on nothing. This covers `#1`/`#3` and explains why `#2` and later clicks at the same spot worked. **Baseline repro** (this host, UE 5.8 Linux, PIE in the level viewport on `L_Core`, unmodified plugin, `BA Welcome Screen`/`Message Log` windows at (70,27,646x366) over the viewport's top half, real pointer parked in the `PinWright Setup` window): `drive.click` on the covered `W_TutorialButton` returned `no_change_within_budget` with an empty diff, no journal events, and no screen change. `drive.click` on the **uncovered** `W_OptionsButton` did the same, with the screen unchanged 3 s later. The identical second click on `W_OptionsButton` opened the settings screen. **The press-handled bool `#1` asked for is not a usable signal.** `editor.simulate_input mouse_click` at the covered point reported "handled by a widget", but the occluder handled it. `ProcessMouseButtonDownEvent` also returns `true` on every path except an input preprocessor (`SlateApplication.cpp:5402`), so surfacing it would have misled callers. It was deliberately not added. **Fix:** `FDriveActionCommon::RunAction` (`Handlers/Drive/DriveActionCommon.cpp`) now resolves the target's own `SWindow` (`FindWidgetWindow`), moves the pointer to the target's center, and injects only once `FDriveInput::WindowUnderPoint` (new: the same `LocateWindowUnderMouse` call the injection makes, so it predicts the injection exactly) names that window. If it does not, a core-ticker poll waits up to 0.3 s for the platform to catch up. A window that still owns the point after that makes the call refuse with the new `TARGET_OCCLUDED` (`Handlers/ErrorCodes.h`), payload `{handle, x, y, occluding_window}`, and nothing is clicked. The gate covers every handle-bearing Slate pointer verb: click, hover, scroll, drag press, and type/key with a handle. `os_input` and retainer virtual windows are exempt. `FDriveInput::ResolveNativeWindowUnder` now reuses `WindowUnderPoint`. Wiki: `docs/wiki-src/drive.md` `drive.click` section. **Verified live after the fix, same layout and pointer parking:** the covered `W_TutorialButton` click returns `TARGET_OCCLUDED ... covered at (214, 278) by window 'BA Welcome Screen'`. The **first** click on `W_OptionsButton` from the stale-pointer state opens settings. The first `W_BackButton` click with the pointer parked in `PinWright Setup` returns to the menu (diff +228/-220). **Tests** (`Tests/Drive/TestDriveClickOcclusion.cpp`, fixture windows across real frames): `PinWright.drive.click_occlusion.OccludedTargetIsRefused` (failure direction: `TARGET_OCCLUDED`, the occluder is not the target's own window, OnClicked count 0; the old code never emits that code, so it fails there) and `PinWright.drive.click_occlusion.UncoveredTargetIsClicked` (counterfactual: success, exactly one click). A run of `PinWright.drive` (135 tests) passed 135/135, both new tests with no skip markers. In the same run `PinWright.core.error_codes.AllEmittedCodesAreRegistered` passed. `RegistryAdoptingFilesUseConstantsOnly` failed on `Handlers/UI/WidgetSetHandler.cpp`, another in-flight change not in this diff. Not addressed here: a widget already holding mouse capture still takes the press through the captor path (`#3`'s hypothesis (c); not seen in this repro, `cursor_captor:null`). `E-drive-click-no-change-on-slow-transition` is unchanged: the fixed `W_OptionsButton` click still reports `no_change_within_budget` although the settings screen opened. Plugin commit `61c243f5`.
