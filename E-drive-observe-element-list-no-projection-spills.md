...
- `#3-postfix-evidence-and-label-filter` `IN-REVIEW` reporter — Additional cross-session evidence (2026-07-17 UI-drive session; ~183-element menu with Russian labels). TWO findings, both verified in source. (1) **`max_bytes` is correctly implemented — NOT a bug.** It is threaded on both the sync game/editor path (`DriveObserveHandler.cpp:52` reads it, passes to `BuildObservation` at `:63` → `FilterObservationElements`, the byte-cap loop at `DriveHandlerCommon.cpp:297-315` that drops elements once the running serialized-byte estimate exceeds `MaxBytes` into `omitted_count`, always keeping >=1) AND the async web path (`DriveWebHandlers.cpp:343` reads it, `:369-370` applies the same `FilterObservationElements`). A session `drive.observe {interactables_only:true, max_bytes:9000}` (surface omitted → game) returned **235,464 chars** and spilled — but that is the **documented screenshot-dominates case, not a `max_bytes` failure**: the caller left `screenshot` at its default `true`, so the inline base64 Set-of-Mark PNG dominated the payload; `max_bytes` caps ONLY the element list, verbatim as the param doc already states (`DriveObserveHandler.cpp:27`: *"max_bytes caps ONLY the element list, so a scan/discovery observe must ALSO drop the default inline screenshot (screenshot:false, or screenshot_mode:file)…"*). The session's own `UNKNOWN_PARAMS` error enumerating `max_bytes` among valid params confirms the fix was compiled into the running editor. Conclusion: **no `B-drive-observe-max-bytes-ignored` ticket** — the code honors `max_bytes`; the screenshot-must-be-off caveat this fix already documents is load-bearing and just recurred in the wild (a scan observe that forgets `screenshot:false` still spills regardless of `max_bytes`). (2) **A separate lever is still missing: a server-side text/label substring filter.** `drive.observe {filter:"СОЗДАТЬ"}` → `[UNKNOWN_PARAMS] Unknown parameter(s) for 'drive.observe': [filter]` — no `filter`/`label_contains`/`query` param exists (confirmed absent across the Drive handlers). The session paid ~6 extra Bash calls grepping the one-line, quote-escaped spill JSON (even `dd` byte-offset extraction) to find button handles by their Russian labels. This is the *which-elements* lever, orthogonal to this ticket's *byte-size* lever, so it is filed as its own ergonomic **`E-drive-observe-no-label-filter`** rather than folded into this IN-REVIEW byte-cap fix. Severity of THIS ticket unchanged (**Low**) — the byte-cap fix stands and aspect (2) is tracked separately. encounters 1→2, `lastSeen` refreshed.
