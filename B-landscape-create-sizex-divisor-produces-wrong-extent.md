...
- `#2-derive-divisor-and-report-measured-extent` `IN-REVIEW` developer — Fixed (1), (2) and (4); (3) deliberately deferred, see below. **Arithmetic:** the literal `floor(size / 1000)` at `LandscapeHandler.cpp:440-446` is gone. `sizeX`/`sizeY` are now only *captured* on the request thread (the divisor's two terms — `ComponentSizeQuads` and `DrawScale` — are not both known there), and the conversion runs inside the existing `AsyncTask(GameThread)` lambda right after the spawn, as `ComponentsX = max(1, RoundToInt(sizeX / (ComponentSizeQuads * DrawScale.X)))`, with the scale READ off the spawned actor (`GetActorScale3D`) rather than assumed. Verified in engine source at HEAD for this checkout: `ALandscapeProxy`'s constructor sets `FVector(128,128,256)` identically on 5.3/5.5/5.7/5.8, and `ULandscapeEditorObject::NewLandscape_DefaultScale` is `FVector(100,100,100)` — reading the actor is what keeps the conversion true across both and any future change. `FLandscapeConfig::SubsectionSizeQuadsValues = {7,15,31,63,127,255}` / `NumSectionValues = {1,2}` confirmed in `LandscapeConfigHelper.cpp`, so the reachable extents really are `N * ComponentSizeQuads * DrawScale` and nothing else — the ticket's claim that 1000 is correct for no legal combination holds. **Snap, not refuse:** the request is rounded to the nearest reachable component count. Refusing would break a documented optional parameter for almost every value a caller could pass (the step is 8064 cm at the defaults, so exact expressibility is the rare case, not the common one), and the house convention for a constrained quantity is measure-and-report, not reject — `LightingHandler.cpp`'s `lighting.configure_shadows` is the shipped shape (`requested` named separately from the measured value, a warning when they diverge). `round` rather than `floor` per the ticket: flooring left every request short by up to a whole component on top of the divisor error. **Reporting:** the success response now carries `worldSizeX`/`worldSizeY` MEASURED off the built grid (`ULandscapeInfo::GetLandscapeExtent` quad extent × the actor's draw scale — so a hollow landscape measures nothing rather than reporting the size it was meant to be), `worldExtentMeasured`, `drawScale{x,y,z}`, and `requestedSizeX`/`requestedSizeY` only when those were supplied. `warnings[]` carries the snap message (naming requested vs built, the component span, and the count) whenever the two differ, and a separate message when the extent could not be measured — in which case `worldSizeX`/`worldSizeY` are OMITTED, never zeroed. `componentsX`/`componentsY` and the actor's default label now report the post-snap count. **Siblings: none shared it.** `landscape.create_procedural_terrain` is, despite the name, the layer-paint verb and has no sizing arithmetic; `environment.build.create_procedural_terrain`'s `sizeX` is a grid vertex count (paired with an explicit `spacing`), not world units, and never divided by anything. A grep for `/ 1000` across the whole plugin found the divisor at those two lines only. **Deliberately NOT fixed: ticket item (3), the missing `scale`/`quadSize` parameter.** That is a feature, not this defect: it would widen the create surface, and the ticket itself notes the scale must go through the landscape's own scale-change path rather than a bare `SetActorScale3D` for the `ULandscapeInfo::DrawScale` reason — untested here. With (1) and (2) shipped, the residual is honest quantisation the caller can see and work with, not silent wrong data. Re-file (3) as an `F-` ticket if an exact extent is wanted. **Regression test:** `Source/PinWright/Private/Tests/Environment/TestLandscapeCreateExtentHonesty.cpp`, id `PinWright.landscape.create.ReportsAchievedExtent` — behavioural, drives the production handler through its async path. Asks for 5000 × 5000 cm at the defaults (not expressible; the step is 8064) and asserts the response reports `worldSizeX`/`worldSizeY`, that they equal the extent measured INDEPENDENTLY off the actor (walking `LandscapeComponents` section bases × the actor scale — a different collection from the `XYtoComponentMap` the handler measures through), that the built count is the nearest reachable one for the actor's real scale, that `requestedSizeX` is named separately and differs from the achieved extent, that `warnings[]` is non-empty, and that `drawScale` is published. Counterfactual: under the old divisor the count is 5 rather than 1, the built extent is 40,320 cm against a 5,000 cm ask, and `worldSizeX` is absent from the response entirely — the first three assertion groups each fail on their own. NOT COMPILED and NOT RUN (build/suite owned by the wave orchestrator); id checked against every `PinWright.landscape.create*` literal for the dot-prefix-adoption trap — no collision. No new error codes, so nothing to register in `ErrorCodes.h`. Nothing in the ticket contradicted the source: the 8064 figure, the absent `SetActorScale3D`/`SetRelativeScale3D`, the `:755-766` response field list and the engine line references all re-verified at HEAD.
