...
- `#2-go-implement-fixture` `IN-REVIEW` developer — GO. Re-verified against UE 5.7 source: only game_features.list ships (GameFeaturesHandler.cpp), and the state machine only exposes a NON-EMPTY GetActions() for a bCanContainContent:true plugin carrying a real on-disk UGameFeatureData (bCanContainContent:false yields a transient EMPTY GFD; GameFeaturePluginStateMachine.cpp:3296-3302), while a content plugin missing that asset errors Registering (:3456). Implementing a test-only fixture that GENERATES a synthetic on-disk GF plugin + a UGameFeatureData carrying one action at test time, drives it Registered->Active via the real UGameFeaturesSubsystem::ChangeGameFeatureTargetState, asserts it reaches Active + GetActions() is non-empty, then deactivates/terminates and deletes the on-disk plugin — no committed asset, no Lyra, no stubbed handler, no leaked state. Co-locating under Tests/World/ (no Tests/Systems/ bucket exists in the taxonomy). SHIPPED: Tests/World/GameFeaturesTestFixture.h (reusable named-namespace helper PinWrightGF::CreateOnDiskGFPluginWithAction / DeleteOnDiskGFPlugin — writes a CanContainContent uplugin under Plugins/GameFeatures/ and SavePackage's a UGameFeatureData carrying one UGameFeatureAction_AddCheats at /<Plugin>/GameFeatureData, all generated + deleted at test time) and Tests/World/TestGameFeaturesFixture.cpp (test PinWright.game_features.fixture.SyntheticPluginReachesActiveWithActions: latent-drives ChangeGameFeatureTargetState to Active, asserts Active + GetActions().Num()>0 + enumeration through the real game_features.list RPC, then deactivate/terminate + delete with no leaked active state). Compiles clean; ran headless -> Result={Success}, GetActions().Num()=1. Hygiene notes for the reuse path: created RF_Public (not RF_Standalone) so GC reclaims the generated package at plugin unmount (avoids the plugin-manager leak ensure), and validate-on-save is suppressed via the UDataValidationSettings CDO by reflection (avoids a Localization-validator ensure) — no production code changed, no Build.cs dependency added.
