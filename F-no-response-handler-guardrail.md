...
`Source/PinWright/Private/Dispatch/RpcDispatcher.cpp:770-776`
...
- `#9-repoint-citations-after-module-rename` `DONE` reporter — Citation maintenance only; **no claim in this ticket changes and the status is untouched**. The plugin module directory was renamed `Source/EditorAutomationRpcGateway/` → `Source/PinWright/` (plugin commit `8962f163`), and `Source/EditorAutomationRpcGatewayTests/` was folded into `Source/PinWright/Private/Tests/`, so every citation under the old root was an **unresolvable path** a fixer could not open — not a stale line number. 1 body citation repointed in place and verified against plugin HEAD `ef8a1f1b`. **Not just a path — the gap this ticket reports is closed and every line range in it is stale.** `:375-381` is now `++RegistryGeneration;` plus the head of `DrainAutoRegistrations`. The text-format `NO_RESPONSE` is `:770-776`, and the guard the ticket says is missing **now exists** at `:438-442`: the auto-registered bridge checks `bCaptureWired && !Capture.bWasCalled` after `RegCopy.Func(Ctx)` and calls `ResolveMissingHandlerResponse` (`:340-358`), which `ensureMsgf`s and resolves with `NO_HANDLER_RESPONSE`. The ticket sits at IN-REVIEW only because testers could not exercise it — `bCaptureWired` is set only on the `Subsystem == nullptr` branch (`:417-422`), which live traffic never takes. Other stale ranges, left verbatim: text-format branch `342-407`→`724-803`; auto-registered path `214-243`→`379-461`; legacy TMap path `416-427`→`810-820`; the tester rows' `:274` guard and `253-265` capture branch → `:438-442` and `:399-422`. Sweep-wide record, including every case that could not be repointed: `E-module-rename-citation-sweep`.
