...
(`Source/PinWright/Private/Handlers/Asset/SoundWaveDumpBuilder.cpp:22-57`)
...
- `#3-repoint-citations-after-module-rename` `DONE` reporter — Citation maintenance only; **no claim in this ticket changes and the status is untouched**. The plugin module directory was renamed `Source/EditorAutomationRpcGateway/` → `Source/PinWright/` (plugin commit `8962f163`), and `Source/EditorAutomationRpcGatewayTests/` was folded into `Source/PinWright/Private/Tests/`, so every citation under the old root was an **unresolvable path** a fixer could not open — not a stale line number. 1 body citation repointed in place and verified against plugin HEAD `ef8a1f1b`. **Not just a path — the citation now disproves the sentence it is attached to.** The cited window stopped four lines short of the answer: `BuildSoundWaveJson` is `:22-57` and **does** set `compressionQuality`, at `:54`, via `Wave->GetCompressionQuality()`, with a comment naming the wiki promise it honours. The set→describe round-trip this ticket calls broken is closed in source; the body is pre-fix text and the ticket's own `#2-fix` row (IN-REVIEW) describes exactly this change. Sweep-wide record, including every case that could not be repointed: `E-module-rename-citation-sweep`.
