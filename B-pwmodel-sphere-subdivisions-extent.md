...
- `#2-doc-clause-and-extent-test` `IN-REVIEW` developer — "Added the `radius/sqrt(3)` clause to the `sphere` `subdivisions` parameter text in `PwModelParser.cpp` (option (b)) so `model.describe_ops` names the factor, the axis it applies to and the `subdivisions=3` boundary; recorded the same derivation with its engine citations at the `AppendSphereBox` call in `GeometryOps_Primitives.cpp` and in `Docs/wiki-src/model.authoring.md` + `Docs/pwmodel-format.md`. Option (a) — a warning at the `ClampRangeWarn` floor site — was NOT taken: `PinWright.Model.Compiler.SphereSubdivisionsBelowTwoIsClampedAndWarned` pins `subdivisions=2` as warning-free, and warning at the floor would fire on every deliberate use of the cheap 12-triangle primitive. Regression test `PinWright.Model.Parser.SphereSubdivisionsDocPublishesTheFloorHalfExtent` (`Tests/Model/TestPwModelSphereExtentDocs.cpp`) generates the sphere at `subdivisions=2` and `=3`, measures the max half-extent off the mesh bounds, and asserts the published text quotes the ratio FORMATTED FROM THAT MEASUREMENT (`0.5774`) plus `radius/sqrt(3)` and `half-extent` — so a dropped or wrong factor fails, and a reworded description does not. It fails on the pre-fix text. The named `sphere subdivisions` doc-domain exception in `Tests/Geometry/TestGeometryClampDomains.cpp` was NOT retired: it tracks a different gap (the clamped domain `2-256` and the `0-or-less takes the default 16` tier are still unpublished), its `Contains 2-substring` assertion still holds, and closing it is out of this ticket's scope. NOT COMPILED — build and suite are the reviewer's step."
