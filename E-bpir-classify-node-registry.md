...
- `#5-repoint-citations-after-module-rename` `DONE` reporter — Citation maintenance only; **no claim in this ticket changes and the status is untouched**. The plugin module directory was renamed `Source/EditorAutomationRpcGateway/` → `Source/PinWright/` (plugin commit `8962f163`), and `Source/EditorAutomationRpcGatewayTests/` was folded into `Source/PinWright/Private/Tests/`, so every citation under the old root was an **unresolvable path** a fixer could not open — not a stale line number. No body citation was rewritten here. 3 citations sit in history rows and are left verbatim per the append-only rule. The one citation is in history row and stays verbatim. Map: `Decompiler/GraphWalker.cpp:69-232` → `Source/PinWright/Private/Decompiler/GraphWalker.cpp:69-234`; the range still contains everything the row names (`GetSemanticsRegistry` `:83-107`, `ClassifyNode` `:146-234`, MacroInstance `:160`, Tunnel `:192-200`, CallFunction `:204-217`, the `GetSuperClass()` walk `:224-231`) but stopped one line short of `return Unknown` at `:233`. Sweep-wide record, including every case that could not be repointed: `E-module-rename-citation-sweep`.
