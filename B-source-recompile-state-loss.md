...
encounters: 4
lastSeen: 2026-09-03T06:50:00+05:00
---
...
- `#9-nanite-has-no-source-spelling` `OPEN` reporter — Additional evidence, not a status change (left `IN-REVIEW`). Same guard, a **different reason the named remedy is impossible**: `#6`/`#7`/`#8` are about a material path compared in two spellings, which `#8` normalises. This is a field the `.pwmodel` format has **no spelling for at all**. Observed 2026-09-03, UE 5.8, PinWright at this checkout's HEAD, building the FPS compound map (map as forcing function; host `CLAUDE.md` § "What this project is for"). Sequence, all on `/Game/FPS/Env/Meshes/SM_ENV_Container20` and repeated identically on `SM_ENV_Barrier_Jersey`: `model.compile` the source, then `asset.nanite_rebuild_mesh {enableNanite:true}` (the only verb that can set it), then edit the source and `model.compile` again. The second compile refuses, or with `overwrite:true` warns:
  ```
  [PWSRC_RECOMPILE_UNMANAGED_STATE] Recompiling .pwmodel asset '/Game/FPS/Env/Meshes/SM_ENV_Barrier_Jersey'
  would discard state the source does not reproduce: naniteEnabled is 'true' but the source does not
  declare it; Nanite settings can be changed in the Static Mesh editor. It differs from the state
  recorded by the previous source compile.
  ```
  `model.describe_ops` lists 76 ops and no `nanite`; there is no `nanite`/`naniteEnabled` key anywhere in the `.pwmodel` grammar or in `model.authoring.*` (checked `document-structure`, `materials`, `collision`, `authoring-contract`, `re-compiling-and-provenance`); `grep -ril nanite Saved/PinWright/wiki/` returns no model-source page. So unlike the material case there is no string the author can write, and the message's own suggestion — "Nanite settings can be changed in the Static Mesh editor" — describes how to *re-set* the state after the loss, not how to prevent it. **Consequence measured on this build:** Nanite is silently off after every source edit, which for a Nanite-only project is a rendering regression that no response field reports as a regression — `model.compile`'s payload has no `naniteEnabled`, so the only way to notice is to read the warning text. 11 meshes had to be re-Nanited by hand after one authoring pass, and the two rebuilt again later had to be done twice. **Fix wanted:** give `.pwmodel` a `nanite { enabled = true ... }` block (or a `nanite = true` key on the model header) so the source can own the field the guard is guarding; until then the diagnostic should not claim the state "differs from the state recorded by the previous source compile" for a field no source can record. **Workaround used:** `overwrite:true` on every recompile, followed by `asset.nanite_rebuild_mesh` and an explicit `mark_package_dirty` + `save_asset(only_if_is_dirty=False)`, verified against the `.uasset` mtime on disk. `encounters` 3 -> 4, `lastSeen` refreshed.
