...
severity: Low
category: bug
tags: [bpir, decompiler, anim-blueprint, animgraph, unknown-entry, regression]
encounters: 2
...
- `#6-regressed-on-animbp-graphs` `OPEN` reporter — **Regression of the `#5` contract, on AnimBlueprints; `encounters` 1 -> 2.** UE 5.8, `X:\src\unreal\unreal-fpv-dev`, plugin before `fa755a4f`. An AnimGraph carries nodes but no decompilable K2 bodies, so by this ticket's ruling it is `UnreachableGraph` and its `bpir.txt` should be the single `# (graph has no decompiled bodies)` marker. Instead `ABP_Manny` emitted **24 blocks** of `entry event UnknownEntry() { }` — anonymous stubs standing in for AnimGraph output-pose roots, one per root. Same axis this ticket settled (marker vs empty-entry form), arrived at from a different direction: `#2`/`#4` moved *empty entries* onto the canonical `entry ... {}` form, and the regression is that AnimGraph roots are now admitted as entries at all, so the classification never reaches `UnreachableGraph` and the marker branch cannot fire. The output is not merely inconsistent, it is misleading — 24 named-looking entry stubs where the asset has none.
- `#7-skip-unknown-entry-signature-roots` `IN-REVIEW` developer — Fixed in plugin commit `fa755a4f` ("Fix three asset-dump regressions in property and bpir export"). Cause: `IsBlueprintEntryNode`'s class-agnostic engine-compile-root backstop admits AnimGraph output-pose nodes, which the emitter has no grammar for, so each one produced an `UnknownEntry()` stub. The decompiler now skips an entry whose signature is the `UnknownEntrySignature` fallback **and** that gates no connected exec output; with those removed the AnimGraph classifies as empty again and `AssetDumpBuilder` restores the `# (graph has no decompiled bodies)` marker. Aspect version `bpir.txt 8->9` (dump core `2->3` in the same commit). **Verified** by re-sweeping `asset-dumps/App` with this build — 8,388 / 8,388 assets, 0 skips, and **no** `bpir.txt` in the mirror contains `UnknownEntry()`. Awaiting an independent tester: the skip predicate is deliberately narrow (fallback signature *and* no connected exec output), so the case worth probing is a graph with a genuinely unknown-signature entry that *does* gate exec — it must still emit, not be silently dropped.
