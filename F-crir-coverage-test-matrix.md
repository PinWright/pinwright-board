...
`Source/PinWright/Private/Tests/Assets/` — one per row above.
...
- `#7-repoint-citations-after-module-rename` `DONE` reporter — Citation maintenance only; **no claim in this ticket changes and the status is untouched**. The plugin module directory was renamed `Source/EditorAutomationRpcGateway/` → `Source/PinWright/` (plugin commit `8962f163`), and `Source/EditorAutomationRpcGatewayTests/` was folded into `Source/PinWright/Private/Tests/`, so every citation under the old root was an **unresolvable path** a fixer could not open — not a stale line number. 1 body citation repointed in place and verified against plugin HEAD `ef8a1f1b`. The cited test directory `Private/Tests/CRIR/` **never existed** — it is absent from the pre-rename tree too. The 16 `TestCRIR*.cpp` files and `CRIRTestHelpers.h` live in `Source/PinWright/Private/Tests/Assets/`; the CRIR implementation is `Source/PinWright/Private/CRIR/`. Sweep-wide record, including every case that could not be repointed: `E-module-rename-citation-sweep`.
