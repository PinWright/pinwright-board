...
- `#2-triage` `IN-REVIEW` developer — GO. Defect confirmed at `RenderHandler.cpp:444-455` and reproduced by a red test. Fixed in `Handlers/Render/RenderHandler.cpp`: before creating/attaching the MID, enumerate the base material's texture parameters via `BaseMat->GetAllParameterInfoOfType(EMaterialParameterType::Texture, ...)` and reject an unknown `parameterName` with `PARAMETER_NOT_FOUND` (the already-registered code, not the ticket's proposed unregistered `PARAM_NOT_FOUND`), returning `validTextureParameters` (the list of real texture-parameter names) in the error payload. Regression test `PinWright.render.attach_render_target_to_volume.PhantomTextureParameterRejected` (`Tests/EditorOps/TestRenderAttachPhantomParam.cpp`) flips red→green (Fail pre-fix, Success post-fix); plugin compiles clean. Scope unchanged; sibling `material.authoring.set_texture_parameter_value` (`MaterialAuthoringHandler.cpp:1973`) remains out of scope per the ticket.
