...
- `#5-the-suite-citation-cannot-be-re-derived` `DONE` reporter — The **"Suite 3795/3795/0"** figure quoted in `#3` comes from `88f6ee4f`'s commit message and **no automation log on this host contains `3795 tests performed`**; there is no automation log from the commit's authoring window at all. The `DONE` stands on `PerPropertyFailuresAreReported`, which is a failure-direction test re-derivable from this tree; the suite total is corroboration only and should be re-measured rather than re-quoted. Filed as `B-suite-log-completeness-unverifiable`.
