...
- `#3-per-package-probe-shipped-enumeration-still-open` `OPEN` reporter — Partial relief landed from `E-python-cannot-mark-package-dirty` (IN-REVIEW): `asset.is_dirty` now answers the SINGLE-package question read-only (`{assetPath, package, isDirty}`, no load / no dirty / no save), and `unreal.PinWrightPackageLibrary.is_package_dirty(obj)` / `is_package_dirty_by_path(path)` answer it from Python. That covers the narrower "did MY asset stay clean" case this ticket's `#2` evidence describes; it does NOT cover the enumeration this ticket asks for ("which packages are dirty right now" with `dirtyCount` across the whole loaded set), so the ticket stays OPEN. Also worth recording against the "reflected API for 'list every dirty UPackage' is non-obvious" line above: the engine already exposes it — `unreal.EditorLoadingAndSavingUtils.get_dirty_map_packages()` and `get_dirty_content_packages()` are both `UFUNCTION(BlueprintCallable)` (`FileHelpers.h:135,143`) and return `TArray<UPackage*>`, so a `python.execute` enumeration is a two-liner today. The remaining gap is a typed JSON MCP verb, not a missing capability, which argues for keeping this at `Low`.
