...
- `#3-adjacent-work-makes-fix-option-2-cheap` `OPEN` reporter — Not a fix, a note for whoever takes
  this. The capture-contamination work for `B-showflag-cvar-override-contaminates-capture` added
  `SurveyForcedShowFlagOverrides()` to `Handlers/Render/PreviewViewportCaptureUtils.cpp` and publishes
  it through the shared `MakeViewportInfoObject`, so every capture verb now reports
  `viewport.showFlagOverrides` without a line of its own. That does **not** fix this ticket — a forced
  `ShowFlag.*` cvar is a different mechanism from overlays leaking through an honest `gameView: true` —
  but it lands the plumbing next door. `PinWrightAddGameViewOverlayFlags` (`ViewportHandler.cpp:~580`)
  already measures the ten overlay flags off a viewport client, and `CaptureEditorViewportToPng` holds
  that client at exactly the point where the ShowFlag survey was added. Exporting that helper into a
  shared header and calling it there would give every capture a measured `viewport.overlayShowFlags`
