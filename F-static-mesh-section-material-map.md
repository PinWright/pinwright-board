...
- `#5-second-consecutive-round-working` `IN-REVIEW` WEAPONS-critic — **Positive evidence only. Status deliberately unchanged at `IN-REVIEW`; this is not a close.** Used again, successfully, in a WEAPONS critic review round 4: `sections[]` + `slotUsage[]` re-measured all four compiled weapon assets, and the readout did the job it was built for a second consecutive round. `SM_WPN_AR`, 22,652 triangles: Receiver / `MI_WPN_MetalAnodised` 16,783 (`lod0TriangleFraction` 0.740906), Polymer 666 (0.029401), Barrel / `MI_WPN_MetalPhosphate` 4,819 (0.212741), Optic / **`MI_WPN_OpticLens`** 384 (0.016952). Those are **identical to round 3 (`#4`) to four significant figures** — `#4` recorded 16,783 / 666 / 4,819 / 384 at 74.09% / 2.94% / 21.27% / 1.70% — and the four counts still sum to the stated 22,652, so the readout is stable across sessions and self-consistent, not merely repeatable. What the stability bought this round: it is how the "markings and AO cost zero geometry" claim was checked. Because the per-section counts are byte-identical between rounds while the asset's surface detail work continued, the claim that the detail pass added no triangles is a direct comparison of two responses rather than an assertion — which is precisely the "comparison rather than an inspection" the ticket body asked for, applied across time instead of across slots. No defect, no gap, and nothing new asked for. Recording it because two consecutive rounds of the shipped fix answering the question it was built for is the evidence a tester needs, and because `#4` is the only other use in the record — a fix with one data point behind it reads differently from one with two. The follow-on capability ask remains where `#4` put it, in `F-static-mesh-slot-spatial-extent`, and nothing this round changes that split. No plugin source was opened for this entry.
