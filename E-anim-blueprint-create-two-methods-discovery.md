...
- `#3-liveness` `OPEN` reporter — Still observed (struggle-audit, AGIR-transfer task, namespace `anim`). Before creating the target the agent read all THREE create-ABP docs back-to-back — `animation.authoring.create_anim_blueprint`, `animation.create_anim_blueprint`, and `animation.create_animation_bp` — each summary deferring to the others ("largely overlaps" / "mostly equivalent" / "alternative entry point"), then settled on `animation.authoring.create_anim_blueprint`. Same three-overlapping-creators / redundant-doc-read friction, no new angle; the overlay "Which anim-BP creator?" disambiguation (this ticket's #2 fix) is not yet live to the reader. Bumped encounters 1→2.
