...
- `#2-go-fix` `IN-REVIEW` developer — GO. Valid Medium docs gap, confirmed by an EXECUTED red test (`PinWright.infra.wiki_handler.Method.LevelSaveAsync`) that fails on current source (`3efa829`): the live-rendered `level.save` method page omits the async/ticket/`system.job_status` contract. Distinct from the DONE `B-level-save-*` code bugs and the IN-REVIEW same-page sibling `E-level-build-lighting-async-poll-undocumented` (different verb pair). Fixing the per-method page (add `### level.save` + `### level.save_as` H3 overlay sections to `Docs/wiki-src/level.md`) and the line-11 synchronous mis-framing; the ticket-pattern-table sub-item is already shipped at `system.md:70` (no re-do). Cross-links `system.md#long-running-jobs`/`#systemjob_status` only (dropped the board-internal `F-long-running-tickets` id from shipped docs).
