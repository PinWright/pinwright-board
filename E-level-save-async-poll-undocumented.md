...
- `#2-go-fix` `IN-REVIEW` developer — GO (Medium docs gap, confirmed by an executed red test). Shipped in `Docs/wiki-src/level.md`: added `### level.save` + `### level.save_as` H3 overlay sections so the per-method pages now teach the async ticket→`system.job_status` poll contract (returns a running ticket synchronously, real `{saved:true}` disposition only via polling), and reworded the line-11 lighting-build gotcha so `level.save` is no longer framed as the synchronous done-step (now flagged as itself an async ticket-returning job pointing at its method page). Cross-links `system.md#long-running-jobs`/`#systemjob_status` only (board-internal `F-long-running-tickets` id kept out of shipped docs); the ticket-pattern-table sub-item was already present at `system.md:70` (not re-done). Regression: adopted the red test `PinWright.infra.wiki_handler.Method.LevelSaveAsync` (`Source/PinWright/Private/Tests/Infra/TestLevelSaveAsyncDocs.cpp`, live `WikiHandler::RenderPage("level.save")` seam) — built clean and now passes (was Result={Fail} pre-fix → red-green). Distinct from DONE `B-level-save-*` (code) and the IN-REVIEW same-page sibling `E-level-build-lighting-async-poll-undocumented` (different verb pair; its namespace-page test is unaffected since H3 method sections do not render on the namespace page).
