...
- `#11-verified-in-fps-build` `IN-REVIEW` VFX — **The module-input addressing works. Direct route retried once per PLAN rule 2 and it succeeded; leaving `IN-REVIEW` for the tester.** Live editor port 27145, UE 5.8, EAContentExamples58, scratch duplicate `/Game/FPS/VFX/Scratch_TicketRetry/E_TR_Curve` (an `asset.duplicate` of `SimpleSpriteBurst` + `niagara.add_module` `/Niagara/Modules/Update/Size/ScaleSpriteSize` into `ParticleUpdateScript`, entryId `8FFC76A3437700FFD470E7AA753DA615`). **Write:** `niagara.set_curve_keys {assetPath:"/Game/FPS/VFX/Scratch_TicketRetry/E_TR_Curve", entryId:"8FFC76A3437700FFD470E7AA753DA615", inputName:"Uniform Curve Sprite Scale", keys:[{time:0,value:0.2},{time:0.35,value:1.0},{time:1,value:0.05}]}` -> `{"addressing":"moduleInput","valueMode":"data","createdOverride":true,"dataInterfaceClass":"/Script/Niagara.NiagaraDataInterfaceCurve","channel":"","written":3}` — the exact response the `## Fix` section predicted. The `DATA_INTERFACE_NOT_FOUND` wall `#1`-`#3` and `#8`-`#9` cycled against is gone.
  **Read-back round-trip holds.** `niagara.get_curve_keys` on the same address returned all three keys with the values as written (`0.20000000298023224`, `1`, `0.05000000074505806` at times `0`, `0.3499999940395355`, `1`), `writable:true`, `valueMode:"data"`. The multi-channel form works too: `set_curve_keys {entryId:"8B92C307443B49670E4498BB75442CCF" (ScaleColor), inputName:"Linear Color Curve", channel:"a", keys:[{time:0,value:1},{time:1,value:0}]}` -> `createdOverride:true, dataInterfaceClass:"…NiagaraDataInterfaceColorCurve", written:2`, and the unfiltered read returned four channels with only `a` populated.
  **Discoverability closed.** `inputName:"Not A Real Curve Input"` -> `[MODULE_INPUT_NOT_FOUND] Module 'ScaleSpriteSize' declares no stack input 'Not A Real Curve Input'. Its data-interface inputs are: Uniform Curve Sprite Scale, Non-Uniform Curve Sprite Scale.` `#1`'s "the caller cannot discover a name that would work" is answered. `inputName:"Scale Alpha"` (a float) -> `[INCOMPATIBLE_DATA_INTERFACE] … is type 'NiagaraFloat', which is not a data interface` — an honest type refusal, not the old uniform not-found.
  **The module asset was NOT edited**, which is the safety property the fix rests on: a SECOND `ScaleSpriteSize` placement added to the same emitter (entryId `A7C246F04FD6AE0DA38FCFB4C8F4961B`) still reports no override, and `C:/UE_5.8/Engine/Plugins/FX/Niagara/Content/Modules/Update/Size/ScaleSpriteSize.uasset` still carries its shipped mtime `2026-06-14 06:18:19` and 102759 bytes.
  **`#2`'s "FRichCurve keys are binary so on-disk verification is impossible" is WRONG and the correction matters** — the keys are plain little-endian float32 in the export data and a byte scan finds them. `asset.save {force:true}` -> `saveState:"written"`, `sizeBytes:89303`; `E_TR_Curve.uasset` mtime `2026-09-05 20:48:38 +0300`, 89303 b; a float32 scan of the file recovers the authored pairs in order and adjacent — time `0.0`@28468 / value `0.2`@28472, time `0.35`@28495 / value `1.0`@28499, time `1.0`@28522 / value `0.05`@28526 — plus `NiagaraDataInterfaceCurve` ×2 and `NiagaraDataInterfaceColorCurve` in the name table. So a curve write is verifiable against disk without a read verb at all.
  **Two things I could NOT verify, stated so no one over-reads this.** (a) **No runtime or visual proof.** Capture is owned by the VFX lead on this stream and I was barred from it, so nothing here shows the ramp actually driving sprite size on screen; `#4`'s sixteen-emitter regression suite is still unrun and `E_Explosion_Shockwave` / `E_Explosion_DustRing` are still uncorrected. (b) **The `valueMode:"default"` read half does not work on this module and the override is not seeded.** `get_curve_keys` against an un-overridden `Uniform Curve Sprite Scale` — before any write, and on the second fresh placement after — refuses `[DATA_INTERFACE_NOT_FOUND] Module input 'Uniform Curve Sprite Scale' carries no override value and its module script declares no default data interface.` So the `## Fix`'s reviewer step "a SECOND fresh `ScaleSpriteSize` placement must still report `valueMode: "default"` with the stock linear 0->1 ramp `#2` read out of the binary" **cannot be executed as written**: the stock ramp is still unreadable through the API, and `#2`'s binary parse remains the only way to see it. Consistent with that, the created override is **not** seeded from the script default despite the wiki's "seeded from that default" claim — the ColorCurve override came back with `r`/`g`/`b` empty after an `a`-only write, so a single-channel edit does NOT keep the authored shape of the rest and a caller must write every channel it wants. Both are narrower than the original ticket and neither blocks authoring; they are the residue a tester should decide on.
