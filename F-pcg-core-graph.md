...
**Fix:** New `Source/PinWrightPCG/Private/Handlers/PCG/`
...
Add focused tests under `Source/PinWright/Private/Tests/` covering
...
- `#6-repoint-citations-after-module-rename` `DONE` reporter — Citation maintenance only; **no claim in this ticket changes and the status is untouched**. The plugin module directory was renamed `Source/EditorAutomationRpcGateway/` → `Source/PinWright/` (plugin commit `8962f163`), and `Source/EditorAutomationRpcGatewayTests/` was folded into `Source/PinWright/Private/Tests/`, so every citation under the old root was an **unresolvable path** a fixer could not open — not a stale line number. 2 body citations repointed in place and verified against plugin HEAD `ef8a1f1b`. 1 citation sits in history rows and is left verbatim per the append-only rule. **A module split, not a prefix change.** PCG left the main module in plugin `b02e4fc7` (“Split optional engine-plugin integrations into gated sub-modules”): the handlers are now `Source/PinWrightPCG/Private/Handlers/PCG/` (13 files) and the tests `Source/PinWrightPCG/Private/Tests/PCG/`. **The integration mechanism this ticket specifies was also replaced:** it asks for `PCG`/`PCGEditor` via `TryAddConditionalModule()`, but at HEAD PCG is a **hard** `PrivateDependencyModuleNames` entry (`PinWrightPCG.Build.cs:53`) in a gated sub-module that fails the build outright when the engine plugin is missing (`:36-42`); `TryAddConditionalModule` survives only in `PinWright.Build.cs` for Metasound and Localization. The feature claim holds — the handlers landed; the integration claim does not. Sweep-wide record, including every case that could not be repointed: `E-module-rename-citation-sweep`.
