...
- `#1-no-usage-flag-read-or-write` `OPEN` reporter — Found during the look-dev polish pass over
  ...
  **Medium** on the readback-omission band, at its expensive end (the fallback is a bespoke audit
  ...
  declined both ways.
- `#2-niagara-variant-visible-in-editor-as-the-default-checkerboard` `OPEN` reporter — Same defect, Niagara flavour, and it is **worse than `#1` describes**: this one is not invisible in the editor, it renders the grey Default Material checkerboard right there in the viewport. Found building the FPS combat VFX package on UE 5.8 / EAContentExamples58. I authored ten particle materials through `material.compile_mgir` and `material.authoring.create_decal_material`, bound them to Niagara sprite and mesh renderers, and captured the level. A muzzle-flash heat sprite came back as a **large grey checkerboard quad** filling a third of the frame. `asset.validate` said `isValid:true`, `asset.get_material_stats` reported the right shading model and sampler count, `material.authoring.compile_material` reported `compileSucceeded:true, compiledWithErrors:false`, and `property.get` on the renderer confirmed the correct `Material` binding. Every reachable readback said the material was fine while the pixels showed the Default Material.
...
- `#4-usage-block-on-both-read-verbs-plus-declaredusages-on-every-probe` `IN-REVIEW` developer — Read side implemented, write side left as the documented `property.set` route per `#1`'s reasoning. New `Plugins/PinWright/Source/PinWright/Private/Handlers/Material/MaterialUsageFlags.h` holds the whole vocabulary: `GetUsageName` (the engine accessor, so no local table can drift), `FindUsagePropertyName` (derives `bUsedWith<Name>` and then VERIFIES it against `UMaterial`'s reflection data, so an engine that breaks the pattern yields an empty string rather than a fabricated property name), `DeclaresUsage`, `DeclaredUsageNames`, `BuildUsageReport`/`AddUsageReport`. `material.authoring.get_material_info` and `material.authoring.get_material_instance_info` (`Handlers/Material/MaterialAuthoringHandler.cpp`) now emit a `usage` block: `declared[]`, `flags[]` with one row per `EMaterialUsage` carrying `{usage, declared, property}`, `autoSetInEditor` (the `bAutomaticallySetUsageInEditor` bit that makes the defect invisible in-editor), and a `hint` naming the write route — `property.set` on the base material's `bUsedWith*` member followed by `compile_material`, with the reason the editor's own auto-set is not a fix. The instance verb additionally passes the parent, adding `parentDeclared[]`, `parentPath` and a per-row `overridden`, because the flag that decides whether an instance renders on a given consumer usually lives on the parent. The short `declaredUsages[]` list also rides on the `shaderCompile` block of EVERY material verb (`Handlers/Material/MaterialShaderState.h`), so a caller who never asks for the read verb still sees it. Deliberately NOT done: no usage-setter verb (per `#1`), no MGIR `property MaterialUsage:` syntax, and no `warnings[]` in `actor.add_component` / the scatter verbs — that diagnostic belongs with the ISM tickets it rides on. Regression test `PinWright.material.usage.UsageBlockCarriesEveryFlagWithItsProperty` in `Source/PinWright/Private/Tests/Material/TestMaterialUsageReporting.cpp` asserts one row per `EMaterialUsage`, that `declared[]` follows a written flag and omits an unwritten one, and that the SkeletalMesh row names `bUsedWithSkeletalMesh`. Counterfactual: revert `MaterialUsageFlags.h` and `BuildUsageReport` does not exist, so the block is absent and every assertion in that test fails.
