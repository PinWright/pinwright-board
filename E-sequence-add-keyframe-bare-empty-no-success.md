...
- `#8-liveness` `OPEN` reporter — still reproduces at HEAD (REALISM-mode `IntroShowcase` 24fps 5s establishing-shot cinematic, focus `sequencer`, 19 calls): both `sequence.add_keyframe` `Location` calls (frames 0 and 120) returned a bare `{}`, and the run again fell back to a `sequencer.list_sections{includeKeys}` readback purely to confirm the two Location keys landed (keyCount:2). Same symptom, same forced readback; no new angle.
