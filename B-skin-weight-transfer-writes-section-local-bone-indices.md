...
- `#4-second-reader-confirms-section-local` `IN-REVIEW` developer — Evidence record; no status change, still IN-REVIEW. The premise underpinning the `#3` fix — that `FImportedSkinWeightProfileData::SkinWeights` holds SECTION-LOCAL slots, so the outbound crossing to `SourceModelInfluences` must resolve through `Section.BoneMap` — previously rested on a **single** reader: `FMeshUtilities::CreateImportDataFromLODModel` at `MeshUtilities.cpp:5774`, plus the struct comment. That was thin support, because it is the same function this ticket already flags above as internally inconsistent on adjacent lines (`:5772-5779`). A **second, independent reader** has now been read, in the non-legacy runtime path and a different module: `C:\UE_5.8\Engine\Source\Runtime\Engine\Private\SkeletalMeshLODModel.cpp:1643-1662` walks `Sections` in order, indexes `SkinWeightProfileData.SkinWeights` by a running per-section vertex counter (`Index++` once per `Section.SoftVertices` entry), and resolves every slot through that same section's map — `if (BoneWeight != 0 && ensure(BoneMap.IsValidIndex(BoneIndex))) { BoneIndex = BoneMap[BoneIndex]; }`. The decisive part is the `ensure`, not the resolution: a section's `BoneMap` is typically far smaller than the reference skeleton, so if the field held reference-skeleton indices that `ensure` would fire on essentially every mesh carrying a profile. The contract therefore now has two independent readers in two modules, plus the struct comment separating `SourceModelInfluences` as "the result of the imported data before the chunking" (`SkinWeightProfile.h:75-77`). **What is still NOT established:** the engine code that WRITES `SkinWeights` during a build was never located. This is corroboration from the read side twice over, not proof from the write side — treat it as strong converging evidence, not a demonstrated invariant, and if a writer is ever found that contradicts it, `#3`'s resolution direction is what has to be revisited.
