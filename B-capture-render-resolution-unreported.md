...
**Verdict: TRUE.** `CaptureEditorViewportToPng` sized the readback to the requested file dimensions but neither selected nor reported the editor viewport's primary resolution fraction; UE 5.8's fallback derives that fraction from viewport pixel count and explicitly ignores `r.ScreenPercentage`.

The shared capture scope now installs a draw-scoped view extension that overrides both the per-view primary fraction and the view family's independent low-DPI secondary fraction to 1.0 without mutating the viewport client's private preview-resolution optional. The extension reads the finalized view family during `BeginRenderViewFamily` and reports `viewport.render` with internal width/height, primary/secondary/combined fractions, screen percentage, effective AA method, pin state, upscale state, and temporal-suppression state. This is the new default because the existing tests contain comparisons and thresholds but no golden image or exact pixel baseline that requires the heuristic path. The settle receipt also includes mean absolute RGB delta, maximum channel delta, and changed-pixel fraction for its final pair.

Files changed: `Source/PinWright/Private/Handlers/Render/CaptureResolutionViewExtension.{h,cpp}`, `Source/PinWright/Private/Handlers/Render/PreviewViewportCaptureUtils.{h,cpp}`, `Source/PinWright/Private/Handlers/Render/FlatRegionStats.{h,cpp}`, `Source/PinWright/Private/Tests/Render/TestCaptureRenderResolution.cpp`, `Source/PinWright/Private/Tests/Render/TestPoseListCaptureStability.cpp`, and `Docs/wiki-src/render.md`. Tests: `PinWright.render.capture.RenderResolutionReported`, `PinWright.render.capture.NativeRasterMatchesDecodedPng`, and `PinWright.render.capture.PixelConvergenceDetectsLocalChange`.

Deliberately not changed: no compatibility parameter preserves heuristic rendering, because no baseline requiring it was found; `system.inspect.get_viewport_info` remains the separate related ticket; no build, editor run, capture, or automation run was performed under this worker brief.
...
- `#3-native-still-policy-and-receipt` `IN-REVIEW` fixer — "Code inspection confirmed the missing selection/reporting path. `CaptureEditorViewportToPng` now scopes `SetPreviewScreenPercentage(100)` plus preview enablement, clears `TemporalAA`, restores entry behavior on exit, and every shared viewport response publishes `viewport.render`; the final settle pair publishes pixel-difference evidence beside the frame mean. Added structural/unit tests and updated the render wiki. No Unreal build, editor capture, or automation run was permitted in this worker pass."

- `#4-draw-time-raster-and-state-fix` `IN-REVIEW` developer — "Independent verification found that the first implementation ignored `SecondaryViewFraction` and converted an unset private preview-resolution optional into an explicit value during restore. The capture now pins primary and secondary resolution only on its view family, records their combined raster during the draw, and never calls the client's preview-percentage setters. `PinWright.render.capture.NativeRasterMatchesDecodedPng` drives a real Cube preview when an RHI exists, compares the reported raster with the decoded PNG, and behaviorally proves the client's unset flag/value state survives. No build, editor capture, or automation run was permitted in this follow-up."
