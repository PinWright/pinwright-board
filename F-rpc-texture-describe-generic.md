...
`asset.dump` now writes a generic `texture.json` sidecar for any `UTexture` via `TextureDumpBuilder::BuildTextureJson` (`Source/PinWright/Private/Handlers/Asset/TextureDumpBuilder.cpp`). The sidecar covers `kind`, `textureClass`, `size.{x,y,z}`, `arraySize`, `pixelFormat`, `compressionSettings`, `lodGroup`, `srgb`, `mipGenSettings`, `neverStream`, and `WITH_EDITORONLY_DATA` `source.{x,y,slices,format}` for `UTexture2D`, `UTextureCube`, `UTexture2DArray`, `UTextureCubeArray`, `UVolumeTexture`, and `UTextureRenderTarget` / `UTextureRenderTarget2D`.
