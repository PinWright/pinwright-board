...
- `#2-refuse-literal-over-linked-pin` `IN-REVIEW` developer — The literal branch of `ApplyModuleMutation`'s `SetModuleInput` operation (`Source/PinWright/Private/Handlers/Niagara/NiagaraEditHandler.cpp`) now classifies the input's override pin with `NiagaraEdit::ClassifyModuleInputBindings` — the same walk the inspect / asset.dump `valueMode` readback uses — before writing. Any mode other than `local` (dynamicInput / linked / data / objectAsset / expression / connected) means the pin has an inbound link, and a literal is refused with `MODULE_INPUT_OVERRIDE_LINKED` naming the mode and the driving parameter or dynamic-input script, instead of stamping a `DefaultValue` the graph never reads and echoing it back. The escape hatch is a new optional `breakExistingLink` boolean: with it, the link and its orphaned upstream chain are removed via `NiagaraResetModuleInput::ClearModuleInputOverride` (the same clear the linked-parameter and dynamic-input branches already make) before the write, and the response carries `replacedOverride: {valueMode, source}` naming what the literal displaced — so the `value` echo is only ever emitted for a pin the graph actually reads. No new `ERR_*` constant: this file spells codes as raw literals and is not registry-adopting, and `FNiagaraEditError::Make` is not an emission site the registry scanner matches. Test added: `PinWright.niagara.set_module_input.RefusesLiteralOverLinkedOverride` in `Source/PinWright/Private/Tests/Niagara/TestNiagaraSetModuleInputLinkedOverride.cpp` — wires a stock `Add_Float` dynamic input onto a `SpawnRate` module input via `NIRTestFixtures::SetModuleInputDynamicInput`, asserts the bare literal write is refused and leaves the chain intact (no false success, no false echo), then asserts the `breakExistingLink` write succeeds, reports `replacedOverride.valueMode == "dynamicInput"` with the displaced script path, and leaves an override pin with zero `LinkedTo` carrying the literal.

- `#3-verified-against-built-binary` `DONE` verifier - Behavioural repro against the running editor (PinWright HEAD `b79ba53e`, UBT "Target is up to date"), 2026-08-28, UE 5.8, on a scratch `asset.duplicate` of `/Niagara/DefaultAssets/Templates/Systems/SimpleExplosion` to `/Game/PinWrightScratch/NiaVerify/NS_Verify_A` (deleted afterwards, never saved). **Fixed, and in both of the scopes `#1` named.** (a) *Dynamic-input chain*: `SimpleSpriteBurst`/`AddVelocity` `Velocity Speed` read `valueMode:"dynamicInput"` before the write; `set_module_input {value:777}` now returns `[MODULE_INPUT_OVERRIDE_LINKED] Module input 'Velocity Speed' is driven by an inbound link (valueMode 'dynamicInput', source /Niagara/DynamicInputs/UniformRange/V2/RandomRangeFloat.RandomRangeFloat), so a literal written on its override pin would never be read.` where `#1` measured `success:true, linked:false, value:"777.0"`. (b) *`User.*` binding*: after re-binding the same input to `User.VerifySpeed`, `{value:42}` is refused by the same code naming `valueMode 'linked', source User.VerifySpeed` - so the refusal keys off "the pin has an inbound link", not off dynamic inputs specifically. **The escape hatch works and takes effect**: `{value:777, breakExistingLink:true}` returned `success:true, pinId:C7B1A1E7..., value:"777.0", replacedOverride:{valueMode:"dynamicInput", source:"/Niagara/DynamicInputs/UniformRange/V2/RandomRangeFloat.RandomRangeFloat"}`, and the subsequent `niagara.inspect {includeStack:true}` readback shows that input as `valueMode:"local", value:"777.0"`. `#1`'s honesty-bounded inference - "the post-write `valueMode` would still read `dynamicInput`" - is settled the other way for the fixed path: it changes to `local` exactly when the write is one the graph will read. The link teardown removed only the `RandomRangeFloat` node (whole-system stack module count 154 -> 150; per-emitter diff recorded in `B-niagara-reset-module-input-corrupts-stack` `#3`). Not re-run: the `NS_Bubbles_Stream` asset and its unlit capture - with the write refused there is no silent no-op left to photograph.
