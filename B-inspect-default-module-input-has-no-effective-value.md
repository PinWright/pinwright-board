...
## History
- `#4-bumped-by-cost` `OPEN` orchestrator — Severity Medium -> High by cost. Costly encounters counted: #1 (a scratch emitter was created and deleted and three surfaces — `niagara.inspect`, `niagara.graph.get`, `niagara.decompile_nir` — were probed against the rebuilt plugin before the input's effective value was still unreadable), #2 (the independent `NS_Blood` report reached the same wall and had to dig out the `PersistentGuid`-vs-`PinId` linkage and hand-decode three `VarData` byte arrays to recover three values), #3 (its own words: "four extra RPCs and a manual little-endian float decode to learn two numbers the stack UI shows greyed-out", on a third module none of the earlier encounters touched). The working route is a two-to-four RPC `property.get` walk plus a by-hand float decode, which is past the README's cheap bar in every encounter. Reach also applies: the three encounters are three independent streams (the FPS VFX retry pass, the `NS_Blood` agent, the water diagnosis) and `niagara.inspect` is the read every Niagara session starts from.
- `#3-third-encounter-subuvanimation-playback` `OPEN` VFX — Third independent encounter, third module, port 27145, UE 5.8, EAContentExamples58, 2026-09-07, in a read-only diagnosis of `NS_Impact_Water`'s Column emitter. `SubUVAnimation` governs the sprite's flipbook and **every one of its 24 numeric inputs reads `valueMode: "default"` with no `value`** — `Play Rate`, `Frames Per Second`, `Playback Time`, `Start Time Offset`, `SubUV Curve`, all of them. Confirmed the three surfaces the ticket names are still silent, on a module none of the prior encounters touched: (a) `niagara.inspect {includeStack:true}` reports `valueMode: "default"`, no value; (b) `niagara.graph.get` on the system emits the `SubUVAnimation` function-call node with **only its static-switch pins** — the numeric inputs are absent from the node entirely, so there is not even an anonymous default pin to guess at from the placement side; (c) the emitter's `spawnRapidIteration`/`updateRapidIteration` stores hold `Start Frame Offset`, `Start/End Frame Range Override` and `SubUV Lookup Index Scale` but **omit `Play Rate` and `Frames Per Second` outright**, so their absence from the store is not evidence of anything — `InitializeParticle.Lifetime` = 2 sits in the same store on a path the `Lifetime Mode: Random` switch has gated off. That last point is worth adding to the ticket's "no other surface" list: the rapid-iteration store is not merely incomplete, it is **unsound as a signal in both directions** — presence does not mean active, absence does not mean default. The `property.get`/`property.list` route from this ticket worked first try and settled it: `property.get {objectPath:"/Niagara/Modules/Update/SubUV/V2/SubUVAnimation.SubUVAnimation:NiagaraScriptSource_0.NiagaraGraph_0", propertyName:"VariableToScriptVariable"}` → `Module.Sub UV Play Rate` → `NiagaraScriptVariable_27`, whose `Variable.VarData` is `[0,0,128,63]` = `0x3F800000` = **1.0**, `DefaultMode: Value`; and `Module.SubUV Play Rate Time` → `NiagaraScriptVariable_33`, `DefaultMode: Binding`, `DefaultBinding: {Name: "Particles.Age"}` — which is the load-bearing half, because it decides whether the flipbook is driven by absolute age or normalized age and therefore which frame of a 64-tile atlas a sprite shows at a given instant. **New sub-angle:** the route reads a *value* fine but the `DefaultMode: Binding` case is the one an author most needs and least expects, and nothing in the placement-side readback hints that an input is bound rather than valued — `valueMode: "default"` covers both. A fix should distinguish them (e.g. `valueMode: "scriptDefault"` with `value`, versus `"scriptBinding"` with the bound attribute name). Severity unchanged (Medium). Cost here: four extra RPCs and a manual little-endian float decode to learn two numbers the stack UI shows greyed-out.
...
- `#5-implemented` `IN-REVIEW` developer — Fixed on both surfaces the ticket names. `Source/PinWright/Private/Handlers/Niagara/NiagaraEditTypes.{h,cpp}`: new `NiagaraEdit::FModuleInputDefaultInfo` plus `DescribeScriptVariableDefault` / `ResolveModuleInputDefault`, which read the module script graph's `UNiagaraScriptVariable` (`GetAllMetaData().Find(var)`, falling back to the exported `UNiagaraGraph::GetScriptVariable(FName)` because the metadata map is keyed on name *and* type while the enumerated stack input can carry the resolved type) and encode `DefaultValue` through `UEdGraphSchema_Niagara::TryGetPinDefaultValueFromNiagaraVariable` — the same canonical pin-default string the `local` path already emits. A `Value` declaration with no allocated data reports the mode and no value rather than the type's zero, because that engine call silently substitutes one. `Source/PinWright/Private/Handlers/Niagara/NiagaraDumpBuilder.cpp` `BuildModuleInputsJson` now stamps `defaultMode` / `defaultValue` / `defaultBinding` on every `valueMode:"default"` entry, so the `DefaultMode: Binding` case history `#3` singled out (`SubUV Play Rate Time` → `Particles.Age`) is distinguishable from a constant. The graph aspect's anonymous MapGet default pins became joinable in the same file: `AddMapGetDefaultPinOwner` reads the protected `PinOutputToPinDefaultPersistentId` through a `using`-shim (the class has no `NIAGARAEDITOR_API`, so it is resolved by name and cast) and stamps `defaultForOutputPin` + `defaultForPinId` on each default pin — the pairing is resolved inside the builder rather than exposing the `PersistentGuid` id space. `niagara_stack.json` 6→7 and `niagara_graphs.json` 3→4 in `Source/PinWright/Private/Handlers/Asset/AssetDumpCache.cpp`; `docs/wiki-src/niagara.md` gained the `moduleInputs[]` field list under `### niagara.inspect`. Tests: `PinWright.niagara.dump.ModuleInputScriptDefault` (`Source/PinWright/Private/Tests/Niagara/TestNiagaraModuleInputDefaults.cpp`) drives `DescribeScriptVariableDefault` on constructed Value/Binding script variables and `BuildModuleInputsJson` on a placed stock `AddVelocityInCone`; counterfactual: revert `NiagaraDumpBuilder.cpp`'s default block and `Cone Axis` carries no `defaultMode`/`defaultValue`, so `TryGetStringField(TEXT("defaultMode"))` fails and the `(1,0,0)` component assertions never run — which is exactly the state that left `(1,0,0)` and the degenerate `(0,0,0)` indistinguishable. `PinWright.niagara.dump.MapGetDefaultPinPairing` (same file) asserts every anonymous MapGet default pin of the `AddVelocityInCone` script graph names its output pin and that the one naming `Module.Cone Axis` holds X=1; counterfactual: revert `AddMapGetDefaultPinOwner` and `defaultForOutputPin` is absent, so paired-vs-anonymous counts differ and the Cone Axis default is unreadable again.
- `#6-acceptance-string-corrected` `IN-REVIEW` developer — The `## Fix` / Acceptance section's expected string `"(X=1.0,Y=0.0,Z=0.0)"` is **not** the canonical Niagara pin-default format and no implementation will produce it. `UEdGraphSchema_Niagara::TryGetPinDefaultValueFromNiagaraVariable` routes through the registered `INiagaraEditorTypeUtilities::GetPinDefaultStringFromValue`, which for a `Vector3f` emits comma-separated components with no `X=`/`Y=` labels and no surrounding parentheses — `1.000000,0.000000,0.000000`. That agrees with this ticket's own measured evidence: the `NiagaraNodeParameterMapGet_1` pin table above reads `defaultValue: '1.000000,0.000,0.000'` off the same declaration. The acceptance string was a paraphrase, not a read. `PinWright.niagara.dump.ModuleInputScriptDefault` therefore asserts the comma-separated form (splits on `,` and checks the three components are 1/0/0) rather than a label-bearing literal, and `defaultValue` deliberately stays byte-identical to what a `local` override's `value` carries, which is the property the ticket actually needs — the two are diffable without a type-aware branch. Reviewers should read the acceptance block as "Cone Axis reports Value / (1,0,0)", not as a literal string match.
- `#7-default-read-from-the-wrong-field` `IN-REVIEW` developer — `PinWright.niagara.dump.ModuleInputScriptDefault` failed on a stock `AddVelocityInCone`: `Expected 'Cone Axis default X is 1' to be 1.000000, but it was 0.000000` — i.e. `#5` published the degenerate `(0,0,0)` that this ticket exists to rule out, for exactly the input it measures. Root cause in `Source/PinWright/Private/Handlers/Niagara/NiagaraEditTypes.cpp` `DescribeScriptVariableDefault`: it sourced the bytes from `UNiagaraScriptVariable::GetDefaultValueData()`, i.e. the private `DefaultValueVariant`, which is **not** the field the engine keeps current. `FNiagaraScriptVariableDetails::OnValueChanged` (`NiagaraScriptVariableCustomization.cpp:419-450`) — the parameter panel's edit, how a module's declared default is actually authored — writes only `ScriptVariable->Variable` and mirrors it to the Map Get default pins, never touching the variant; `UNiagaraGraph::GetVariable` (`NiagaraGraph.cpp:4314`) returns `Variable`, and `UNiagaraNodeParameterMapGet::CreateDefaultPin` (`NiagaraNodeParameterMapGet.cpp:129-142`) builds the pin default string from it. So on a stock module the variant sits zero-filled while `Variable` and the default pin carry the authored value — which is why this ticket's own sibling test `MapGetDefaultPinPairing`, reading the pin, passed with X=1 in the same run that this one read 0. Fix: read `ScriptVariable.Variable` first and fall back to the variant only when `Variable` has no allocated data **and** the variant's byte count matches the type size (`SetData` memcpys the type's full size out of the source buffer, so an unmatched variant was an out-of-bounds read as well). Regression test added in the same file as a host-independent third case (1a'): a script variable whose variant was allocated at 0 and whose `Variable` was then set to 2.5 — the panel-authored shape — must report 2.5; counterfactual: restore the variant-first read and it reports 0.0, the same way it reported `(0,0,0)` for Cone Axis. `niagara_stack.json` 7 -> 8 in `Source/PinWright/Private/Handlers/Asset/AssetDumpCache.cpp`, because a version-7 dump carries the type's zero for every panel-authored input. **Not fixed, same defect one surface over:** `Source/PinWright/Private/NIR/NIRDecompiler.cpp:236-243` compares an override's bytes against `GetDefaultValueData()` to decide whether to suppress a NIR line, so it compares against the same stale variant — an override equal to the real default is still emitted, and one equal to the type's zero is wrongly suppressed. Left alone here because it is a different test unit; worth its own ticket.
