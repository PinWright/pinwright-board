...
- `#4-explicit-tangent-values-shipped` `IN-REVIEW` developer — "Added numeric `arriveTangent` / `leaveTangent` to `sequence.add_keyframe` and `sequencer.add_keyframe`, and as batch-level plus per-key values on the new `sequencer.add_keyframes`. They are honoured only under `tangentMode` `user` or `break`; supplying them under `auto` is refused with INVALID_ARGUMENT before any mutation, because `AutoSetTangents` would erase them on the spot and again on the next load, which from the wire is indistinguishable from a write that worked. The shared vocabulary (`FKeyTangents`, `ParseKeyTangents`, `TangentModeKeepsExplicitTangents`, `ApplyKeyTangents`, one error string) lives in `Handlers/Sequencer/SequencerKeyInterp.h`; the transform write seats the values at key construction through the new `SequenceKeyframeHelpers::ApplyTransformKeyWrite` in `Handlers/Sequencer/SequenceHandler.cpp`; `Handlers/Sequencer/SequencerHandler.cpp` stamps them onto the key its typed adder just inserted and echoes the STORED pair back. Units are curve value per TICK, repeated in every parameter description, per `#3`'s first trap. The readback half of `#2` in the proposal was already live — `Utils/MovieSceneJsonUtils.h` `BuildChannelKeysJson` emits `tangentMode`/`arriveTangent`/`leaveTangent` — so it was not duplicated. `sequencer.close_loop` and `sequencer.auto_set_tangents` (the optional item 3 and `#3`'s closing suggestion) were NOT implemented; explicit values plus the existing readback close the stated gap. Covered by `PinWright.Sequencer.SequenceAddKeyframe.ExplicitTangentValuesSurviveUnderUserMode` in `Source/PinWright/Private/Tests/Sequencer/TestSequenceAddKeyframeInterp.cpp`, which asserts the FIRST and LAST key both keep the authored non-zero tangents (the loop seam — the two keys auto mode forces flat), that `sequencer.list_sections {includeKeys:true}` reports them, and that `auto` plus values is refused without writing a key. NOT compiled and NOT run here; a build and suite pass is the verification."
