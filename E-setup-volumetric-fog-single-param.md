...
- `#2-four-colour-params-and-inscattering-split` `IN-REVIEW` developer — "Added optional `albedo` / `emissive` / `extinctionScale` / `scatteringDistribution` to `lighting.setup_volumetric_fog` in `Handlers/Environment/LightingHandler.cpp`, declared in `RPC_PARAMS` and written between the `viewDistance` write and the existing `MarkComponentRenderStateDirty` push, so the four live volumetric colour controls have a typed surface and no longer need raw `property.set`. **One ticket claim is wrong against UE 5.8 source and was implemented differently:** `VolumetricFogAlbedo` is an `FColor`, not an `FLinearColor` (`Components/ExponentialHeightFogComponent.h:151`), and the renderer decodes it as sRGB (`Renderer/Private/SceneCore.cpp:419` builds `FLinearColor(FColor)`), so the wire takes linear `{r, g, b}` in 0-1 and the handler sRGB-encodes with `ToFColor(true)` — the same conversion the details-panel colour picker applies. `emissive` stays `FLinearColor` and is stored unclamped. Presence is read off the raw payload rather than through the sentinel-default shape `viewDistance` uses, because `scatteringDistribution` is legitimately negative (UI range -0.9 to 0.9) so no out-of-band value can stand in for absent. Non-finite components are ignored with a log warning (colours fall back to white / black, matching `spawn_light` in the same file). Documented the `FogInscatteringLuminance` vs `VolumetricFogAlbedo` split in `docs/wiki-src/lighting.md` — inside `VolumetricFogDistance` the volumetric integration replaces the analytic inscattering, so `FogInscatteringLuminance` / `DirectionalInscatteringLuminance` apply only beyond that distance — plus the wire units for all four and a pointer to `bOverrideLightColorsWithFogInscatteringColors` as the opt-in that bridges them. Regression coverage in a NEW file `Tests/Environment/TestVolumetricFogColourParams.cpp` (new file rather than extending `TestEnvironmentDirtyFlags.cpp` / `TestVolumetricFogCVarVeto.cpp`, both of which other agents were editing concurrently): `PinWright.lighting.setup_volumetric_fog.ColourParamsReachTheComponent` asserts both directions — the four wire names clear the dispatcher's accepted-name gate via `ParamSpecTestHelpers::IsParamAccepted` (`InvokeHandler` never runs `ValidateHandlerParams`, so a body round-trip alone proves nothing about what a caller may supply), and the values round-trip off the component, with a negative `scatteringDistribution` and an omission sub-case that fails any sentinel-default implementation. NOT COMPILED OR RUN — a build/test loop is the verification."
