...
- `#3-verified-fixed` `DONE` verifier — 2026-08-28. Plugin rebuilt from a clean tree at `b79ba53e` and verified against disk, not against the build's own success message: `UnrealEditor-PinWright.dll` 39,898,624 -> 40,644,096 bytes at 2026-08-28 08:11:48, `UnrealEditor-PinWrightGeometry.dll` 4,983,296 -> 5,113,344, canonical link with no `-000N` artifacts in `UnrealEditor.modules`. Editor restarted on that DLL and the ticket's own repro re-run. Both grades closed, with a pre-fix baseline taken in the same session on the OLD DLL so the repro is known to have been run correctly. Before: `actor.list {filter:"Fog", fields:["folder"]}` -> `{"actors":[{}],"count":1,"totalMatches":1}` (grade 2, the empty-object wipe). After: `{"actors":[{"folder":"Atlantis/Lighting"}]}`. The unknown-key half is now a refusal rather than a silent drop: `fields:["name","bogusKey"]` -> `[INVALID_PARAMS] Unknown fields[] entry(s) for 'actor.list': [boguskey]. Valid fields: [label, name, path, class, folder]`, and the message names `actor.describe` for `level`, `guid`, `tags`, `transform`, `properties`, `components`. `folder` is now in the allow-list, so the wiki sentence that claimed it mirrors `actor.describe` is no longer false about the one key that motivated the ticket.
