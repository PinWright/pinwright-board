...
- `#4-ab-on-ten-rocks-count-was-identical` `DONE` tester — **Verified live by A/B on ten rocks, and the headline finding is that the count field could not tell the two runs apart.** Same ten probes, same world, same subjects; the only difference was the surface exclusion. **Without** the exclusion all ten probes were answered by a `HierarchicalInstancedStaticMeshComponent` and the rocks seated at **Z 562-805**. **With** it all ten were answered by a `LandscapeHeightfieldCollisionComponent` and seated at **Z 239-400** — the ground, several metres lower, which is where they belong. Both directions of `#2`'s and `#3`'s work are therefore confirmed on a real level rather than on a fixture: the component-class axis reaches the filter and changes which surface answers, and `#3`'s `surfaceComponents[]` names that surface. **The point of this entry: BOTH runs reported `placed: 10`.** The correct run and the incorrect run are byte-identical on the count field, so `placed` carries no information about whether the batch landed on ground or on a scatter — a caller checking it sees success in exactly the case this ticket was filed about. **The fix's value is entirely in the provenance rows.** `#3`'s `surfaceComponents[]` inside `groundProvenance` (emitted once from `GroundPlacement::MakeProvenanceJson`, `Handlers/Spatial/GroundPlacementUtils.cpp:880-883` for the `surfaceComponents` / `surfaceComponentCount` pair) is the only field in either response that distinguishes them, and it does so unambiguously: `componentClass` reads `HierarchicalInstancedStaticMeshComponent` in the bad run and `LandscapeHeightfieldCollisionComponent` in the good one. That is precisely the diagnosis `#3`'s docs paragraph tells a reader to make — see a scatter component where you expected a heightfield and the SURFACE SPEC, not the seat, is what needs fixing. Nothing in `#2`'s deliberately-declined half is reversed by this: the ten rocks were peeled off the scatter by an explicit `excludeComponentClasses` entry, which is exactly the "now expressible where before it was expressible on no axis at all" outcome `#2` argued for, and plain HISM remains ground by default. **Scope of this verdict:** an A/B over ten probes on one level, not a re-run of the body's 268/930. That figure is not re-measured here and is not claimed to be.
