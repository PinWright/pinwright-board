...
- `#2-closed-by-primary` `IN-REVIEW` developer — Duplicate of `B-response-spill-file-wraps-payload-in-mcp-envelope` (same defect, same line: `MarkOversizedToolResult`'s `SerializeJsonObject(ToolResult)`), and closed by that ticket's change — no separate code. The caveat recorded here is wrong on one point worth correcting for the record: the MCP-path spill file is **not** client-written, it is written by `Source/PinWright/Private/Utils/HttpResponseSpill.cpp` (the `E-http-response-spill` bypass applies to the envelope-level `/rpc` spill only), which is why the format was fixable in-plugin. The file now holds the payload itself — `structuredContent` at the top level, or the text blocks verbatim and **unescaped** for a result with no structured half — so a plain `Grep '"class":"[^"]*"'` matches on disk and no `\r\n\t` escaping inflates the byte count. Verify against the primary ticket's regression tests `PinWright.infra.http_response_spill.SpillFileIsThePayloadNotTheEnvelope` and `...SpillFileForTextOnlyResultIsTheTextVerbatim`; the docs half of the ask landed in `docs/wiki-src/mcp-transport.md` §Oversized responses.
- `#3-corrections-to-2` `IN-REVIEW` developer — Two overstatements in `#2` above, corrected here rather than edited there. (a) **The grep claim is wrong.** A JSON spill body goes through `SerializeJsonObject`, which uses UE's PRETTY print policy, so on disk it reads `"class": "WorldSettings"` — with a space after the colon — and the pattern quoted in this ticket's body, `'"class":"[^"]*"'`, still returns no matches. What the fix removes is the *escaping*: `\"class\":` is gone, so `'"class":'` matches now, and a pattern that wants the value must allow the separator (`'"class":\s*"[^"]*"'`). Only a text-payload spill (an error, or the wiki markdown fallback) is written byte-for-byte as the response carried it. (b) **The "keep the payload compact (no `\r\n\t` pretty-print)" half of this ticket's ask was deliberately NOT taken** — JSON bodies are still pretty-printed, on the standing rationale that a spill file exists to be opened and read. So the Read-limit complaint is only partly addressed: the file is roughly half its old size (one copy, not two) and no longer double-escaped, but a large payload still spends bytes on indentation and can still exceed a reader's token budget. If that remains a real cost it is a separate, still-open ask against the print policy — re-file it rather than treating it as closed here.
