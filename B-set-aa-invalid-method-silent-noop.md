...
- `#2-fix` `IN-REVIEW` developer — Applied the house validate-before-mutate / fail-loud remedy in `post_process.set_anti_aliasing`. In `Plugins/PinWright/Source/PinWright/Private/Handlers/Environment/PostProcessHandler.cpp`, replaced the `if (MethodValue >= 0) { ...write CVar + echo... }` gate (which had no `else`, so an unresolved token silently fell through the already-`success:true` response) with an early `if (MethodValue < 0) { Ctx.SendError(TEXT("INVALID_PARAMS"), "Unrecognized anti-aliasing method '<x>'; must be one of None|FXAA|TAA|MSAA|TSR."); return true; }` guard placed BEFORE any CVar write, mirroring the landed `GASHandler.cpp:872-879` `replicationMode` rejection and the reject-whole-call-before-mutate precedent (`E-water-underwater-settings-silent-drop`). A rejected call now writes neither `r.AntiAliasingMethod` nor `r.ScreenPercentage`; the `method`-omitted path (e.g. `screenPercentage`-only) still succeeds because resolution stays inside the `TryGetStringField("method")` guard. Regression test `PinWright.post_process.set_anti_aliasing.RejectsInvalidMethod` added to `Plugins/PinWright/Source/PinWright/Private/Tests/Environment/TestPostProcessHandlers.cpp`: case 1 asserts `{"method":"TAAU"}` returns an error with code `INVALID_PARAMS` and leaves the CVar untouched (fails if the guard is reverted to a silent success); case 2 asserts a valid `TSR` still succeeds, echoes `antiAliasingMethod`, and writes the CVar to 4 (no over-rejection); case 3 asserts an omitted `method` still succeeds. Fixture built entirely in-code (JSON payloads + engine-global CVars), no asset load. Not compiled/tested here — later phase verifies.
