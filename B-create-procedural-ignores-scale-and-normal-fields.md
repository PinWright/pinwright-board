...
- `#4-procedural-scale-and-the-age-that-makes-it-observable` `IN-REVIEW` developer — Closes the half `#3` returned. Taken in one pass with `F-procedural-foliage-simulation-knobs-unreachable` and `B-create-procedural-density-writes-paint-density`, which rewrite the same property block. **`ProceduralScale` is written, alongside `ScaleX/Y/Z` rather than instead of them**, exactly as `#3` prescribed: `ApplyFoliageScaleAndAlign` is untouched, so `foliage.add_type`'s painting path keeps its write and the two verbs still cannot drift on the three shared keys. The new per-type key is `proceduralScale`, an interval accepted as `{min, max}` or `[min, max]` (the shapes `bounds.size` already takes), validated against the engine's own `ClampMin` 0.001 — a zero minimum makes `GetScaleForAge` return 0 at age 0, i.e. an invisible instance rather than a small one — and routed through `NoteSkippedType` per entry rather than failing the batch. **`#3`'s "one knob more" is implemented as an explicit ordering, which is the substance of this entry.** `MaxInitialAge` is resolved LAST in `ApplyFoliageProceduralSimConfig`, after `MaxAge` and `ProceduralScale` are written, and the comment there states why: `GetInitAge` returns `MaxInitialAge * GetFraction()`, so at the engine default 0 every seed starts at age exactly 0, `GetNextAge` advances by integer 1 per step, and `GetScaleForAge` evaluates at `Age / MaxAge` — collapsing any requested range to `NumSteps + 1` discrete sizes, the `{1.0, 1.2, 1.4, 1.6}` ladder `#3` measured. When the caller supplies a non-degenerate `proceduralScale` and no `maxInitialAge`, the verb **raises `MaxInitialAge` to the effective `MaxAge`** (hence the ordering: the target has to be written first), which spreads the initial ages continuously over `[0, MaxAge]` and is what makes the requested range observable at all. It is disclosed, not silent: `max_initial_age_source` in the new per-type `foliage_types[]` echo reads `"supplied"` / `"default"` / `"raised_for_procedural_scale"`, and a `warnings[]` line names the raise and how to override it. An explicit `maxInitialAge` — 0 included — is honoured verbatim and gets the opposite warning, naming the collapse it causes. **Regression test:** `Source/PinWright/Private/Tests/World/TestFoliageCreateProceduralSimulationConfig.cpp`, `PinWright.foliage.create_procedural.AppliesSimulationConfig`, routed through `FRpcDispatcher::ProcessRequest`. It reads `ProceduralScale.Min/Max` back off the generated `_FT_0` and asserts the supplied `(0.5, 2.5)` rather than the CDO `(1, 3)`, asserts `ScaleX.Min/Max` and `AlignToNormal` STILL carry the paint-side values (so a future edit cannot "fix" this by moving the write instead of adding one), and asserts `MaxInitialAge == MaxAge` on the entry that asked for a range without pinning the age — the ordering assertion. A second entry pins `maxInitialAge: 0` alongside a range and asserts it is honoured, not raised. `PinWright.foliage.create_procedural.WarnsOnInvertedOverlapPriority` covers the other ordering rule. **`#3`'s DrawScale3D-spread assertion was NOT added, and that is a stated gap rather than an oversight.** `B-spawned-volumes-have-no-brush-geometry` makes `instances_spawned` read zero in this environment for an unrelated reason, so an assertion on placed instances would be measuring that ticket, not this one. What makes the asset assertion discriminating now — and it was not before — is that `procedural_scale` and `max_initial_age` are the properties the procedural branch actually opens, which was precisely `#3`'s complaint about the property `#2`'s test read. A live run comparing placed `DrawScale3D` spread is still the measurement that would settle it and has not been made. **Also landed in the same pass, from the sibling tickets:** `tileOverlap` (was hardcoded to 0 with no echo at all) and `savePath` (was a hardcoded `/Game/ProceduralFoliage`) are now parameters with `tile_overlap` / `save_path` echoes, joining the `tile_size` / `num_unique_tiles` pair `#2` landed. `MinimumQuadTreeSize` is still unreachable but is no longer silent: it joins `UnreadPerTypeFields` so a caller who passes it is told in `ignoredFields`. **Files changed:** `Source/PinWright/Private/Handlers/Environment/FoliageHandler.cpp`, `Docs/wiki-src/foliage.md`, `Source/PinWright/Private/Tests/Infra/TestFoliageNestedInputSchemaDocs.cpp`, plus the new test file above. `TestFoliageCreateProceduralTypeConfigHonesty.cpp` was deliberately left alone — its assertions are all still true and still worth keeping. Not compiled and not run — per instruction, the wave owner builds and runs the suite.
