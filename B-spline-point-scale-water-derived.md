...
- `#3-partially-verified` `IN-REVIEW` integrator — Compiled and suite-green, but **deliberately NOT moved to DONE**, because the one property this ticket is about was not demonstrated by a reload. Build: clean module rebuild, `-DisableAdaptiveUnity -NoHotReloadFromIDE`, `Result: Succeeded`, zero diagnostics in the build log and UBT's `Log.txt`. Suite: **3760 tests performed, 3758 pass, 2 fail** (both the pre-existing `localization.Validation.*`), `started == success + fail`, ZenServer probe 0, all five integration sub-modules loaded. `PinWright.spline.set_spline_point_scale.RefusesDerivedWaterSplineScale` **passes** in 0.8 s against a real `AWaterBodyRiver` spawned and torn down through the production verbs, which also settles the changeset's open item #9 (`FScopedEditorWorldActorGuard` cleaning up a water body — it does, without incident). So **what is proven**: the refusal fires on the engine predicate, carries `DERIVED_PROPERTY`, names the authoritative verb, leaves `Scale.X` untouched, and the new `water.set_river_width_at_spline_point` moves `Scale.X` measured off the component rather than off its own response. **What is NOT proven**: that a width written through the authoritative verb is still there after a **genuine level reload** — `Docs/rpc-design.md` §5, written by this very changeset, says only a reload is proof, and the original defect is precisely a value that survives read-back and save and dies on load. That check was not run because demonstrating it requires saving a map owned by another agent, which this pass refused to do. The sibling ticket `B-compile-material-landscape-consumers-stale` did get a frame-level runtime proof and moved to DONE; this one has no equivalent. **To close it**, one bounded check suffices: set a river width with `water.set_river_width_at_spline_point`, save, reload the level genuinely (a restart or `editor_restart` with `map`, not `open_level` on the already-open map), and confirm the width is unchanged. `actor.duplicate` is a cheaper proxy that exercises the same `PostDuplicate` -> `SynchronizeWaterProperties` path without a save. Shipped in `fadecf81`, pushed to origin/master as a fast-forward. Follow-ups filed from an audit of this changeset: `B-derived-state-report-contract-gaps` (empty `authoritativeVerb` for a future spline type; the width verb named even for a `Scale.Y` write; `pointIndex` has no `point_index` alias) and `B-derived-state-verbs-are-ue58-only` (the river width/depth accessors are UE 5.6+, unguarded).
