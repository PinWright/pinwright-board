...
- `#4-particle-attribute-links-unreachable` `IN-REVIEW` reporter — Encounter against the shipped `#2` implementation, 2026-09-02, UE 5.8, EAContentExamples58 checkout, authoring `/Game/FPS/VFX/NS_Impact_Concrete`. **Status deliberately untouched — this is a scope observation on a delivered feature, not a return.** The `User.*` case works as `#2` describes; a `Particles.*` attribute link does not. Both of these fail: `set_module_input {entryId:<ScaleSpriteSize>, inputName:"Uniform Scale Factor", value:{link:"Particles.NormalizedAge"}}` and `{inputName:"Initial Sprite Size", value:{link:"Particles.Initial.SpriteSize"}}` → `[PARAMETER_NOT_FOUND] Cannot infer a type for linked parameter '<name>' (module input type unknown)`. Writing a literal to the same input first (creating the override pin, echo `"1.0"`) and retrying the link does **not** change the answer, so the failure is in `FindModuleInputDeclaredType`, not in the pin state: `EnumerateScriptInputs` surfaces only the script's parameter-map input node, never the module's stack-function inputs, so a non-`User.` scope — whose type must come from the input's declared type rather than from the exposed-parameter store — can never be typed. That is the same `EnumerateScriptInputs` limitation `F-niagara-dynamic-input-nested-inputs` is scoped around; the two want the same `FNiagaraStackGraphUtilities::GetStackFunctionInputs` + `FCompileConstantResolver` enumeration. Worth recording because the error text blames the *parameter* ("PARAMETER_NOT_FOUND", "Cannot infer a type for linked parameter") while the parenthetical names the real cause, so a caller reads it as "that attribute does not exist" and goes looking for the right spelling of a name that was never the problem. Impact in this session: the standard grow-over-life idiom — bind a `ScaleSpriteSize` factor to `Particles.NormalizedAge` — was unreachable, and dust expansion had to fall back to the module's `Uniform Curve` mode whose stock default curve PinWright can neither read nor edit (`niagara.set_curve_keys` answers `DATA_INTERFACE_NOT_FOUND` for `Constants.<Emitter>.ScaleSpriteSize.Uniform Curve Sprite Scale` because the module input is `valueMode:"default"`, i.e. it has no DI override to edit). Suggested follow-up, either here or as a split: accept `Particles.*` / `Engine.*` links, and reword the refusal so the type-resolution failure is not reported as a missing parameter.
