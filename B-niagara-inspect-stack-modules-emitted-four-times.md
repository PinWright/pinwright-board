...
- `#2-walk-shared-graph-once` `IN-REVIEW` developer — Root cause confirmed exactly as filed and fixed at the walk, not by deduping the output. `NiagaraDumpBuilder.cpp`: `CollectStackModuleNodesInExecutionOrder` now returns `FStackModuleEntry{node, scriptUsage, index}` — it collects the graph's `UNiagaraNodeOutput` nodes, sorts them by pipeline stage (`GetStackStageRank`: System Spawn/Update, Emitter Spawn/Update, Particle Spawn/Update, then event handlers and simulation stages), walks each chain through the unchanged `PinWrightNiagara::CollectModuleNodesForOutput`, stamps `EnumToString(OutputNode->GetUsage())` on every entry and restarts `index` per stage; the existing cross-output dedup and the NodePosY-ordered unattached tail (now carrying an empty `scriptUsage`) are kept. `BuildStackModuleJson` gained a `scriptUsage` field, the same vocabulary `AddScriptCompileEntry` already emitted. The four/two per-script calls collapse into `AddEmitterDataStackModules` / the system block, which `AddUnique` the graphs behind the owner's scripts and walk each distinct graph exactly once — so the fourfold emission is gone because the shared graph is no longer re-walked, and consequence 2 (the missing stage discriminator) is closed at the same time. Bumped `niagara_stack.json` 4 -> 5 in `AssetDumpCache.cpp` (same commit as the shape change); `niagara_model.json` needed no bump — `NiagaraModelBuilder` already partitioned by usage via `CollectFunctionNodesForUsage` and never had this defect, which is why only the `niagara.inspect` stack aspect and `niagara_stack.json` were affected. Regression test `PinWright.Assets.Niagara.DumpBuilder.StackListsEachModuleOncePerStage` (`Tests/Assets/TestNiagaraDumpBuilder.cpp`) builds the real shape — one `UNiagaraScriptSource` shared by all four emitter scripts, one graph with four stage output nodes, one module chained into each — and asserts `modules` has exactly 4 entries, each `entryId` counted once, each carrying its stage in `scriptUsage` at `index` 0; the pre-fix builder emits 16 entries with every id four times, so it fails on the count, the per-id count and the absent `scriptUsage`. Not shared root with `B-niagara-entry-id-not-unique-across-emitters`: that one is `entryId` being a raw `NodeGuid` copied when an emitter is duplicated, so *distinct* modules under *different* emitters collide; this one was one module re-listed under one owner. Fixing this removes the same-owner repeats but leaves every cross-emitter collision that ticket measured intact. Not compiled or run here (build/test explicitly out of scope for this pass) — needs a compile + `PinWright.Assets.Niagara.*` run before DONE. Wiki: `docs/wiki-src/niagara.dump-files.md` now documents `scriptUsage`, the per-stage `index`, and once-per-owner listing.
