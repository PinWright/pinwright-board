...
- `#2-additional-alphabetical-cap-silent-truncation` `OPEN` reporter — Fresh occurrence on a clean/done "campfire embers reusable VFX from scratch" build (namespace `niagara`, `/Game/VFX/NS_CampfireEmbers`, outcome ergo — distinct from the `#1` `/Game/FX/` task; the judge filed only the orthogonal keyword-miss `E-niagara-standard-stack-recipe-undocumented` for this same task, not this response-size gap). The three broad stage-filtered listings each spilled well past the 10000-char inline threshold — `{usage:Module, stage:EmitterUpdate, limit:50}` -> 59251 chars, `{stage:ParticleSpawn, limit:50}` -> 59999, `{stage:ParticleUpdate, limit:50}` -> 61830 — each forcing a `Grep` over the `HttpResponses` spill file to extract assetPaths (reconfirms the `#1` core spill). NEW angle: those listings are ordered alphabetically then hard-capped at `limit:50` with no relevance ordering and no in-band truncation signal, so the two most fundamental modules the build needed — `InitializeParticle` (ParticleSpawn) and `ParticleState` (ParticleUpdate) — fell OUTSIDE the first 50 and were silently omitted from their own stage's listing, contributing to the agent abandoning `search_modules` entirely and globbing the installed engine's `C:/UE_5.7/Engine/Plugins/FX/Niagara/Content/Modules` folder for exact paths. This makes the `#1` "keep `totalMatches`/`truncated` so elision stays detectable" ask load-bearing: without relevance ordering + a truncation indicator (or the proposed compact projection to fit more hits inline), a stage scan silently hides the very modules a stack build wants, and the caller cannot tell the list was cut. Same method `niagara.search_modules`, same response-size/projection family; orthogonal to the keyword-match miss in `E-niagara-standard-stack-recipe-undocumented`. Severity unchanged (Low — recovers via disk glob/Read, but the silent cap can hide the wanted module with no signal).
