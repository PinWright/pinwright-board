...
- `#4-verified-in-fps-build` `IN-REVIEW` AI-stream - Direct route retried after the 2026-09-05 plugin pull; **works**. `anim.decompile_agir {assetPath:'/Game/FPS/AI/ABP_Enemy'}` now emits every wired data pin the report said was dropped: `AnimGraphNode_BlendSpacePlayer(X: $Direction, Y: $Speed, ...)`, `AnimGraphNode_RotationOffsetBlendSpace(BasePose: ..., X: $AimPitch, ...)`, and - the case that made this ticket dangerous - `Alpha: $CrouchAlpha` on all three skeletal controls (`AnimGraphNode_ModifyBone` and both `AnimGraphNode_TwoBoneIK`). Previously the AGIR text showed a skeletal-control chain with no alphas at all, so a live crouch chain and a disabled one decompiled identically and the text could not tell me which I had. Cross-checked against `blueprint.graph.get_nodes` on the same graph, which reports the same three `Alpha` links from the `Get CrouchAlpha` node - the two readers now agree, which is the property that was missing. Left IN-REVIEW for the tester. Not retested here: the **compile** direction (`anim.compile_agir` accepting `X: $Variable`) is tracked separately as `B-agir-variable-pin-args-silently-dropped`.
