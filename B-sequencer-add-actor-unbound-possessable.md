...
- `#2-disposition` `IN-REVIEW` developer — GO. Verified the defect present at HEAD: the three add-verbs (`SequenceHandler.cpp` add_camera `:671`, add_actor `:792`, add_actors `:903`) all call `MovieScene->AddPossessable` and never `ULevelSequence::BindPossessableObject` — a whole-plugin grep finds `BindPossessableObject` in ZERO production handlers (only in tests); the FK resolver's reverse lookup (`ControlRigSequencerHandler.cpp:116` `FindBindingFromObject`) therefore cannot match, and an in-tree red test reproduced the false-success. Severity High confirmed. SHIPPED: appended `LevelSeq->Modify();` + `LevelSeq->BindPossessableObject(BindingGuid, *Found /*Spawned for add_camera*/, GEditor->GetEditorWorldContext().World())` after `AddPossessable` in all three verbs (`Handlers/Sequencer/SequenceHandler.cpp`) — binding with the editor world the resolver scans (the proven pattern at `TestSequencerControlRigTrack.cpp:155`), not the ticket's `Found->GetWorld()`. Adopted the in-tree red test `PinWright.Sequencer.AddActor.BindsPossessableToObject` (`Tests/Sequencer/TestSequencerAddActorBinding.cpp`) as the regression gate — it drives the production `sequencer.add_actor` handler and asserts `FindBindingFromObject` resolves the returned bindingGuid; observed FAIL pre-fix, PASS post-fix (red→green). Plugin compiles clean; test executed + passing. SPLIT the same-root-cause defect at `SequencerHandler.cpp:390` (`AddCameraRigTrackInternal` actorPath branch, shared by `add_camera_rig_rail`/`add_camera_rig_crane`) into new OPEN ticket `F-sequencer-camera-rig-possessable-unbound` — distinct verb family + actor-resolution path, not covered here. Two immaterial ticket inaccuracies noted (the `add_camera` parenthetical claiming a camera-cut track is added is wrong — add_camera only spawns+AddPossessable; line numbers drifted +10) — cosmetic, no reword.
