...
encounters: 2
lastSeen: 2026-07-05T12:11:31.3663200+03:00
---
...
## History
- `#4-liveness` `IN-REVIEW` reporter — still reproduces at HEAD (realism task building `/Game/AI/BT_GuardPatrolChase`; replay-confirmed live via `mcp__pinwright__call` on temp `/Game/McpOracle/BT_OracleWaitReplay`, Wait task `BD82CA524E69B10BF6D54ABDA09BDACA`, deleted after replay): identical shape to `#3` — `set_node_properties {WaitTime:2.5}` returns `ok:false` `[INVALID_PROPERTY] ... WaitTime: Unsupported JSON type for struct property` with `droppedFields:[{name:WaitTime,reason:"Unsupported JSON type for struct property"}]`, `partiallyApplied:false` (fix still in effect, no silent success), `{WaitTime:{DefaultValue:2.5}}` succeeds, decompile emits `WaitTime: "(DefaultValue=2.500000)"`. Residual ergonomic tail unchanged (error/wiki still don't name the `FValueOrBBKey_Float` `{DefaultValue:<n>}` shape). No new ticket.
