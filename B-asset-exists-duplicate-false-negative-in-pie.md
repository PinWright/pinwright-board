...
The root cause was the shared `EditorScriptingHelpers::CheckIfInEditorAndPIE()` gate at the start of every `UEditorAssetLibrary` read: it returns false whenever `GEditor->PlayWorld` or `GIsPlayInEditorWorld` is set, so the handlers misread play mode as missing content. `Utils/AssetUtils.h/.cpp` now provides one registry/object-table/package-file resolver and directory probe that have no editor-library PIE gate, while `Utils/PieState.h/.cpp` exposes the exact shared predicate. `asset.exists` uses the safe resolver; `asset.duplicate`, `asset.rename`, `asset.move`, `asset.delete`, `asset.create_folder`, and `asset.reload` refuse operations that still require PIE-blocked editor APIs with `PIE_ACTIVE`, before any path-not-found verdict.

Files changed: `Source/PinWright/Private/Utils/AssetUtils.h/.cpp`, `Source/PinWright/Private/Utils/PieState.h/.cpp`, `Source/PinWright/Private/Utils/PieSaveBlockGuard.cpp`, `Source/PinWright/Private/Handlers/Editor/EditorCommandHandler.cpp`, the five `Handlers/Asset/Asset*Handler.cpp` files using these reads, `Source/PinWright/Private/Tests/Assets/TestAssetPieSafeResolution.cpp`, and `docs/wiki-src/asset.md`. Tests: `PinWright.Assets.AssetResolution.RegistryAndLoadAvoidEditorAssetLibrary`, `PinWright.Assets.AssetResolution.AssetHandlersAvoidPieBlockedLibraryReads`, and `PinWright.Assets.AssetResolution.PiePredicateMirrorsEngineGate`. Deliberately unchanged: the mutating `UEditorAssetLibrary` calls themselves, because replacing Unreal's duplicate/rename/move/delete APIs is outside this ticket; their Asset handler entry points now refuse PIE explicitly.
...
- `#4-pie-safe-resolution-fix` `IN-REVIEW` developer — Source review confirmed `UEditorAssetLibrary::DoesAssetExist`, `LoadAsset`, `DoesDirectoryExist`, and `ListAssets` all enter the engine's play-mode guard before doing useful work. Added shared registry/object/package resolution, migrated the Asset handler read gates, and made operations whose editor mutation API remains PIE-blocked return `PIE_ACTIVE` before checking paths. Added three structural/unit automation tests and updated the asset wiki; no live editor, build, or suite run was performed under this worker brief.
