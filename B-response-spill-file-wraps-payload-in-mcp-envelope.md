...
- `#6-fixed-spill-file-is-the-payload` `IN-REVIEW` developer — `MarkOversizedToolResult` (`Source/PinWright/Private/Utils/HttpResponseSpill.cpp`) now writes the **payload** to the spill file instead of `SerializeJsonObject(ToolResult)`. New file-local `BuildSpillBody` emits the `structuredContent` object (pretty-printed JSON) when there is one, else the text blocks verbatim and unescaped, joined with `\n`; `CollectTextBlocks` was extracted so the size gate and the file body share one traversal of `content[]`. The inline stub now states the shape it wrote: `file.payload` is `"structuredContent"` or `"text"`, `file.contentType` follows it (`application/json` / `text/plain`), and the notice gains "That file IS the payload: its top level is this response's structuredContent object, not an MCP envelope." Everything else about the rewrite is unchanged — same `characters` / `fileCharacters` / `threshold`, same `content[0].text` + `structuredContent` replacement, so the top-level `docs` field still survives a spill — and the `/rpc` path (`MaybeBuildSpilledResponse`, `E-http-response-spill`) is untouched. Header comment in `Source/PinWright/Private/Utils/HttpResponseSpill.h` and the shape docs in `docs/wiki-src/mcp-transport.md` (§Oversized responses) and `docs/arch.md` updated to match. Regression tests in `Source/PinWright/Private/Tests/Infra/TestHttpResponseSpill.cpp`: `PinWright.infra.http_response_spill.SpillFileIsThePayloadNotTheEnvelope` parses the written file and asserts `valid` / `dataInterfaceCheck` read at its top level with no wrapper keys present, and `PinWright.infra.http_response_spill.SpillFileForTextOnlyResultIsTheTextVerbatim` asserts a text-only result's file equals `content[0].text` byte for byte. Counterfactual: restore `const FString Body = SerializeJsonObject(ToolResult);` and the first test fails at `dataInterfaceCheck` — the file's top level becomes `content`/`structuredContent`/`isError` and the verdict field is absent, which is exactly the silent false negative this ticket reports; the second fails its verbatim equality because the body becomes the envelope with the payload JSON-escaped inside `content[0].text`. One assertion in the pre-existing `...ToolResultOverflowStillSpillsAndReportsBothSizes` test was retuned in the same commit: it asserted the on-disk copy carried the payload twice, and now bounds `fileCharacters` to a single pretty-printed copy (`< characters + 1024`), which the old ~2x envelope body fails.

- `#7-error-results-spill-their-text` `IN-REVIEW` developer — Follow-up on a verifier finding against `#6`, same files. `BuildSpillBody` took `structuredContent` whenever it existed, which silently dropped an ERROR's code and message: `MakeToolCallError` (`Source/PinWright/Private/Transport/McpRequestCore.cpp`) puts only the handler payload in `structuredContent` and the `[<CODE>] <message>` line, the report hint and the `Docs:` pointer only in the text block — and the inline text block is exactly what the spill notice overwrites. An over-threshold `ERR_PARSE_FAILED` therefore spilled its diagnostics and lost the sentence saying what failed. Fixed in `Source/PinWright/Private/Utils/HttpResponseSpill.cpp`: an `isError` result now writes its text blocks (which repeat the payload, so nothing is lost), with a fall-back to the payload if it somehow has no text block at all; success results are unchanged. New regression test `PinWright.infra.http_response_spill.SpillFileForErrorResultKeepsCodeAndMessage` builds the error shape (payload in `structuredContent`, `[ERR_PARSE_FAILED] …` + payload in the text, `isError: true`), asserts the rewritten inline text no longer carries the code, and asserts the on-disk body does. Counterfactual: restore the unconditional `structuredContent` preference and the file becomes `{"diagnostics": …}` — `StoredBody.Contains("[ERR_PARSE_FAILED]")` fails, and the code and message then exist nowhere in the response. Also corrected in the same pass: the `fileCharacters` comment claimed pretty-printing makes the file "slightly larger" (it costs per LINE, so a nested payload lands well above the condensed count), a stale "DISAGREE" comment above the retuned assertion in `...ToolResultOverflowStillSpillsAndReportsBothSizes`, and the error-case + pretty-print notes in `docs/wiki-src/mcp-transport.md` and `docs/arch.md`.
