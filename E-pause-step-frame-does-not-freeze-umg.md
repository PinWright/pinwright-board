...
- `#5-residual-closed-by-world-lever` `IN-REVIEW` developer — The residual `#3-returned` names is closed by [`B-step-frame-world-advance-constant`](B-step-frame-world-advance-constant.md) `#6`; nothing on this ticket's own UI half changed. `#3` returned this ticket solely because reviewer-verification step 3's WORLD half failed (`worldSecondsAdvanced` ~= `deltaSeconds`) while its UI half passed exactly; the cause was that this ticket's fix engaged only the process fixed time step (`FApp::SetUseFixedTimeStep` + `SetFixedDeltaTime`), which sizes the ENGINE frame, whereas `UEditorEngine::Tick` sizes each PIE world's tick from `FWorldContext::PIEFixedTickSeconds` when that is above zero and otherwise from the editor's real frame delta (`EditorEngine.cpp:2133-2146`, `Engine.h:442-443`). `PieTimeControl.cpp` now engages that per-context lever alongside the process one for the stepped frame and restores both, so step 3 can pass in both halves. The pause/freeze half of this ticket (`SetUiClockDelta(0.0)`, `SuppressPieWidgetTicks`, the `ResumePIE` / `EndPIE` release, `freezeUi` / `uiFrozen`) is untouched and its verification steps 1, 2, 4, 5 and 6 stand as written. Two additions the reviewer will see on this ticket's own verbs: `editor.pause` and `editor.step_frame` now also report `motionBlurSuppressed`, because the freeze holds `r.MotionBlurQuality` at 0 (a world that is not ticking still renders velocity vectors from before the pause — see `B-step-frame-world-advance-constant` `#4` / `#5`), and `editor.step_frame` reports `deltaHonoured` / `worldSecondsExpected` / `worldDeltaClamp` / `timeDilation` beside the two advance figures. Regression tests for the world half are on the B ticket (`PinWright.editor.step_frame.PieContextTickDelta`, `PinWright.editor.step_frame.WorldStepBudget`); no test was added or changed here. NOT COMPILED and NOT RUN.
