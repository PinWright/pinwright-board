...
`Source/EditorAutomationRpcGateway/Private/Utils/PropertyUtils.cpp`
...
1. **Missing numeric encoders.** `FUInt32Property`, `FInt8Property`,
   ...
   `FIntProperty`/`FInt64Property` at lines 442–451.
...
2. **Missing weak/lazy/optional object dispatch.** `FWeakObjectProperty`
   ...
   same path-string-or-null shape as `FObjectProperty` at lines
   500–510. Specifically, `ExportObjectRefElement` (PropertyUtils.cpp
   ~124-173) handles `FObjectProperty`, `FClassProperty`,
...
- **Skip `CPF_Transient` UProperties** in the top-level dump walk (not
  ...
  per-property emission, near `ExportPropertyToJsonValue` line 1961).
...
- **Add typed encoders for the remaining numeric / smart-ptr property
  ...
  - `FWeakObjectProperty`, `FLazyObjectProperty` →
    same path-string-or-null shape as `FObjectProperty` lines 500–510,
