...
- `#2-the-in-code-concession-cites-the-wrong-ticket` `OPEN` reporter — **Status unchanged, severity unchanged, `encounters` deliberately NOT bumped, no code touched.** A traceability defect on the exact lines this ticket owns, small enough that it should be swept by this ticket's fix rather than filed on its own. The comment this ticket quotes in full, `Source/PinWright/Private/Handlers/Environment/FoliageHandler.cpp:916-918` at plugin HEAD `1a9e5778`, says the two literals *"are the scale/offset half of B-foliage-paint-ignores-align-to-normal-and-random-yaw and are NOT fixed here."* **That is no longer the owner.** The align ticket is `IN-REVIEW` for the rotation half only and says so itself at its `#3`: *"Scope for this ticket's tester is unchanged and is the rotation half only — `#2`'s fix does not claim scale, and `DONE` should not be held on it."* The scale/offset half was carved out to this ticket by that same entry. **And the ticket that does own it is named nowhere in the code:** a whole-repo grep of the plugin for `B-foliage-paint-hardcodes-scale-and-zoffset` (excluding `.git`) returns **zero** occurrences — not in `Source/`, not in `Docs/`, not in the tests. **Why this is a real defect and not a style nit:** citing board ids in source is established practice in this file. The align ticket's id appears at `FoliageHandler.cpp:839`, `:918` and `:1052`, and at `Source/PinWright/Private/Tests/Environment/TestFoliagePaintTypeRotation.cpp:4` — four sites, deliberately placed so a reader of the code can find the reasoning. The convention is what gives a stale citation its cost: a reader who follows `:918` lands on an `IN-REVIEW` ticket whose stated tester scope explicitly *excludes* the thing the comment sent them to read about, concludes the residue is covered by a fix already in review, and stops. The `OPEN` ticket that actually owns it is reachable only by searching the board for text the code never mentions. This was half-observed already — the align ticket's `#3` records that the literals sit *"under the hand-off comment `#2` wrote at `:916-918` naming this ticket as the owner"* — but recorded as a fact rather than as something to correct, which is why it has survived. **Ask, deliberately scoped to this ticket rather than filed separately:** whoever lands the Fix above must update the comment in the same change. It is free — `:916-918` is the comment directly above `Instance.DrawScale3D = FVector3f(1.0f); Instance.ZOffset = 0.0f;` at `:919-920`, the two lines the fix replaces, so the edit is already open in the diff; if the fix reads the type's ranges the comment should be deleted outright, and if it lands partially the id should be corrected to `B-foliage-paint-hardcodes-scale-and-zoffset`. If this ticket is instead deferred or closed `WONTFIX`, the id still needs correcting in place, because the concession then becomes permanent documentation pointing at the wrong ticket. **Not filed as its own ticket, argued:** on its own it is `Low` (the rubric's *"docs, discoverability, naming"* band, a comment that misroutes a reader), it cannot be landed without touching these lines, and the fixer of these lines is already required to read this ticket — so a separate `Low` ticket would add a board entry whose only possible action is an edit inside this one's diff. Recorded here instead, following `B-pcg-connect-pins-silently-replaces-edge` § *Noted here, deliberately NOT filed*, which draws the same line for a finding that belongs inside another change. `encounters` stays 1: the hardcoding was not observed again, and this entry is about the comment above it, not the defect.
