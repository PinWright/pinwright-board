...
encounters: 2
lastSeen: 2026-08-28T09:34:00+05:00
---
...
>
> **Superseded, 2026-08-28 — do not use this diff either. See History `#6(c)`.**
> **Pool eviction closes an asset editor without emitting `LogSlate: Window '...' being
> destroyed` at all**, so this diff under-counts closes and reads a healthy session as a
> continuing leak. Measured: a 14-capture soak scored 19 opens against 8 named destroys by
> this recipe, while `SM_Fish_A` and `SM_Column_Doric` were *provably* closed by two
> independent probes and never logged a destroy line. The reliable oracles are
> `assetEditorWasAlreadyOpen` on a re-capture of the same asset (`false` = the earlier editor
> is gone), or, for a Niagara subject, an `EDITOR_OPEN`-guarded verb such as
> `niagara.add_emitter` answering `EDITOR_OPEN` while the window lives and succeeding once it
> has been evicted. The 29-of-37 figure above stands as the historical measurement of the
> original unbounded leak; it is not a baseline to re-measure against.
...
- `#6-soak-both-modes-both-providers-pool-bound-measured` `DONE` verifier — 2026-08-28, rebuilt DLL at plugin HEAD `b79ba53e` (UBT `Target is up to date`, so the same binary `#4` and `#5` measured), editor pid 14932, third instance on it. Closing this: the two things `#4` and `#5` explicitly left unmeasured are now measured, and the teardown that faulted no longer runs on the release stack at all.
  **Sustained run: 14 captures over 8 assets, both providers, both close modes, one session.** Seven with `closeAfterCapture` default and seven with `false`, interleaved rather than batched, Niagara and staticMesh alternating: `NS_Bubbles_Stream` (crash A's own asset) and a Niagara duplicate of it, plus `SM_Portal_Ring`, `SM_Fish_A`, `SM_Column_Doric`, `SM_Coral_Fan`, `SM_Coral_Brain`, `SM_Kelp_Blade`, `SM_Arch_Ruin` through the mesh provider (crash B's entry point). Then a **150 s idle with an asset editor deliberately left open**, because this ticket records a capture that returned clean and was followed by a kill 90 s later on unrelated work. Result: zero asserts, zero `EXCEPTION_ACCESS_VIOLATION`, no `Saved/Crashes` entry attributable to this process, same pid throughout. A `register_slate_post_tick_callback` probe ran across the whole soak — 22,870 ticks, longest game-thread stall **2.30 s** — so the deferred close and the pool cost nothing observable in frame time.
  **(a) The deferred close drains, 7/7, counted the way `#2(b)` demands.** Every default-close capture's window shows `LogSlate: Window '<AssetName>' being destroyed` 0.5-2.5 s after its `Opening Asset editor for` line (UTC): `NS_PwLiveVerify` 04.24.32 -> 04.24.34, `SM_Portal_Ring` 04.24.39 -> 04.24.40, `NS_Bubbles_Stream` 04.25.20 -> 04.25.21, `SM_Coral_Fan` 04.25.26 -> 04.25.26, `SM_Coral_Brain` 04.25.38 -> 04.25.39, `NS_PwLiveVerify` 04.25.43 -> 04.25.45, `SM_Kelp_Blade` 04.25.48 -> 04.25.49. Responses carried `assetEditorClosed: false` with `assetEditorCloseDeferred: true` on every one, so the honesty change in `#3(c)` reads correctly against what actually happened.
  **(b) The pool-of-one bound is measured — the assertion `#5` said had never been independently checked.** Two independent probes agreeing. First, through a different subsystem entirely: with a Niagara editor left open by `closeAfterCapture:false`, `niagara.add_emitter` on that system answered `EDITOR_OPEN`; after one further capture of a **different** asset with `closeAfterCapture:false`, the identical `add_emitter` **succeeded** — so the pool had evicted the earlier window, observed by a verb that has no idea the pool exists. Second, from the capture verb's own measurement: after seven `closeAfterCapture:false` captures across seven distinct assets, re-capturing the most recent (`SM_Arch_Ruin`) reported `assetEditorWasAlreadyOpen: true` while re-capturing an earlier one (`SM_Fish_A`) reported `false`. One capture-opened editor alive, the rest gone. The measured 29-of-37 unbounded leak is bounded.
  **(c) Correction to this ticket's own verification recipe — it will mislead the next verifier.** `#2(b)` instructs diffing `Opening Asset editor for` against named `LogSlate: Window '...' being destroyed`. **Pool eviction closes an editor without emitting that Slate line.** `SM_Fish_A` and `SM_Column_Doric` were provably closed by probe (b) above, yet neither ever logged a window destroy. Counted `#2(b)`'s way this soak reads as 19 opens against 8 named destroys and looks like a continuing leak; it is not one. The reliable oracles are `assetEditorWasAlreadyOpen` on a re-capture, or an `EDITOR_OPEN`-guarded verb for a Niagara subject. `#2(b)` replaced one bad grep with another.
  **(d) The end-to-end gap `#4` held this open for is now covered behaviourally, though still not by a test.** `#4`'s objection was that no test calls `ReleaseSubject` / `ReleaseMeshSubject` / `Niagara::Release`, the frame immediately above `CloseAssetEditor` in both crash stacks. All 14 captures drove the whole verb, so that frame ran 14 times through both providers. And it no longer reaches the fault: `CloseAssetEditor` (`CaptureSubject.cpp:1638`) contains no `CloseAllEditorsForAsset` — it calls `ScheduleDeferredAssetEditorClose(Asset)` and returns `false` (`:1711-1715`), and the drain runs from a one-shot `FTSTicker::GetCoreTicker()` pass that re-runs `CountPreviewSceneViewportHolders` (`:1423`) and the `FindEditorForAsset` read-back (`:1413`, `:1434`) at execution time under an `FScopedUnattendedRpc` (`:1402`). The access violation lived in a toolkit destructor chain reached from the release stack; that chain is not reached from there any more. Registration into the pool happens at the OPEN (`:1556-1559`), with `bWasAlreadyOpen` measured just before (`:1548`) and never adopted (`:1301-1306`), so a window the caller already had open is never evicted.
  **What is still true and wants a follow-up ticket rather than a Critical staying open:** the test gap itself is unchanged — `TestCaptureSubjectDeferredClose.cpp` still calls `AcquireAssetEditorViewport` / `CloseAssetEditor` directly and never drives the verb, and the Niagara provider still has no test; and `#3`'s named follow-up is undone — `render.capture_animation_preview` publishes `assetEditorClosed` from its own local (`AnimationPreviewCaptureHandler.cpp:1424`) and now reports `false` there, with `assetEditorCloseDeferred` present only inside its `subject` block.
  Also measured here, feeding `B-safepoint-tick-gate-inert-on-simpletickobjects-path` (verified DONE today): **14/14 of these captures logged a SafePoint deferral**, so the verb whose crashes ran inline inside `SimpleTickObjects` is now gated on the transport path. The three siblings named under `## Do not fix one of these and stop` were all exercised in this same session — the family was verified together, not one at a time.
- `#7-stale-verification-recipe-annotated-in-body` `DONE` verifier — 2026-08-28. No status change and no new measurement; this entry exists so the correction is findable where it will actually be read. `#6(c)` correctly retires `#2(b)`'s `Opening Asset editor for` vs `LogSlate: Window '...' being destroyed` diff, but `#2(b)`'s instruction also lives in the ticket **body** (under `## The other half`), where it stood unqualified at the top of a Critical ticket while its retraction sat ~230 lines below it in History. Annotated the body blockquote in place, following this ticket's own precedent of in-body correction blockquotes: it now names pool eviction as the reason the diff under-counts, cites `#6(c)`'s 19-opens-against-8-destroys soak and the two assets provably closed with no destroy line, and points at the two reliable oracles (`assetEditorWasAlreadyOpen` on a re-capture; an `EDITOR_OPEN`-guarded verb for a Niagara subject). Also marked the 29-of-37 figure as the historical measurement of the original unbounded leak rather than a baseline to re-measure against. `#2(b)`'s author's text is left intact above the annotation — the correction is additive, and its own reasoning about why `grep -ci "clos.*editor"` returns 0 remains right.
