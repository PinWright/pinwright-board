...
- `#3-row-space-stamp-and-transaction` `IN-REVIEW` developer — Re-verified at HEAD before designing, and one premise in circulation is WRONG here: `actor.set_instance_transforms` does NOT ignore `previousTransform`. `InstancedMeshHandler.cpp` already makes an undo row's pose the base that omitted fields fall back to, so a same-space replay already restored correctly. What actually held at HEAD is gap 1 (rows carry no space marker), gap 5 (a `{index}`-only row is a counted no-op), gap 2 (`ground_instances` rows correct only by coincidence) and gap 3 (the false "survives an editor restart" claim). Chose the ticket's own recommendation — make the record self-describing — over "replace it with a transaction", because a transaction alone does not close the reported defect: the record still ships in the response and a caller replaying it in the wrong space still corrupts the scatter and gets `updated: N`. Verified the ticket's transaction arithmetic myself against `C:\UE_5.8\...\EditorTransaction.cpp` (`FTransaction::SaveObject` builds an `FObjectRecord` only when `ObjectRecords->Records.Num() == 0`, then only `++SaveCount`), so the transaction is affordable and was added as well rather than instead. **Shipped:** (1) `InstancedMeshUtils::SpaceToken` + `MakeMovedInstanceRow`, so every `movedInstances[]` row is now `{index, space, previousTransform}` and BOTH producers build it in one place — closes gaps 1 and 2, and makes the ground/set space dependency explicit instead of coincidental; (2) `InstanceRpcReconcileRowSpace` in `InstancedMeshHandler.cpp`, running before any write: rows' space is ADOPTED when the replay states none (so the naive local-space paste now simply works, and the response says `spaceFrom: "instances[].space"` beside the echoed `space`), a stated `space` that disagrees with the rows REFUSES the whole batch with `INVALID_ARGUMENT`, and rows disagreeing with each other refuse too — that refusal is the core of this ticket, the silent false-success is gone; (3) a pose-less row (none of `previousTransform`/`location`/`rotation`/`scale`) is refused naming the row index and warning that row keys are outside the top-level `UNKNOWN_PARAMS` gate — closes gap 5; (4) ONE `FScopedTransaction` per batch in `actor.set_instance_transforms` and in `spatial.ground_instances`' apply path only (a dry run opens none), closed before `FinishInstanceWrites`; (5) the `InstancedMeshUtils.h` header paragraph rewritten — the O(written x total) arithmetic replaced with the `SaveObject` dedup finding, and the "survives an editor restart" sentence deleted outright (gap 3); (6) both verbs' summary and param docs updated. **Tests** appended to `Tests/Spatial/TestGroundPlacement.cpp`: `actor.set_instance_transforms.UndoRecordStatesItsSpaceAndReplaysOrRefuses` (red before the fix in three places — the row carries no `space`; the naive replay of a local record lands instance 1 at the world origin instead of restoring it; and a replay explicitly declaring `world` is accepted rather than refused) plus mixed-space and pose-less refusals with unmoved-instance ground truth, and `spatial.ground_instances.UndoRecordStatesWorldSpace` asserting EVERY row is stamped. Every transform assertion re-reads the component, and every response field is read at full nesting depth against a sentinel fallback (`-987654.0`) no real measurement can produce. **The `previousTransform.z`-vs-`previousTransform.location.z` hazard the ticket family warns about is NOT present in this checkout** — the four existing sites all read three levels with an `AddError` on a missing object; nothing was weakened. `check_test_ids.py`: CLEAN, 4765 ids, no dot-prefix collisions. **NOT done, deliberately, and still open work:** gap 4 (`set_instance_transforms` emits `movedInstances` unconditionally, both ground verbs gate on non-empty) is untouched; the full `undo` envelope of the proposal (`schema`/`verb`/`actorPath`/`component`/`instanceCount` folded into one object, accepted back as an `undo` INPUT param) was not built — the per-row stamp closes the dangerous path at a fraction of the blast radius, and encounter 2's gaps 6 (no size bound on `movedInstances[]`) and 7 (no identity for what an index pointed at) are untouched and still want the envelope or a sidecar; `Docs/wiki-src/spatial.md` still has no `ground_instances` section and `actor.md` nothing for the per-instance verbs (item 9), and `Docs/rpc-design.md` item 10 was not written — both files are being edited by other agents this wave. Not compiled and not run: per wave rules the orchestrator builds. `spatial.ground_actors`' `movedActors[]` was left alone — no `space` exists anywhere on the actor path, so there is nothing there to stamp yet.
