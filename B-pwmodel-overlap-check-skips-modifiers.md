...
- `#3-the-check-cannot-see-self_union-and-its-remedy-text-names-the-op-that-loses-the-slot` `OPEN` reporter — Two problems at the other end of the same diagnostic, found while removing 2,800 wrongly-slotted triangles from `Content/FPS/Weapons/Meshes/SM_WPN_Pistol.pwmodel` (see `B-pwmodel-boolean-output-takes-slot-zero` `#3`).

  **(a) The warning fires on overlaps a following `self_union` resolves.** `PWMODEL_UNUNIONED_OVERLAP` compares appended footprints and has no notion of a later resolve, so a part written as `sibling ; sibling ; self_union` — which is the ONLY spelling that both merges the shells and keeps each triangle's material slot — warns once per overlapping pair anyway. Measured on `part controls`: 4 occurrences (`lines 557, 599, 600, 601`), against a compile reporting `health.selfIntersections: 0` and a `componentCount` of 119 that is arithmetically the merged count (frame 1 + stipple 110 + magazine 1 + trigger 1 + **controls 6**, where 6 is what you get only if the thumb pad merged into its lever and the three legend ribs merged into their plate — unresolved it would be 10). So the shells provably were resolved and the warning still fired. It is not fatal, but it trains an author to ignore the one diagnostic that names the most-cited trap in the format, which is worse than not firing.

  **(b) The remedy the message prints is the defect in the neighbouring ticket.** The text says: *"Use `union { }` only when both solids use the same material slot; otherwise move them apart or keep the overlap deliberate."* Every triangle a `union { }` block appends lands on material ID 0 whatever the source says (`RunBoolean` builds with `bNested=true`, `PwModelCompiler.cpp:2925`; `RunGenerator`'s tagging site is gated `if (!bNested)`, `:1681`), and ID 0 is *the first part in the document's* slot — so on a multi-slot model the recommended remedy is precisely how the geometry ends up on another part's material, silently, with a clean compile. The three offered options also omit the correct one. `self_union` (dispatched at `PwModelCompiler.cpp:2643` as a modifier, so it never nests and never renumbers) resolves the overlap AND preserves per-triangle IDs; it should be the FIRST thing this message names, and the `union { }` sentence should carry the slot-0 consequence rather than a same-slot precondition that reads as advice.

  **Fix:** (a) skip the warning for footprints that a `self_union` later in the same op list covers — or, cheaper and still honest, keep firing but downgrade and say "resolved by `self_union` at line N" when one follows; (b) rewrite the remedy string to lead with `self_union` and to state that `union { }` moves the block's output to slot 0. (b) is a string change and is worth doing on its own, ahead of the reachability work in `#1`/`#2`.
