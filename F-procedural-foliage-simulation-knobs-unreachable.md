...
- `#2-simulation-surface-exposed-with-its-interactions` `IN-REVIEW` developer — Implemented as the superset of this ticket and its two siblings (`B-create-procedural-density-writes-paint-density`, `B-create-procedural-ignores-scale-and-normal-fields` `#3`), which all rewrite the same per-type property block; the parameter surface was designed once from here and the two defects checked off against it. **All fifteen table rows are now writable**, as nested keys on `foliageTypes[]` alongside the existing `minScale` / `maxScale` / `alignToNormal`: `initialSeedDensity`, `collisionRadius`, `shadeRadius`, `numSteps`, `seedsPerStep`, `averageSpreadDistance`, `maxAge`, `maxInitialAge`, `overlapPriority`, `canGrowInShade`, `spawnsInShade`, `randomPitchAngle`, and the interval-valued `proceduralScale` / `height` / `groundSlopeAngle` (`{min, max}` or `[min, max]`, the shapes `bounds.size` already takes). Plus the two hardcodes: `tileOverlap` (a top-level `RPC_PARAM_OPT`, echoed as `tile_overlap`) and `savePath` (sanitized through `SanitizeProjectRelativePath`, `SECURITY_VIOLATION` on an unmounted or traversing path, echoed as `save_path`). Read and validated through `ReadFoliageProceduralSimConfig` and written by `ApplyFoliageProceduralSimConfig`, both file-scope in `FoliageHandler.cpp` next to the scale helpers, with every field a `TOptional` so an absent key leaves the engine CDO alone rather than being overwritten by a hardcoded copy of it. A rejected value routes through the existing `NoteSkippedType` channel with a reason and does not consume a `_FT_<n>` index, per the shape `#2` of the sibling ticket established. A value present in the wrong JSON shape is an error too, not a silent skip. **The four interaction constraints are the content of the ask, and all four are implemented rather than only documented.** (1) `overlapPriority` — the verb holds every entry's radii and priority in one call, so it computes effective radius `max(collisionRadius, shadeRadius)` per entry and compares the two orders pairwise; an inversion produces a `warnings[]` line naming both entry indices, both radii and both priorities. It **warns rather than refuses**: a deliberate inversion is a legitimate if rare authoring choice, and refusing would fail a whole batch over a judgement call. Only checked when the caller actually set priorities — all-default zeroes are not an ordering claim. The ticket calls this the single highest-value thing here and it is a real check, not a doc line. (2) `maxInitialAge` — implemented as an explicit ORDERING in the writer, which is commented at length: `MaxInitialAge` is resolved LAST, after `MaxAge` and `ProceduralScale`, and when the caller supplied a non-degenerate `proceduralScale` without pinning the age it is raised to the effective `MaxAge` (the reason the target must already be written). Disclosed as `max_initial_age_source: "raised_for_procedural_scale"` plus a `warnings[]` line; an explicit `maxInitialAge` — 0 included — is honoured verbatim and gets the opposite warning naming the `numSteps + 1` collapse. (3) `height` / `groundSlopeAngle` — documented as filters-over-an-already-simulated-set at the parameter and on the wiki page, and a `warnings[]` line fires when either is supplied, because the ticket's own evidence is that a caller reads them as stratification controls. (4) `tileSize` — the 1000-vs-engine-10000 divergence is now stated where a caller reads it, in both the param description and the wiki, with the reason the default was NOT changed (it would alter existing callers' scatter silently). A fifth interaction the ticket did not name is handled the same way: `GetSpawnsInShade()` is `bCanGrowInShade && bSpawnsInShade`, so `spawnsInShade` alone is a literal no-op — warned per entry, and the echo carries `spawns_in_shade_effective` beside `spawns_in_shade`. **Response honesty.** A new `foliage_types[]` array carries one row per type built, with every simulation value **read back off the asset after the write** rather than echoed from the request, plus `asset_path`, the paint-side `density`, `effective_radius`, and the two `*_source` fields for the derived numbers. `tile_overlap` and `save_path` join `tile_size` / `num_unique_tiles`. Nothing that was decided by the handler is now invisible. **Still unreachable, and no longer silent:** `spreadVariance`, `distributionSeed`, `maxInitialSeedOffset`, `scaleCurve`, `minimumQuadTreeSize` were added to `UnreadPerTypeFields`, so a caller who guesses one of those engine names is told in `ignoredFields` instead of having it dropped. They are listed on the wiki page too. **Regression tests:** `Source/PinWright/Private/Tests/World/TestFoliageCreateProceduralSimulationConfig.cpp` — `PinWright.foliage.create_procedural.AppliesSimulationConfig` (every knob read back off the generated `_FT_0`, each set to a value that differs from its CDO so nothing can pass by matching a default; the derived-vs-supplied `initialSeedDensity` split; the `MaxInitialAge == MaxAge` ordering assertion; the `tile_overlap` / `save_path` echoes; a negative assertion that a correctly ordered priority set produces no inversion warning) and `PinWright.foliage.create_procedural.WarnsOnInvertedOverlapPriority` (a large-radius species given the lower priority — the 46 -> 0 shape — plus `spawnsInShade` without `canGrowInShade`). Both route through `FRpcDispatcher::ProcessRequest`, so the pre-fix failure is the dispatcher's `UNKNOWN_PARAMS` before any readback. **Not done, named:** placement is not verified on placed instances, because `B-spawned-volumes-have-no-brush-geometry` makes `instances_spawned` read zero for an unrelated reason; the asset readbacks are discriminating because the properties read are the ones the simulation opens, but a live scatter comparison has not been run. `F-resimulate-existing-foliage-volume` is untouched — nothing here re-simulates an existing volume. **Files changed:** `Source/PinWright/Private/Handlers/Environment/FoliageHandler.cpp`, `Docs/wiki-src/foliage.md` (create_procedural H3: the simulation-key table, the four interaction rules, the tiling trio, `savePath`, the `foliage_types[]` readback, the still-unreachable list, a two-species worked example), `Source/PinWright/Private/Tests/Infra/TestFoliageNestedInputSchemaDocs.cpp` (one more overlay-exclusive marker pair, per `E-foliage-nested-input-schemas-undocumented`'s same-commit requirement), plus the new test file. `Content/Python/check_test_ids.py` run clean over 4734 ids — the two new leaves collide with nothing. Not compiled and not run — per instruction, the wave owner builds and runs the suite.
