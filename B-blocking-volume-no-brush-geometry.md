...
encounters: 2
lastSeen: 2026-07-01T19:04:41.0034217+03:00
---
...
- `#8-additional-in-review-fix-misses-non-brush-triggerbox` `IN-REVIEW` reporter — Additional evidence (arena gameplay/atmosphere-volumes task, REALISM mode, no seed; touched create_kill_z/post_process/reverb/audio/trigger_box/blocking_volume + set_volume_extent/set_volume_properties/get_volumes_info). NEW ANGLE that corrects a mechanism error running through #2/#4/#5 and warns the #3 verifier: those entries assert "`ATriggerBox` is an `ABrush`, so both setters take the `Cast<ABrush>`→`CreateBoxBrushForVolume` branch." That premise is **false**, and a live replay **at the #3-fix-deployed HEAD on this fuzz host** (current source has the fix: `CreateBoxBrushForVolume` :122-125 routes through `BuildBoxBrushGeometry` :100-120 with the `Brush` UModel init) proves `ATriggerBox` never touches that helper — it is an `ATriggerBase` (an `AActor`, NOT an `ABrush`), so both the create and the set path take the **non-brush** routes the #3 fix does not modify, and the TriggerBox defect **still reproduces unchanged**: (a) `volume.create_trigger_box {volumeName:"Replay_Trigger", location:{7200,0,-2300}, boxExtent:{400,400,250}}` echoed `boxExtent {400,400,250}` but `get_volumes_info {filter:"Replay_Trigger"}` immediately read back `extent {128,128,128}` — the boxExtent was dropped because `SpawnVolumeActor<ATriggerBox>` selects the **non-brush overload** (`VolumeHandler.cpp:290-317`, `TEnableIf<!TIsDerivedFrom<TVolumeClass, ABrush>>`) which spawns + labels the actor and **never applies `Extent`**; the create echo (:478-482) re-emits the requested value regardless. (b) `volume.set_volume_extent {volumeName:"Replay_Trigger", extent:{1000,2000,3000}}` → readback `extent {400,800,1200}` = exactly `40 × (extent/100)`, i.e. `SetActorScale3D(NewExtent/100)` (:1318, the `else` branch, since `Cast<ABrush>` at :1311 is null for a trigger box) applied to the engine-default 40-unit box; a second set `{120,400,220}` → readback `{128,160,128}` (y=160=0.4×400; x/z pinned to the ~128 editor-sprite bound). The `newExtent` echo (:1325-1329) blindly re-emits the raw request, so it can never match the `get_volumes_info` readback. `get_volumes_info` itself is the **truthful side** — its `Trigger->GetActorBounds(false, Origin, BoxExtent)` (:1631) reports ground-truth world bounds (the attempt agent mis-blamed `get_volumes_info` as a "lossy transform"; the culprits are `create_trigger_box` + `set_volume_extent`). Source proof `ATriggerBox` ≠ `ABrush`: `FindVolumeByName` gates on `IsA<AVolume>() || IsA<ATriggerBase>()` (:244) and `get_volumes_info` iterates triggers in a separate `ATriggerBase` branch (:1618-1639). Consequence for the #3 verifier: the IN-REVIEW fix repairs the genuine brush volumes (Blocking/KillZ/TriggerVolume) but leaves `create_trigger_box` (boxExtent dropped) and `set_volume_extent` on `ATriggerBox`/`ATriggerCapsule`/`ATriggerSphere` (scale-not-extent + raw-request echo, per #5/#6/#7) unfixed — the create path needs the **non-brush** overload to set the collision `UBoxComponent`'s `BoxExtent` (not route through `CreateBoxBrushForVolume`, which `ATriggerBox` never reaches), and `set_volume_extent`'s non-brush branch must set `BoxExtent` and echo the applied `GetActorBounds()` half-extent instead of `SetActorScale3D` against a 40-unit base. Same culprit family; no new ticket.
