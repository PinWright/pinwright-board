...
- `#5-spawn-gate-drains-the-queued-on-demand-request` `IN-REVIEW` developer — **The flag is not stale, and admitting the system on its script statuses would have been the wrong fix.** `UNiagaraSystem::HasOutstandingCompilationRequests` (`NiagaraSystem.cpp:2130`) returns true for `NeedsRequestCompile()` alone, and UE 5.8 sets `RequestCompileStatus = RequestPendingOnDemand` at PostLoad (`:969`) for any system whose script change IDs are out of sync while `fx.Niagara.OnDemandCompileEnabled` defers the compile. `ActiveCompilations` stays empty, so `hasActiveCompilations:false` beside `hasOutstandingCompilationRequests:true` with every script `NCS_UpToDate` is exactly that state: a real compile request nobody had issued. `IsReadyToRunInternal` (`:1731-1757`) returns false while it is outstanding, so the engine itself would refuse to instance the system — reading the per-script statuses as an admission would have spawned an asset Niagara says is not runnable. The gate drains instead.

  **`Source/PinWright/Private/Handlers/VFX/EffectHandler.cpp` (~1349)** now passes `bMayFlushRequestCompile=true` into the same bounded pumped wait `niagara.compile` uses, so `PollForCompilationComplete(true)` issues the queued request and the loop pumps it to completion inside the 60 s ceiling. This is what the engine does at the same point: `UNiagaraComponent::Activate` (`NiagaraComponent.cpp:1356-1363`) calls `PollForCompilationComplete()` itself when the asset reports outstanding requests. Spawning is the demand on-demand compilation defers to, which is what entitles this call site to flush.

  **One readiness predicate.** `Handlers/Niagara/NiagaraCompileWait.{h,cpp}`: the wait's flag is renamed `bCompileWasRequested` -> `bMayFlushRequestCompile` (it now means "entitled to start the queued compile", not "already started one"), and the readiness test is exported as `HasPendingCompileWork`, used by both `IsSystemReadyAfterCompileWait` and the handler's reported state. They used to disagree: readiness read the GPU-excluded queue while the published `compileStatus` / `outstandingCompilationRequests` came from the diagnostics block's GPU-inclusive `HasOutstandingCompilationRequests(true)` (`NiagaraDumpBuilder.cpp:1875`) — mixed predicates in one response, which is why `compileStatus` said `outstanding` about work the refusal had not measured.

  **The GPU literal is gone.** `niagara.compile_status` published `outstandingIncludesGpuShaders` as a hardcoded `true` (`NiagaraCompileHandler.cpp`); it is now measured by `HasGpuComputeSimulation` — true only when an emitter declares `GPUComputeSim` and owns a GPU compute script. That literal is what `#2` read as the blocking condition on a `CPUSim`-only asset. It never blocked anything and now reads `false` there.

  **Regression tests.** `PinWright.effect.spawn_niagara.QueuedCompileRequestIsDrained` (`Source/PinWright/Private/Tests/Niagara/TestEffectSpawnNiagaraState.cpp`) parks a queued-but-inactive request on a duplicated stock fixture through `SetCompileForEdit`, asserts this ticket's shape (request pending, nothing actively compiling, an observation-only wait leaves it untouched), then invokes the registered handler and asserts it neither returns `SYSTEM_NOT_COMPILED` nor leaves the request behind. Counterfactual: revert the `bMayFlushRequestCompile=true` argument in `EffectHandler.cpp` and `HasWaitWork` degrades to `HasActiveCompilations()` (false here), so the wait returns in microseconds without flushing, `IsSystemReadyAfterCompileWait` still sees `HasPendingCompileWork` true, and the spawn assertion fails with `SYSTEM_NOT_COMPILED` — the ticket's own observation. `PinWright.niagara.CompileStatus.GpuShaderScopeIsMeasured` (`Tests/Niagara/TestNiagaraCompileStatusGpuScope.cpp`) checks the reported scope against an independent walk of the fixture's emitter sim targets; on the CPU-only stock fixture it fails against the old literal `true`. The existing `GPU inclusion is explicit` assertion in `Tests/Niagara/TestNiagaraCompileSave.cpp` encoded the literal contract and is updated to the measured one (an emitterless fixture must report `false`).

  **Docs:** `docs/wiki-src/effect.md` (the gate drains a queued request; one predicate behind the refusal and the reported state) and `docs/wiki-src/niagara.md` (the `outstandingIncludesGpuShaders` row).

  **Not done, deliberately:** no build, no editor, no suite run, no live repro — source-only per this pass's constraints. And the trade the fix makes is explicit: `effect.spawn_niagara` can now block for a real compile where it used to refuse instantly, bounded at the shared 60 s ceiling.

- `#6-gpu-field-split-and-drain-cost-documented` `IN-REVIEW` developer — Three verifier follow-ups on `#5`; the spawn-gate fix itself is unchanged.

  **`outstandingIncludesGpuShaders` is restored to the literal `true` it was, and the measurement gets its own field.** `#5` overloaded the existing name: the value became "this asset has GPU emitters" while the name asserts the *scope* of `outstandingCompilationRequests`, and that scope really is GPU-inclusive on every asset — so on a CPU-only system the field would have said `false` about an inclusive check, misleading in the opposite direction. `niagara.compile_status` now publishes both: `outstandingIncludesGpuShaders` (always true, the scope statement) beside a new `hasGpuSimulation` (measured by `HasGpuComputeSimulation`). `Tests/Niagara/TestNiagaraCompileSave.cpp`'s original `GPU inclusion is explicit` assertion is restored unchanged and a `hasGpuSimulation:false` assertion added beside it; `PinWright.niagara.CompileStatus.GpuShaderScopeIsMeasured` now asserts the scope statement holds AND that the measurement matches the fixture.

  **Test oracle aligned with the production contract.** That test's independent oracle checked only `SimTarget == GPUComputeSim`, while `HasGpuComputeSimulation` also requires `GetGPUComputeScript()`; the oracle now checks both, so it pins the shipped predicate rather than a looser one.

  **Fixture duplication de-duplicated.** The byte-identical duplicator in `Tests/Niagara/TestEffectSpawnNiagaraState.cpp` and the new compile-status test is now one shared `NiagaraEditTestUtils::DuplicateFixtureSystemWithEmitters(Prefix, OutPath)` in `Tests/Assets/NiagaraEditTestUtils.h`, beside `NewTransientSystem` (which empties emitter handles and is therefore the wrong base for emitter-derived state). Both call sites in the effect test use it directly.

  **The drain's cost is now documented rather than implied.** `docs/wiki-src/effect.md` states that `effect.spawn_niagara` exposes no `timeoutSeconds`, so the drain can hold the game thread up to the shared 60 s ceiling, and that `UNiagaraSystem::RequestCompile` ends by destructing an `FNiagaraSystemUpdateContext` over the system — reinitializing every live component of it in the open level. Callers who need to know whether a spawn will be instant are pointed at `niagara.compile_status` first.

  Still no build, editor or suite run in this pass.
