...
- `#4-verb-removed-defect-unreachable` `DONE` verifier - 2026-08-28, PinWright HEAD `b79ba53e`, against the running editor. `niagara.graph.set_parameter` **no longer exists**: calling it returns `[UNKNOWN_ACTION] Unknown action: niagara.graph.set_parameter` with a did-you-mean over the five surviving verbs; the generated wiki has no `niagara.graph.set_parameter.md`; and `NiagaraGraphHandler.cpp` registers only `get`, `connect_pins`, `remove_node`, `create_node` and `search_ops`. It was deleted in `e0d0fe2c` ("RPC audit: remove 170 dead methods, fix 25 in place"), an ancestor of HEAD, which took `#2`'s `PARAMETER_NOT_FOUND` / `UNSUPPORTED_PARAM_TYPE` branches and its `### niagara.graph.set_parameter` section of `Docs/wiki-src/niagara.graph.md` with it. The opaque `[PARAM_FAILED] Parameter not found or type not supported (Float/Bool only).` this ticket is about is therefore unreachable by any caller - closed by removal rather than by the reword, which is a stronger outcome than `#2` aimed at and makes the missing docs section moot. **`#3`'s refutation is also resolved**: the escape hatch both `#1` and `#2` pointed at is real now, not a dead end - `niagara.set_parameter` writes emitter-scoped `Constants.*` rapid-iteration scalars once `emitter` is passed, measured 2 -> 9.5 against a live store in `B-niagara-set-parameter-emitter-scope-unreachable` `#3`, so `NS_Confetti`'s per-emitter scalars would be settable today and the re-do in `#1` would not be needed.
