...
- `#4-configure-vehicle-removed-in-cull` `DONE` reporter — Note (traceability): the opaque `physics.configure_vehicle` this ticket superseded was removed entirely in the RPC cull recorded in [`E-rpc-cull-151-record`](E-rpc-cull-151-record.md) (it is one of the 55 stubs — it built nonexistent `CreateVehicle`/`AddVehicleWheel`/... exec commands and echoed params back, never touching real Chaos vehicle state). Its 4 dedicated tests in `Tests/Gameplay/TestPhysicsHandlers.cpp` (`MissingVehicleName`, `MissingVehicleType`, `ValidParamsNoCrash`, `AllParamsNoCrash`) were deleted with the handler, and the `docs/wiki-src/physics.md` vehicle clause was trimmed. No capability lost — the typed `vehicle.*` authoring shipped by this ticket is the real path. (Aside: `docs/rpc-hard-removal-rejected-candidates.md:143` had previously rejected removing `physics.configure_vehicle`; that rationale assumed its exec commands worked, which they never did, so the rejection was void.) This DONE ticket's own `vehicle.*` methods and their tests are unaffected.
