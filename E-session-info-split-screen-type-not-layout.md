...
encounters: 4
lastSeen: 2026-07-11T01:11:16.9334180+03:00
---
...
- `#5-additional-enabled-boolean-no-roundtrip` `IN-REVIEW` reporter — Additional evidence (NEW ANGLE: the **`splitScreenEnabled` boolean** vs the `configure_split_screen` `enabled` param, the field this ticket's #4-fix explicitly left "untouched (still the live `LocalPlayerCount > 1` bool)"; prior encounters #1-#2 demonstrated only the `splitScreenType` layout enum, now fixed). Surfaced by a LAN couch-coop setup task (focus `session.configure_session_interface`, outcome ergo). Replay-confirmed live at HEAD (editor, PIE NOT running): `session.configure_split_screen {enabled:true, splitScreenType:"TwoPlayer_Horizontal"}` → `{"enabled":true,"splitScreenType":"TwoPlayer_Horizontal","verticalSplit":false,"success":true,"status":"Game user settings configured and saved","settingsSaved":true}`, then `session.get_sessions_info {}` → `{"sessionsInfo":{...,"splitScreenEnabled":false,"splitScreenType":"TwoPlayer_Horizontal",...}}`. So post-#4-fix the layout enum now DOES round-trip (`splitScreenType:"TwoPlayer_Horizontal"`, confirming the fix), but the `enabled` boolean does NOT: the caller sets `enabled:true` and gets `success:true`/`settingsSaved:true`, yet the readback reports `splitScreenEnabled:false`. Source ground truth: `configure_split_screen` reads `bEnabled` (`SessionsHandler.cpp:173`) and echoes it (`:209`) but persists ONLY the layout (`:180` `SplitScreenLayout = bEnabled ? SplitScreenType : "None"`) — there is no `SplitScreenEnabled` field on `FConfiguredSessionSettings` (`State/PluginState.h:20`-`36` has only `bConfigured`/`SessionName`/`MaxPlayers`/`bIsLANMatch`/`SplitScreenLayout`), so the `enabled` bit is dropped; the getter derives `splitScreenEnabled` solely from live player count (`SessionsHandler.cpp:795` `SetBoolField("splitScreenEnabled", LocalPlayerCount > 1)`). Whether this is a lie is arguable — `splitScreenEnabled` reads as the live *runtime* activation (genuinely false pre-PIE with 0 local players), so the friction is really the same field-name-collision this ticket already owns: the `configure_split_screen` `enabled` param and the getter's `splitScreenEnabled` field share a name but different meanings (config intent vs runtime activation), so the natural write→read confirm of the enable flag silently can't close. The lowest-cost fix stays consistent with the #4-fix approach: either persist an `enabled`/`bSplitScreenConfigured` bit and read it back (closing the round-trip), or rename the getter's runtime field (e.g. `splitScreenActive`) so it no longer collides with the setter's `enabled` intent. Grouped here (duplicate of IN-REVIEW, new field angle) rather than a near-duplicate per-field file — this ticket already names `splitScreenEnabled` in its body (`:29`-`30`) and its #4-fix explicitly scoped it out, making this the owning ticket. The task's other friction (no `interfaceType` field in the readback — LAN inferable only via `isLANMatch`, set by `configure_local_session_settings` not `configure_session_interface`) is the session-identity readback gap owned by `E-session-info-readback-hardcoded`; the `add_local_player [NO_GAME_INSTANCE]` on this PIE-stopped host is the docs/prerequisite gap owned by `E-session-wiki-pie-prerequisite-undocumented`. Neither re-filed here.
- `#6-liveness` `IN-REVIEW` reporter — still reproduces at HEAD (same shape as #5: a LAN couch-coop task set `configure_split_screen {enabled:true, splitScreenType:"ThreePlayer_FavorTop"}` which echoes `enabled:true`, then `get_sessions_info` reads back `splitScreenEnabled:false` pre-PIE; only the layout enum differs from #5's `TwoPlayer_Horizontal`, so no new angle). Source unchanged (`SessionsHandler.cpp:180` persists only `SplitScreenLayout`; `:795` derives `splitScreenEnabled` from `LocalPlayerCount > 1`, false with 0 local players pre-PIE); the live editor was unreachable this iteration, so this reconfirm is source-grounded rather than a fresh MCP replay. The task's other note — the host's own LAN IP not being discoverable for `session.join_lan_server`'s `serverAddress` — is not a defect (the task completed with a placeholder IP, real UE LAN join uses subnet auto-discovery, and this namespace is config-only echo stubs); not filed.
- `#7-liveness` `IN-REVIEW` reporter — still reproduces at HEAD (same #5/#6 shape, layout enum `TwoPlayer_Vertical`): `configure_split_screen {enabled:true, splitScreenType:"TwoPlayer_Vertical"}` echoes `enabled:true`/`success:true`/`settingsSaved:true`, then `get_sessions_info` reads back `splitScreenType:"TwoPlayer_Vertical"` (layout round-trip intact post-#4-fix) but `splitScreenEnabled:false` at `localPlayerCount:0` pre-PIE; the couch-coop task additionally observed `splitScreenEnabled` flip false→true once a 2nd local player joined in PIE and back to false after removal — the documented `LocalPlayerCount > 1` live-count semantics (`SessionsHandler.cpp:797`), not the configured `enabled` bit.
