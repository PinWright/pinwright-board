...
## History
- `#3-delay-query-overflow` `OPEN` reporter — reproduces on a non-enum query too, so the trigger is result-set breadth, not only enum expansion. Building seven FPS MetaSound Sources under `/Game/FPS/Audio/MetaSounds/`, the very first discovery call — `audio.authoring.search_metasound_nodes {query:"Delay", limit:30}`, needed only to learn that the trigger-delay class is spelled `UE.Trigger Delay` (with a space) and carries pins `In`/`Reset`/`Delay Time`→`Out` — returned **12617 chars** against the 10000 threshold and spilled to `Saved/PinWright/HttpResponses/20260902T191723Z/20260902T193544Z_be19441f-….json` (33976 file chars). Note `limit:30` was already *below* the default 50 and still overflowed. Recovering the one class the build needed took a disk `Read` plus a stdlib JSON filter to pull the single matching entry out of 30 results, most of which were irrelevant `Convert.Enum:GrainDelayEnvelope.Int32` / `VariableAccessor.*` registry noise. Confirms the proposed `compact:true` shape would have made this inline: className+displayName+flat pins for 30 results is roughly 2 KB. Low severity stands — fully recoverable — but this is the second distinct query family (after `Sine`) to hit it on the first call of a MetaSound build.
