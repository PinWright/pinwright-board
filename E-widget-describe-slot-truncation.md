...
- `#5-repoint-citations-after-property-utils-split` `DONE` reporter — Citation maintenance only; **no behavioural claim changes and the status is untouched**. `Utils/PropertyUtils.cpp` was split into `PropertyExport.cpp` / `PropertyImport.cpp` / `PropertyInspection.cpp` / `PropertyDiff.cpp` (`PropertyUtils.h` survives only as a deprecated umbrella forwarder), so `#2`'s `PropertyUtils.cpp` citation was an unresolvable path, not a stale line number. `#2` is left verbatim per the append-only rule and maps here, verified at plugin HEAD `ef8a1f1b`: `ExportPropertyToJsonValue` `PropertyUtils.cpp:122-129` → `Utils/PropertyExport.cpp:605` (function), with the object-reference branch `#2` means at `:737-759` and the full-path-string emission at `:756-757`. **`#2`'s other citation is stale in the more dangerous way and is corrected here rather than left:** `WidgetDescribeHandler.cpp:385-395` still resolves — same filename, new module path `Source/PinWright/Private/Handlers/UI/WidgetDescribeHandler.cpp` — but at HEAD those lines are **binding-target collection** (`BindingTargetsByFunctionName`, `functionName` / `entryType` / `graphName` / `nodeId`), not slot serialization, so a fixer following the citation lands on live but unrelated code and can reasonably conclude the row is wrong. The slot serialization `#2` describes is at `:459-468`: the `bIncludeSlot && Widget->Slot` gate at `:460`, `type` at `:464`, and `props` via `CollectOverriddenProperties` at `:466-467`. `#2`'s chain still holds — `CollectOverriddenProperties` is defined at `WidgetDescribeHandler.cpp:33` and calls `ExportPropertyToJsonValue` at `:58` — so the row's conclusion is unaffected; only its coordinates were.
