...
- `#4-verified-on-disk-with-the-plugin-enabled` `DONE` tester — **VERIFIED, and verified against the `.uasset` bytes rather than against the verb's own echo.** `#3`'s blocker is gone: the **Procedural Vegetation Editor** plugin is now enabled in `EAContentExamples58.uproject` and the editor restarted, so `/Script/ProceduralVegetation.ProceduralVegetationGraph` is a concrete, loaded `UPCGGraph` subclass on this host — the exact condition `#3` said would settle it.

  **What was measured, this pass, by the tester.** Two graphs created into `/Game/PinWrightScratch/PVTest/` and explicitly saved, then their package bytes grepped on disk (occurrence counts, not line counts):

  | created with | `/Script/ProceduralVegetation` | `ProceduralVegetationGraph` | `/Script/PCG` | `PCGGraph` |
  |---|---|---|---|---|
  | `graphClass: "ProceduralVegetationGraph"` | **2** | **4** | 2 | 1 |
  | `graphClass` omitted | **0** | **0** | 3 | 5 |

  The subclass graph carries the PV package import and its own class name in the serialized package; the defaulted graph carries neither. That is the ticket's central assertion — *a non-default `graphClass` yields an asset of that class* — settled by the file, independently of the response. The echo agreed (`graphClass: "/Script/ProceduralVegetation.ProceduralVegetationGraph"`, `assetClass: "ProceduralVegetationGraph"`), but the echo is not what closes this: `#2` reads it off `Graph->GetClass()`, so an echo can only ever confirm the in-memory object, and the question was what got written.

  **Both advertised spellings produce the subclass.** The table's row was created with the **short** name; an earlier pass this session got the identical result from the full path. So `ResolveUClass` (`PCGGraphCreate.cpp:51`) delivers on the schema's *"full path (`/Script/<Module>.<Class>`) or short class name"* (`PCGGraphCreate.cpp:30`) for this verb.

  **The plugin's own permanently-skipped test is no longer permanently skipped.** `PinWright.pcg.create_graph.GraphClassSelectsSubclass`'s block 4 sweeps `TObjectIterator<UClass>` for a non-abstract `UPCGGraph` subclass and calls `PinWrightTestSkip::SkipAssertions(..., "no-upcggraph-subclass-loaded", ...)` when it finds none (`Source/PinWrightPCG/Private/Tests/PCG/TestPCGGraphHandlers.cpp:455-482`, skip at `:477`). On a host with this plugin enabled that sweep now finds `UProceduralVegetationGraph`, so **the skip will not fire and block 4 runs for real.** Re-running the suite on such a host is the cheap way to convert `#3`'s honest skip into standing coverage; that is a suite-run, not a code change, and nothing here is blocked on it.

  **`#1`'s and `#2`'s two standing questions, answered — neither changes this ticket's correctness, and both were the value questions `#1` said they were.**

  1. *Is `NewObject<UProceduralVegetationGraph>` sufficient without the PV factory?* **For the graph object, yes — proven above. For the PV workflow, no, and the distinction is a separate defect.** The asset the Procedural Vegetation editor actually opens is not this class: `UAssetDefinition_ProceduralVegetation::GetAssetClass()` returns `UProceduralVegetation` (`C:/UE_5.8/Engine/Plugins/Experimental/ProceduralVegetationEditor/Source/ProceduralVegetationEditor/Private/AssetDefinition_ProceduralVegetation.cpp:21-24`), which is not a `UPCGGraph` at all but a `UProceduralVegetationInterface` subclass (`.../ProceduralVegetation/Public/ProceduralVegetation.h:60-61`, abstract base `:48`) that **owns** a `UProceduralVegetationGraph` in a private `UPROPERTY` (`:65-66`), built by `UProceduralVegetation::CreateGraph` (`.../Private/ProceduralVegetation.cpp:22-33`). Since `pcg.create_graph` is gated on `IsChildOf(UPCGGraph)` (`PCGGraphCreate.cpp:62`) it structurally cannot make that outer asset, and correctly should not try. Filed as its own ticket — `F-generic-asset-create-verb` — rather than reopening this one: this verb's product is a real, correctly-typed `UProceduralVegetationGraph`, which is exactly what the ticket asked for.
  2. *Do PV nodes execute outside the PV editor UI?* **They author and schedule; what they produce is unobservable.** Measured this pass in the graph above: `pcg.add_node` accepted `/Script/ProceduralVegetation.PVSeedGeneratorSettings` and returned `SeedGenerator_0`, `pcg.connect_pins` wired it, and `pcg.inspect` read the structure back — the whole link-2/3/4 chain this ticket predicted would need nothing. What does not work is reading the result: every countable field `pcg.generate` publishes is structurally zero for a PV graph whether it worked or not, filed as `B-pcg-generate-blind-to-non-point-data`. That bounds this ticket's **value** precisely as `#1` anticipated ("Neither question changes the fix; both change how much the fix buys"), and does not touch its correctness.

  **Not re-litigated, and confirmed present at HEAD:** the `CLASS_Abstract` guard and the `IsChildOf` gate (`PCGGraphCreate.cpp:61-63`), the resolution-before-`CreatePackage` ordering, and the `PLUGIN_DISABLED` routing at `:52`. `#3` verified all three by inspection and nothing in this pass contradicts it.

  **Two findings that came out of this verification and are filed elsewhere, not here.** (a) The sibling verb `pcg.add_node` **refuses** the short class name that `create_graph` accepts — measured, on the stock and definitely-loaded `PCGCreatePointsSettings` as well as on `PVSeedGeneratorSettings`, both `CLASS_NOT_FOUND` — because it resolves with `FindObject`/`LoadClass` and never calls `ResolveUClass` (`PCGGraphAuthoring.cpp:62-66`; zero `ResolveUClass` occurrences in the whole `PinWrightPCG` module outside `PCGGraphCreate.cpp:51`). Filed `E-pcg-add-node-rejects-short-class-name`, and it corrects a claim on `E-plugin-disabled-guard-misses-short-class-name`. (b) `pcg.md:12`'s provenance sentence is *narrowly* true now that a real subclass exists and still false under its natural reading — recorded on the ticket that already owns it, `E-pcg-graph-class-echo-not-provenance`, which stays OPEN.
