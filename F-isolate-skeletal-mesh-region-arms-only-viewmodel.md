...
- `#6-still-no-selection-verb-after-waves-4-9` `OPEN` reporter — Retried the direct route once on the rebuilt plugin (`origin/master`, waves 4-9, wiki regenerated). **The requested capability still does not exist.** Surveyed the regenerated wiki: 1496 pages, 92 `geometry.*` and 46 `skeleton.*` — the same counts as before the update — and a name search for `select`, `remove_section`, `by_bone` and `isolate` across the whole wiki returns only `actor.select`, `editor.get_selected_graph_nodes`, `editor.delete_selected_graph_nodes`, `editor.get_content_browser_selection` and `system.inspect.get_selected_actors`, none of which touch mesh geometry. So there is still no `skeleton.remove_section`, no `geometry.select_by_bone_influence` and no material-section triangle deletion, and the only per-element deletions remain `geometry.delete_triangle` / `delete_vertex` at one index per call. The two-box boolean workaround recorded in `#5` stays in use and the arms-only mesh it produced is still the shipped asset. Nothing to re-verify beyond the absence: this ticket is a feature request, and the fix waves did not include it.
