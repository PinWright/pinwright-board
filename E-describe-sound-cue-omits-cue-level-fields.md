...
## History
- `#2-fix` `IN-REVIEW` developer — Extended the shared `SoundCueDumpBuilder::BuildSoundCueJson` to emit a cue-level block alongside the node graph: `concurrency` (sorted `ConcurrencySet` paths, empty array when none), `attenuation` (`AttenuationSettings` path, empty string when unset), `soundClass` (`SoundClassObject` path), `volume` (`VolumeMultiplier`), `pitch` (`PitchMultiplier`) — mirroring `SCIRDecompiler::AppendCueFields`. Because `describe_sound_cue` (AudioAuthoringHandler.cpp:2907) and the `sound_cue.json` sidecar (REGISTER_DUMP_JSON_SIDECAR at SoundCueDumpBuilder.cpp) share this one builder, both surfaces now verify cue-level writes without a `decompile_sound_cue` (SCIR) fallback. Bumped the `sound_cue.json` aspect version 2→3 in AssetDumpCache.cpp per the cache-freshness rule, and updated the describe_sound_cue handler summary. Files: `Source/PinWright/Private/Handlers/Asset/SoundCueDumpBuilder.cpp`, `Source/PinWright/Private/Handlers/Asset/AssetDumpCache.cpp`, `Source/PinWright/Private/Handlers/Audio/AudioAuthoringHandler.cpp`, `Source/PinWright/Private/Tests/Assets/TestSoundCueDumpBuilder.cpp`. Regression test: `PinWright.Assets.SoundCue.DumpBuilder.CueLevelFields` (builds an in-code transient cue with concurrency/attenuation/soundClass/volume/pitch set, asserts BuildSoundCueJson surfaces all five; fails if the fix is reverted).
