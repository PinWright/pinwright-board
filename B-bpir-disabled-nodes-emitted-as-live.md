...
- `#6-no-longer-reproduces-three-surfaces-agree` `IN-REVIEW` WEAPONS-critic — **Verification of `#5`'s fix in use: it works. NO LONGER REPRODUCES.** Status deliberately unchanged at `IN-REVIEW`, and `encounters` / `lastSeen` deliberately **not** bumped — the defect was not observed, so recording an encounter would be false. Closing is the user's call, not mine. Re-measured in a WEAPONS critic review round 5 on `/Game/FPS/Weapons/BP_Weapon_AR`, the same asset that carried `#1` through `#4`. `asset.dump`'s `bpir.txt` now emits `entry event Tick(float DeltaSeconds) disabled @(0,416) { parent_call BP_WeaponBase_C::ReceiveTick(DeltaSeconds: $DeltaSeconds) disabled @(200,416) }`, and the same shape for `ActorBeginOverlap`. **Both halves the ticket was filed for are fixed in one emit:** the `disabled` marker is present on the entry *and* on the instruction inside it (ask 1 from `#1`), and the parent-call qualifier is present and correct as `BP_WeaponBase_C::` — the **generated** class. Note on the qualifier for anyone comparing against the older entries: `#2`/`#3`/`#4` recorded the working form as `SKEL_BP_WeaponBase::`, which is the **skeleton** class; the emit now names the generated class, which is the right one and is not a regression against those entries — see `B-bpir-decompile-emits-skel-class-prefix-on-self-event-calls` for why the `SKEL_` spelling was itself the wrong target. That directly retires the consequence `#3` and `#4` rated this High for: a mechanical round-trip of the AR's BPIR no longer produces `call ReceiveTick(...)` inside `entry event Tick`, so the infinite-self-recursion path is gone with the qualifier restored. **Cross-checked on two further surfaces, and all three agree:** `blueprint.graph.find_nodes {query:"Tick", includeNodeState:true, graphName:"EventGraph"}` returns `K2Node_Event_2` and `K2Node_CallParentFunction_2` both `enabledState:"disabled", isEnabled:false`; `blueprint.get` returns `ReceiveTick enabled:false`, `ReceiveActorBeginOverlap enabled:false`, `ReceiveBeginPlay enabled:true`. So BPIR, the graph-node layer and the events summary now report the same node state, which is the three-surface consistency `#1`'s Related section set as the target. **What this entry does not cover, so a tester knows the remaining bar:** `#5`'s own note flagged that its automation bypasses the `blueprint.decompile` / `blueprint.compile_bpir` handlers and saved-asset persistence, and that parent-call identity was a follow-up. This entry exercises the **decompile** direction through the real `asset.dump` handler on a saved asset and finds it correct — including the parent call, which now emits as `parent_call`, a distinct opcode rather than a bare `call`. It does **not** exercise `compile_bpir` on that text, so the round-trip half of ask (2) is still unverified here; `#2` and `#3` are the entries whose predictions about recompilation remain untested. No plugin source was opened for this entry; every claim above is read off `asset.dump` / `bpir.txt`, `blueprint.graph.find_nodes` and `blueprint.get` responses, and no `file:line` is claimed.
