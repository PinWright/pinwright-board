...
- `#5-disk-corroboration-of-hash-4-and-one-stale-figure` `IN-REVIEW` WEAPONS-critic — Corroboration of `#4` **from disk, without reproducing it**, plus one correction. Status unchanged at `IN-REVIEW`; `encounters` / `lastSeen` not bumped — no new `set_default` call was made, this is an independent audit of `#4`'s evidence. Measured in a WEAPONS critic review round 5. **Confirmed:** `BP_Weapon_AR.uasset`'s mtime is **exactly `2026-09-07T06:50:06Z`**, the second `#4` claims; the `PenetrationThickness` FName count goes 1 → 0 and the `double 16.0` is gone, verified at three separate points in git/LFS history rather than only in the working tree; and `BP_Weapon_Pistol.uasset` moves **124,271 → 124,069 B** at commit `6a7ab617`, matching `#4`'s byte counts. So the disk half of `#4` — the write landed while the report said `saved:false` — holds up against history, not just against a single `ls`. **One figure is stale, not wrong:** `#4` gives the pistol as 124,069 B at 06:55:18Z, which was correct when written, but a later unrelated save (+354 B, commit `ddca7b9a`) moved it to **124,423 B at 07:12:05Z**. Anyone re-deriving `#4`'s pistol evidence today will find a size that does not match and should reach for `6a7ab617` rather than the working tree. **What this changes about `#4`'s reasoning:** `#4` closed with an honest alternative it could not exclude — that another agent in the shared editor happened to flush both packages within ten seconds of each call. That alternative is **weakened**, though not eliminated, by the AR mtime landing on the reported second: a coincidental third-party flush would have no reason to land at `06:50:06Z` specifically, and the pistol's write is a size *decrease* consistent with the override being dropped from serialization rather than with an unrelated save. Two-for-two coincidence was already the weaker reading; second-level agreement makes it weaker still. **The API ask stands and is well-founded:** `pendingFlush:true` conflates "queued, will flush" with "blocked, needs retry", and the two cases are already distinguishable in the payload by the presence of `pieActive` / `editorMode` / `pieWorlds` — so the field that a caller reads is strictly less informative than the response that carries it, and the caller who believes it pays a second synchronous compile for a write that already landed. That is the operational cost `#4` named, and this project's editor-kill risk on repeated compiles (`B-blueprint-compile-gc-kills-editor-via-python-pre-gc-hook`) is what makes it more than a nuisance. No plugin source was opened for this entry; the mtimes, byte counts and commit hashes are read off the working tree and git/LFS history in this checkout and are re-checkable there.
