...
- `#6-retract-the-complex-as-simple-mechanism-not-the-gap` `DONE` tester — **Correcting my own `#5`, same session, before anyone builds on it.** `#5` states that a `CTF_UseComplexAsSimple` body "is a hull that reports one [face index]" and uses that to explain why the `warning` cannot fire for an instanced scatter. **That engine behaviour is unverified and should not be read as established.** The follow-on ticket's author went looking for it and found that `InstancedStaticMesh.cpp` and `HierarchicalInstancedStaticMesh.cpp` contain **zero** occurrences of `CollisionTraceFlag` or `ComplexAsSimple`, and that ISM builds per-instance bodies from `GetBodySetup()`, so whether a complex-as-simple HISM yields a face index on a simple trace is an open question, not a finding. It came into `#5` from my brief as an assumption and I did not flag it as one; that is the error being corrected here. **The gap `#5` names is unaffected and still holds**, because it does not depend on that branch. The `warning` is gated on `PrimitiveColumns > 0` (`Handlers/Spatial/GroundPlacementUtils.cpp:889`) and `PrimitiveColumns` is only the `else` arm of `if (Column.GroundFaceIndex >= 0)` (`:518-525`), so the alarm fires on **face-index absence**, not on **wrong surface** — and `B-hull-warning-blind-to-instanced-scatter` was rebuilt on two routes that are source-verified rather than on the retracted one: an explicit `traceComplex: true` (which is the workaround this very ticket publishes, and which routes hits into `triangleColumns` by design), and `B-trace-complex-hits-render-geometry`'s collisionless-foliage path. So `#5`'s conclusion stands on better evidence than `#5` gave for it, and the DONE verdict is unchanged: what shipped here does what this ticket asked for. Nothing else in `#5` is retracted — the quoted `#4` sentence is still narrower in practice than it reads, for the ordinary no-face-index hull case, and the two-signals-two-questions distinction is unaffected.
