...
- `#3-scoped-fixup-and-collateral-report` `IN-REVIEW` developer — Fixed items 1 and 2. Scope now lives in RedirectorFixupPolicy (ESweepScope / ParseSweepScope / PackageFoldersForAssets / BuildSweepFilter / IsInPackageFolders, Utils/RedirectorFixupPolicy.h/.cpp) so both verbs share one definition of "inside the requested set". asset.bulk_delete gains fixupScope (string, optional, RPC_PARAM_DEF default "paths"): "paths" sets FARFilter.PackagePaths to the deleted assets' own package folders with bRecursivePaths=FALSE (recursive would re-widen to the whole project for an asset near a content-root top), "project" is the explicit opt-in to the old class-only filter, anything else is refused with INVALID_ARGUMENT parsed BEFORE ObjectTools::DeleteObjects runs so a typo costs no delete. An empty resolved folder set pins PackagePaths to an impossible path, because an empty FARFilter.PackagePaths means every path — the original bug's back door. Collateral reporting: FResult::DeletedRedirectorPackages records the packages actually removed, measured after the delete off a weak pointer that stopped resolving rather than predicted from the candidate list (DeleteObjects refuses in-memory-referenced objects and only lowers its count); surfaced as redirectorsDeletedPaths[] via AddReport on BOTH verbs, plus redirectorsDeletedOutsideScope[] on bulk_delete — always emitted, empty array included, since an omitted field cannot be told apart from not looking. Item 3 (global write/delete gate) NOT attempted: separate structural feature, out of scope. Item 4 (TestActorDuplicateMeshIntegrity) NOT attempted: owned by B-tests-leak-host-content. asset.fixup_redirectors' empty-directoryPath default DELIBERATELY LEFT project-wide — the sweep is that verb's job, it already has a scope parameter, and its default is documented; the decision and its re-open condition are recorded at the call site in AssetWorkflowHandler.cpp, in defect-backlog D-75, and on the wiki page. Tests: Tests/Assets/TestBulkDeleteRedirectorScope.cpp — PinWright.asset.bulk_delete.OutsideRedirectorsSurviveTheDefaultScope (dispatches the real verb at defaults against sibling-folder fixtures and asserts the outside redirector survives), .ProjectScopeStillReachesOutsideRedirectors, .EmptyPathSetMatchesNothing, .UnknownFixupScopeIsRejected, .FixupScopeIsDeclaredWithPathsDefault. All fixtures are never-saved in-memory packages under /Game/PinWrightTests/BulkDeleteScope/, announced with FAssetRegistryModule::AssetCreated; nothing is written to host Content. NO test dispatches the verb at fixupScope:"project" — that would delete every redirector in the host, i.e. commit the very damage; the opt-in is asserted on the FARFilter against the live registry as a read. Docs updated in the same change: Docs/wiki-src/asset.md gains an ### asset.bulk_delete section and stops documenting the whole-project sweep as intentional; Docs/plans/defect-backlog.md D-72 reframed from "freezes the editor for minutes" to the data loss and marked RESOLVED, with the fixup_redirectors decision noted under D-75; Docs/rpc-design.md §3 gains the universal rule (scope every registry query whose results you mutate; wide scope is a named opt-in; empty scope matches nothing; report collateral always) plus a before-you-ship checklist line. Compile-checked with Build.bat -SingleFile -NoHotReloadFromIDE: RedirectorFixupPolicy.cpp, AssetWorkflowHandler.cpp and TestBulkDeleteRedirectorScope.cpp each produced a real [1/1] Compile line and Result: Succeeded. NOT VERIFIED BY ME: the automation suite was not run — a peer agent owns the editor and the single full build/suite for this wave.
