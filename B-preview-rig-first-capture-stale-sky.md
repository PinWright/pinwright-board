...
## History
- `#4-bumped-by-cost` `OPEN` orchestrator — Severity Medium -> High by cost. Costly encounters counted: #1 (the capture agent's first `camera.orbit_shots` after the rig change returned a near-black frame certified `previewScene.applied:true` + `warmup.settled:true`, so a wasted shot was published as good and only the next call disclosed it), #2 (the rendering agent's 240-still set came back at the settle ceiling and had to be re-shot after `r.AntiAliasingMethod 0`), #3 (18 shots decoded off disk with a stdlib decoder to measure a period-4 plinth cycle, which also retracted this ticket's own Ask 1 — the "apply once for the whole set" seam it named does not exist, and two source comments assert the same false thing). Reach also applies: the encounters come from three independent streams (capture agent, rendering agent, audit pass) and every preview-scene capture on either verb goes through the same uncticked path.
...
- `#5-drain-the-sky-capture-once-per-rig` `IN-REVIEW` developer — The capture now drives the sky/reflection drain the editor tick never runs for it. New `PinWrightPreviewSceneRig::UpdatePreviewSceneCaptures(FPreviewScene*)` (`Source/PinWright/Private/Handlers/Render/PreviewSceneRig.h`/`.cpp`) calls `FPreviewScene::UpdateCaptureContents()` then `World->SendAllEndOfFrameUpdates()` then `FlushRenderingCommands()`, and returns false (never claiming a drain) when the preview world has no `FSceneInterface`. `FScopedPreviewSceneRig` calls it in ALL THREE of its construction paths and records the answer in `bCaptureContentsUpdated` / `CaptureContentsUpdated()`: on the viewport path after `ApplySceneRigValues` and before `Client->Invalidate()`; on the omitted-parameter path too, because a set with no `previewScene` write reads the same never-drained queue (this ticket's `#2`); and on the transient mesh path, where the two pre-existing bare `Scene->UpdateCaptureContents()` calls are now routed through the helper so they also flush and report. ONCE PER RIG LIFETIME, NOT PER SHOT: the guard is constructed once per set at `PoseListCapture.cpp` `CaptureCameraPoses`, and the answer is forwarded to each shot's receipt through the new `bPreviewSceneCaptureUpdatedAtSetEntry` on both `FPoseListCaptureRequest` (`PoseListCapture.h`) and `FViewportCaptureRequest` (`PreviewViewportCaptureUtils.h`), mirroring the existing `PreviewSceneRigAtSetEntry` handoff; `MeshPreviewCaptureUtils.cpp` reports it off its own guard. RESPONSE SHAPE: `viewport.previewScene` gains `captureUpdated` (published beside `applied`, which is a true statement about the WRITES only) and a `skyCaptureWarning` gated on `!captureUpdated` alone — NOT on `!bWarmupSettled`, since the settle loop's pump cannot drive the work it would have to observe. Docs updated: `Docs/wiki-src/render.preview-scene-rig.md` (two field rows plus a new section '`applied` is about the writes; `captureUpdated` is about the pixels') and `Docs/preview-scene-rig.md` §1b. Regression test `PinWright.render.preview_scene_rig.RigApplyDrivesTheSkyCaptureUpdate` in `Source/PinWright/Private/Tests/Render/TestCapturePreviewSceneRig.cpp`, in two halves: a pure serializer half (never skippable) asserting `captureUpdated` is published both ways and that `skyCaptureWarning` is present only on false, and a production half that builds an `FPreviewScene` + `FEditorViewportClient`, drains the fixture's own world with the ENGINE's `UpdateCaptureContents()` so the precondition does not depend on the code under test, asserts `USkyLightComponent::HasSkyCapturesToUpdate()` is then false (skipping with the `PINWRIGHT_ASSERTIONS_SKIPPED` marker when the process-wide queue is not isolable or the preview world has no renderer scene), calls `SkyLight->SetCaptureIsDirty()`, asserts the queue is armed, and then measures INSIDE the guard's scope. COUNTERFACTUAL: delete the `UpdatePreviewSceneCaptures` call from `FScopedPreviewSceneRig`'s constructor and the queued capture is still queued when the scope is measured — `HasSkyCapturesToUpdate()` stays true and `CaptureContentsUpdated()` reports false, so both the in-scope `TestFalse` and the `captureUpdated` assertion fail. The omitted-parameter half of the test carries the same counterfactual for the no-rig path. One row added to `GPoseListFields` in `Source/PinWright/Private/Tests/Render/TestCaptureVerbParameterParity.cpp`, which parses `FPoseListCaptureRequest` from its header and fails on an unmodelled field. NOT DONE, and deliberately: Ask 3 (a warning at the settle-ceiling / relative-tolerance boundary) is left alone — `#2` and `#3` of this ticket show that signature is neither specific (it fires on every TSR host) nor sufficient (the measured stale set reported `settleRounds: 1`), and its real fix is a per-pixel settle criterion, which belongs with the anti-aliasing ticket rather than here.
- `#6-driven-is-not-completed-plus-queue-isolability` `IN-REVIEW` developer — Verifier follow-ups on `#5`, no behaviour reverted. (1) `captureUpdated: true` said the drain was DRIVEN and was being read as "the pixels are current". `UpdateSkyCaptureContentsArray` re-queues a component as `SLCS_CapturedButIncomplete` while shaders/textures/meshes are async-compiling and retries no sooner than 5 s later (`SkyLightComponent.cpp:771-787`, `:837-860`), so a full drain can change nothing — the first capture after any asset load is the ordinary way there. `UpdatePreviewSceneCaptures` now takes `bool& bOutCaptureIncomplete` and fills it from `USkyLightComponent::HasSkyCapturesToUpdate()` read AFTER the drain; the guard exposes `CaptureContentsIncomplete()`; the response emits `previewScene.captureIncomplete: true` plus `captureIncompleteWarning`, and OMITS both on the clean path and on the `captureUpdated:false` path where there is nothing to have measured. Deliberately NOT folded into `captureUpdated`: the engine predicate is process-wide, so folding would let another editor window's pending capture decide this capture's verdict. The warning states the one direction it can be wrong in — false alarm from a foreign world, never silent while this scene's own capture is pending. Serializer comment at `PreviewViewportCaptureUtils.cpp` (the `captureUpdated` field) softened from "reached the PIXELS" to "necessary, and on its own not sufficient"; the same softening applied to the `bPreviewSceneCaptureUpdated` field comment in `PreviewViewportCaptureUtils.h`, the `UpdatePreviewSceneCaptures` header contract, and both doc pages. Plumbed as `bPreviewSceneCaptureIncompleteAtSetEntry` on `FPoseListCaptureRequest` and `FViewportCaptureRequest` alongside its `Updated` twin (one more `GPoseListFields` row in `TestCaptureVerbParameterParity.cpp`), and off the guard in `MeshPreviewCaptureUtils.cpp`. (2) TEST FLAKINESS: `HasSkyCapturesToUpdate()` reads a PROCESS-WIDE static, and isolability was established only once, before the requested-rig scope — a sky light dirtied anywhere in the editor between the two scopes (the guard's own destructor can broadcast into every live `FAdvancedPreviewScene` on the profile-restore path) would have turned the omitted-parameter half into a false FAILURE. Isolability is now re-established immediately before that half and takes the `PINWRIGHT_ASSERTIONS_SKIPPED` marker (`reason=sky-capture-queue-not-isolable`, noting that the requested-rig half already ran its assertions) instead of failing. The wire half of `PinWright.render.preview_scene_rig.RigApplyDrivesTheSkyCaptureUpdate` gains the `captureIncomplete` cases — published with its warning when the drain left something queued, omitted when clean, omitted when the drain never ran, and NOT accompanied by `skyCaptureWarning` (whose premise is false once the drain ran) — and the production half asserts the guard's own `CaptureContentsIncomplete()` is false, so the published field is exercised and not just the static the direct assertion reads. That half is pure and unskippable, so the counterfactual in `#5` is unchanged and this round adds one: revert the `bOutCaptureIncomplete` plumbing and the deferred-capture case fails with no `captureIncomplete` field. NOT DONE, and out of scope here: `camera.orbit_shots` — the verb this ticket was filed from — has NO shader/asset-compile readiness-gate call site, so the deferred state is reported rather than waited out. That gate belongs to the capture-readiness ticket's follow-up (another implementer is adding `CaptureReadinessGate` on `render.capture_open_level` / `editor.screenshot`), not to this one.
