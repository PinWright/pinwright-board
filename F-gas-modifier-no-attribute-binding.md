...
encounters: 3
lastSeen: 2026-07-11T10:38:29.0609816+03:00
---
...
- `#5-liveness` `IN-REVIEW` reporter — still reproduces at HEAD (regeneration-buff GAS task, seed `gas.set_effect_period`, culprit `gas.add_effect_modifier`): documented `_C.AttrName` form `attribute:"/Game/GAS/AS_Vitals_C.Health"` → `[ATTRIBUTE_SET_NOT_FOUND] Could not load attribute set class '/Game/GAS/AS_Vitals_C'`, recovered via `gas.get_gas_info` + package-qualified `attribute:"/Game/GAS/AS_Vitals.AS_Vitals_C.Health"` (success). Broken example string still ships at `GASHandler.cpp:2096` (`gas.add_effect_modifier`) and `:2228` (`gas.set_modifier_attribute`) — the `#3`/`#4` fix has not landed.
- `#6-liveness` `IN-REVIEW` reporter — still reproduces at HEAD (stacking-poison GAS task, seed `gas.set_effect_stacking`, culprit `gas.add_effect_modifier`): documented `_C.AttrName` form `attribute:"/Game/GAS/AS_Combat_C.Health"` → `[ATTRIBUTE_SET_NOT_FOUND] Could not load attribute set class '/Game/GAS/AS_Combat_C'`, recovered via package-qualified `attribute:"/Game/GAS/AS_Combat.AS_Combat_C.Health"` (success, `modifierCount:2`). Broken example string still ships (`GASHandler.cpp:2096`/`:2228`) — the `#3`/`#4` fix has not landed.
