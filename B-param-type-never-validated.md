...
- `#2-dispatcher-type-gate-landed` `IN-REVIEW` developer — Landed Option A's gate directly (no warn stage) and skipped Option B; both deviations are stated below. `FRpcDispatcher::ValidateHandlerParams` now runs a third pass after the unknown-name pass: `CollectDeclaredTypesByWireName` resolves the declared type per MATCHED WIRE NAME (canonical, untyped alias inheriting its spec's type, typed alias using `FParamAliasSpec::Type`), and the new `Handlers/ParamTypeCheck.h` decides accept/refuse. Refusals collect every offending key into one `PARAM_TYPE_MISMATCH` (new `ErrorCodes.h` constant, deliberately distinct from the domain-axis `INVALID_PARAM_TYPE` / `INVALID_PARAMETER_TYPE` / `PARAMETER_TYPE_MISMATCH`) naming param, declared type and received type. **JSON `null` is refused for every declared type including `any`**, required or optional, with "omit the parameter instead of sending null" — that closes C1, C4 and C13 and the whole serializer-emits-null vector. Lossless coercions are kept: number/bool→string, numeric-string→number, boolean-spelled-string→boolean, bool↔number. The numeric predicate is `FString::IsNumeric` and the boolean predicate mirrors `FToBoolHelper::FromCStringWide` **exactly**, because those are the predicates `TJsonValueString::TryGetNumber` and `FCString::ToBool` gate on — so the gate cannot promise a recovery the accessor declines (this is why "1e5" and " true " are refused, and why `LexTryParseString` was rejected: it answers true for "2s"). Unparseable/unknown declared types fail open. Order is missing-required → unknown-name → type, so neither existing answer is displaced. New test file `Tests/Infra/TestParamTypeGate.cpp`, 7 leaves under `PinWright.infra.dispatcher.ParamTypeGate.*` (`check_test_ids.py` CLEAN), covering required-null, optional-null, `any`-null, six wrong-shape rows, the lossless set, typed-alias-uses-its-own-type both ways, ordering, and the destructive instance **C1**: `widget.replace_class` with `preserveProperties: null` (and with `"y"`) must answer `PARAM_TYPE_MISMATCH` — pre-fix it reaches the body and answers `NOT_FOUND`, so the assertion is red before the fix and no widget is loaded, renamed or discarded after it.
  **Compatibility was measured, not assumed, by two exhaustive surveys.** Only ~53 test files route through the dispatcher at all (`Tests/TestUtils.h`'s `InvokeHandler` calls `Reg.Func` directly and never reaches the gate); ~297 (method,key) pairs were hand-checked plus the ~1,664 machine-shaped probes of `RequiredParamGate.EveryVerb`, whose `SetPlaceholder` `StartsWith` ladder agrees with this grammar on every union spelling in the tree. That found real UNDER-DECLARATIONS, fixed here rather than by loosening the gate: `exposure` was declared `object` on all 8 capture verbs while `ParseExposurePin` documents and implements a bare-number shorthand → now `object|number`; the 13 projection `fields` params were `array` while `ReadFieldProjection` documents "A single string is also accepted" (and their singular `field` alias inherits the type) → now `array|string`; plus `audio.create_ambient_sound.location` → `object|array`, `material.authoring.add_function_input.defaultValue` → `array|number`, `landscape.create.location` → `object|array`, `niagara.create_ribbon.color` → `object|array`. `Docs/rpc-design.md`'s "Declaring the param IS the coverage" bullet, which the ticket cites as unqualified, now carries the type half.
  **WHAT REMAINS, and it is the reason a follow-up is wanted rather than a clean close.** (1) The `object` row accepts an ARRAY. That is a measured carve-out, not JSON semantics: 205 vector-shaped slots (`location` 68, `rotation` 66, `extent` 23, `scale`, `center`, `offset`, `translation`, `point`, `force`, `deltaWorld`, `additionalOffset`, `min`, `max`) across 36 handler files declare `object` while `FHandlerContext::GetVector`/`GetRotator` → `ExtractVectorField` → `ReadVectorFieldImpl` (`Utils/JsonUtils.cpp:16-47`) reads `{x,y,z}` **or** `[x,y,z]` at 111 confirmed call sites. Refusing the array form would have broken a working documented convention on ~200 slots with nothing in the suite to catch it, and no confirmed instance here needs that direction refused. The residual is that an `object` slot which genuinely means object-only still silently drops an array through `GetObject()`'s nullptr. The honest fix is to correct those 205 declarations to `object|array` verb by verb against their read path and then tighten the row; a blind sweep would loosen the genuinely-object ones instead. Pinned by `ParamTypeGate.AcceptsLosslessCoercions` so tightening is deliberate. (2) **Option B's two-accessor patch was deliberately NOT shipped, because as written it contradicts Option A.** `RequireInt` gaining `HasTypedField<EJson::Number>` would refuse `"100"`, and `RequireString` gaining a strict check would refuse a number — both are the lossless coercions Option A keeps, so the accessor and the gate would give opposite answers for the same value. With the gate in place both accessors are only reachable with an already-type-checked value, so the patch buys nothing and costs a contradiction. (3) `FParamAliasSpec::Type` is now READ at runtime but is still not covered by `ParamTypes.ValidTypeNames`, which iterates `Reg.Params` only — a novel typed-alias type would fail open silently (one production producer today, `BlueprintHandlerUtils::MakeBlueprintPathParamTypedAliases`, always `"array"`). (4) No warn stage was run, per the wave brief's requirement to refuse; the surveys above are what replaced its dwell time, and they cover the automation suite and the declaration vocabulary but not real-world wire callers. (5) `BuildCallToolDescriptor` still advertises `args` as a bare `{"type":"object"}`; the per-verb schema export is untouched. Not built and not run — rule 2 of the wave brief; the orchestrator owns the compile and suite.
