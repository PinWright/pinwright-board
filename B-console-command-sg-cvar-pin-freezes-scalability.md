...
- `#3-cross-link-the-member-cvar-door` `IN-REVIEW` reporter — **Cross-link only. Status unchanged, and `encounters` / `lastSeen` deliberately left untouched** — this is not a re-observation of the `sg.*` defect, and bumping a work-ordering tiebreak would tell the picker otherwise. **The guard `#2` shipped is correct for what it covers and does not cover the wider door: a scalability group is a bundle of ordinary cvars, and setting one of those members by console pins it at `ECVF_SetByConsole` through the identical engine path.** `IsScalabilityGroupLine` (`Source/PinWright/Private/Handlers/ScalabilityConsoleGuard.h:38-41`) is `CommandLine.TrimStart().StartsWith(TEXT("sg."), ESearchCase::IgnoreCase)` and nothing else — no console-object lookup, no `ECVF_Scalability` test — so `r.ViewDistanceScale 0.6`, `r.Shadow.Virtual.MaxPhysicalPages 4096` and `r.Streaming.PoolSize 2048` are all allowed and all freeze their group against the user's Scalability panel exactly as `sg.<Group> N` would. The chain that makes them equivalent, re-derived at HEAD and not relayed: the console pins *any* cvar given a value at `ConsoleManager.cpp:3228` (`CVar->Set(*Param2, ECVF_SetByConsole);`), while the panel reaches the **member** cvars one hop past the citation this ticket already carries — `Scalability.cpp:907` writes the group cvar, its sink (`:559-562`, `:569-572`) calls `SetGroupQualityLevel` (`:412`), which applies the whole `[<Group>@N]` ini section at `Scalability.cpp:446` -> `Runtime/Core/Private/Misc/ConfigUtilities.cpp:365` (`CVar->Set(Value, (EConsoleVariableFlags)SetBy, Tag)` with `SetBy == ECVF_SetByScalability`); `r.ScreenPercentage` takes an ini-less variant of the same at `Scalability.cpp:551`. Same `FConsoleVariableBase::CanChange` veto, same Warning. **Measured on the build that ships this ticket's own guard** (editor 13:32, plugin `d8f1bc32`): `Saved/Logs/EAContentExamples58.log` frame 178 records six cvars at `LastSetBy: Console`, of which `r.Shadow.Virtual.ResolutionLodBiasDirectional` (ShadowQuality) and `r.ScreenPercentage` (ResolutionQuality) are scalability-driven and are therefore frozen for the session — set by `system.console_command` lines the guard did not refuse. **Filed as its own ticket, `B-console-member-cvar-pin-freezes-scalability` (OPEN, High), not as an encounter here**, because the fixes are different work with different failure modes (a two-character prefix test versus a flag lookup on the resolved console object, which has an over-refusal risk on bare reads that the prefix test does not), because neither fix closes the other, and because this ticket is IN-REVIEW — widening its scope now would ask its tester to verify something nobody built. One correction to this ticket's own § *Nothing in the RPC surface can observe the pin*, offered rather than edited in: the plugin **can** read `SetBy`, in exactly one place — `Handlers/Render/PreviewViewportCaptureUtils.cpp:1397-1398` calls `::GetConsoleVariableSetByName(CVar->GetFlags() & ECVF_SetByMask)` and publishes it as `setBy` at `:2803` — scoped to the forced-show-flag survey inside capture output. The general gap is real and is `F-console-search-setby-priority`'s.
- `#4-cross-link-the-typed-verb-door` `IN-REVIEW` reporter — **Cross-link only. Status unchanged, and `encounters` / `lastSeen` deliberately left untouched** — this is not a re-observation of the `sg.*` defect. A **third** door to the same pin is now filed as `B-performance-typed-verbs-pin-scalability-cvars` (OPEN, High), and it is the one that bears hardest on the guard `#2` shipped, because **it involves no console string at all**. `performance.configure_texture_streaming` (`Handlers/Debug/PerformanceHandler.cpp:558`, write `:572`) and `performance.apply_baseline_settings` (`:939`, `SetCVar` lambda `:949-956`, write `:953`) call a bare `CVar->Set(x)`, and the templated helper at `IConsoleManager.h:766` defaults its flags to `ECVF_SetByCode = 0x0E000000` (`:183`) — one notch below the `ECVF_SetByConsole` this ticket covers, and still six levels above the `ECVF_SetByScalability = 0x02000000` (`:159`) the panel writes at, so the same `CanChange` veto applies. Seven `ECVF_Scalability` cvars are written across the two verbs, re-derived from the cvar declarations rather than from `BaseScalability.ini`. **Why it bears on `#2` specifically:** the refusal text `#2` ships steers the caller *"Use `performance.set_scalability` instead"* — off the console and onto typed verbs — and two typed verbs in that same namespace inflict the pin themselves, so the guard's own advice routes a well-behaved caller past the guard and into the defect by another door. `IsScalabilityGroupLine` cannot be widened to cover it: `PerformanceHandler.cpp` does not include `ScalabilityConsoleGuard.h` (zero hits) and there is no command line to test. Three doors, three files, one defect — all now cross-linked in both directions, so nobody closes this ticket believing the pin is gone.
