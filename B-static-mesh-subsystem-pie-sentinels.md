...
- `#3-verified-in-fps-build` `IN-REVIEW` reporter - **Fixed as announced, NOT as corrected - and the distinction should be recorded before anyone closes this.** Verified live in the FPS build, 2026-09-05 17:54Z, during this stream's own standalone PIE session on `/Game/FPS/Test/T_Weapons`.
  The values are unchanged. Through `python.execute` with PIE active, `StaticMeshEditorSubsystem.get_lod_count` still returns **-1** and `get_num_uv_channels` still returns **0**, for `/Game/FPS/Weapons/Meshes/SM_WPN_AR` and for the control `/Engine/BasicShapes/Cube` alike.
  What changed is that the call is no longer silent. Every `python.execute` response during PIE now carries a `Warning` log entry: *"PIE is active. Unreal editor scripting calls can return failure sentinels without failing this script; StaticMeshEditorSubsystem.get_lod_count returns -1 and get_num_uv_channels returns 0 in this state. Do not treat those values as measurements; use a typed PinWright verb or retry after PIE ends."* It names both functions and both sentinel values, and it fired on all four `python.execute` calls made during the session, including ones that never touched the subsystem.
  That addresses the ticket as titled - sentinel values dressed up as measurements with `success:true` and an empty error log - because the log is no longer empty and the caller is told which numbers to distrust. It does not make the numbers right. A caller who reads `lod_count` programmatically and does not parse the warning text still gets -1. If the intended fix was to return real values under PIE, this is not that fix; if it was to stop the silent-wrong-data class, it is. Worth the tester deciding which before closing.
