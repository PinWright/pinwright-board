...
## History
- `#2-plumb-struct-default` `IN-REVIEW` developer — Root-cause fix: plumbed the archetype/CDO default container through `FIrTextUtils::FormatReflectedPropertyValue` into the struct-export fallback so nested sub-fields suppress against their real per-field defaults, not the property type's zero-value. Added a `const void* Default` parameter (after `Container`, mirroring `AppendReflectedFields(Instance, Default, ...)` and the engine's `ExportTextItem_InContainer(Container, DefaultValue, ...)`); the struct fallback at `IrTextUtils.cpp` now passes `Default` instead of the literal `nullptr`, and `AppendReflectedFields` forwards the `Default` (CDO) it already holds for its own top-level `Identical` suppression. This aligns the inner-field suppression with the top-level CDO-based suppression the two layers previously disagreed on: a non-default bool sub-field set to `false` (its zero-value, e.g. PCG `bRandomizedPruning` whose real default is `true`) is now emitted, and a sub-field left at a non-zero default (e.g. selectors / `RadiusSimilarityFactor`) is no longer spuriously emitted. Verified via engine source that `UScriptStruct::ExportText` (Class.cpp:3567+) threads `Defaults` per sub-field and `FProperty::ExportText_Direct` (Property.cpp) gates emit on `Data==Delta || !Identical(Data,Delta)`, so a valid CDO default flips both halves of the inversion; no empty `()` risk since `AppendReflectedFields` already skips whole structs Identical to the CDO. Fixes all six IR decompilers routing through the shared helper (PCGIR/MGIR/AGIR/NIR/SCIR/BTIR). Files: `Source/PinWright/Public/IrCore/IrTextUtils.h`, `Source/PinWright/Private/IrCore/IrTextUtils.cpp`; test fixture in `Source/PinWright/Private/Tests/Core/TestIrTextUtilsReflectedPropertyEmit.h`, updated existing direct-call site there. Regression test `PinWright.core.ir_text.reflected_property.StructSubfieldDefault` (`FIrTextUtilsReflectedStructSubfieldDefaultTest` in `TestIrTextUtilsReflectedPropertyEmit.cpp`): builds an in-code USTRUCT (`bDefaultTrueFlag=true`, `DefaultedInt=5`) on a transient host, sets the bool to `false`, and asserts `AppendReflectedFields` emits `bDefaultTrueFlag=False` while suppressing the defaulted `DefaultedInt`, plus a positive control that an overridden `DefaultedInt=9` still emits — all four assertions flip if the `nullptr`-default is restored. No asset load; no Lyra content.
