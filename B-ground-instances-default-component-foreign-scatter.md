...
- `#3-the-notes-overlay-still-documents-largest-wins` `IN-REVIEW` reporter — **Status deliberately NOT changed, `encounters` deliberately NOT bumped, no code touched.** `#2` removed largest-wins from the resolver and rewrote the `component` parameter text, but did not sweep the hand-written Notes block in the wiki overlay, so the shipped page now carries both the fix and the defect it removed. Re-derived at plugin HEAD `1a9e5778`. **The code is right:** `InstancedMeshUtils::ResolveInstancedComponent` refuses at `Handlers/Actor/InstancedMeshUtils.h:162` (`if (ComponentName.IsEmpty() && Resolution.Available.Num() > 1)`) with `ErrorCodes::ERR_AMBIGUOUS_INSTANCED_COMPONENT` at `:174`, and the selection at `:188` is now `ComponentName.IsEmpty() ? Sole : Named` — `Sole`, assigned at `:134`, not the `Largest` this ticket's `#1` cites at the old `:100-104` / `:127`. There is no largest-wins path left. Asserted by `Tests/Spatial/TestGroundPlacement.cpp:2570-2571`, `:2644`, `:2655`. **The prose has not moved.** `Docs/wiki-src/spatial.md:305`, the Notes block's `component` bullet, still reads: *"`component` (string, optional; aliases `componentName` / `component_name`) - object name of the component, case-insensitive. **Omit to take the component with the MOST instances.** **Resolve it by static mesh plus instance count immediately before writing:** component object names are not stable across a session [...]"*. That reaches `Saved/PinWright/wiki/spatial.ground_instances.md:40`, twenty-seven lines below the parameter-table entry at `:13` which says the opposite in as many words — *"Omit ONLY when the actor carries exactly one; with several the call is refused with AMBIGUOUS_INSTANCED_COMPONENT [...] There is no largest-wins default"*. One generated page, both claims, and the Notes half is the half a caller reads when they want the argument list in prose. Worse, the surviving advice is now unreachable rather than merely wrong: "resolve it by static mesh plus instance count immediately before writing" is a mitigation for a default that no longer exists, and a caller who follows it instead of naming the component gets the refusal. **The same Notes block is stale on a second, independent axis:** it predates `contactRadius`. `Docs/wiki-src/spatial.md` contains **zero** occurrences of `contactRadius` — its seat-solve bullet at `:308` lists `samples` / `footprintInset` / `seatPercentile` / `embedFraction` / `embedDepth` and stops — while the parameter table documents the parameter at length (generated `spatial.ground_instances.md:19`), including the measured 46.7x canopy-to-contact ratio and the 177/177 wrongly-lifted instances that motivated it. A caller reading the Notes for the argument list does not learn the parameter exists. **Correction to the report this entry came from, recorded rather than propagated:** the Notes do *not* say the caller must compute the seat themselves — no such sentence is on the page. What they say is the two clauses quoted above, and only the first is a contradiction; the second is orphaned advice. Stated so a fixer does not go hunting for a sentence that is not there. **Ask:** rewrite `Docs/wiki-src/spatial.md:305` to match the parameter text `#2` already wrote, drop the stale-name mitigation or re-point it at `expectedCount` (which `#2` shipped for exactly that case), and add `contactRadius` to the seat-solve bullet at `:308`. This is the documentation half of `#2` — that entry's own note that *"the `apply` parameter text now says so"* shows doc updates were in its scope — so it is recorded here rather than filed, and it is a fix-completeness gap for the tester to close with the rest of `#2`, **not** grounds to return this ticket to `OPEN`: nothing about the refusal, `expectedCount` or the pre-flight identity is contradicted by the measurement. `encounters` is left at 3 because the defect was not observed again — the refusal is in place and the omitted-component seat is unreachable — and this entry follows the precedent of `B-pcg-connect-pins-silently-replaces-edge` `#3` and `B-foliage-paint-ignores-align-to-normal-and-random-yaw` `#3`, both of which append a reporter finding to an `IN-REVIEW` ticket without a status or `encounters` change.
