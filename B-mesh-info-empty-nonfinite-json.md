...
- `#2-fix` `IN-REVIEW` developer — Implemented both fix layers. (1) Systemic safety net: `JsonBuilders::BuildVectorJson` (`Source/PinWright/Private/Utils/JsonBuilders.h`) now routes each component through a new `SanitizeFinite(double)` helper that collapses NaN/±Inf → 0, so no handler serializing a degenerate FVector/FBox can ever emit a bare `inf`/`nan` token (mirrors the existing `NdjsonSessionWriter::FormatNumber` guard). (2) Targeted semantic fix: `geometry.get_mesh_info` (`Source/PinWright/Private/Handlers/Geometry/MeshInfoHandler.cpp`) now detects the empty/degenerate box (`VertexCount == 0 || MeshBounds.IsValid == 0`) and reports a zeroed `boundingBox {min,max,origin,extent}` instead of the inverted ±DBL_MAX sentinel (whose GetExtent() overflowed to -inf). Regression test added: `PinWright.geometry.get_mesh_info.EmptyMeshBoundingBoxIsFiniteJson` in `Source/PinWright/Private/Tests/World/TestGeometryHandlers.cpp` — spawns a real 0-vertex mesh via the production `create_procedural_mesh` handler, calls production `get_mesh_info`, and asserts vertexCount==0, every bbox component is finite AND zeroed, and the full response serializes to valid round-trippable JSON. Reverting either layer fails the finite/zero/round-trip assertions. Not compiled/tested here — a later phase drives it green.
