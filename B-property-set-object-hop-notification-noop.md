...
- `#3-verified-by-downstream-behaviour-on-the-1332-build` `DONE` verifier — 2026-08-30, **13:32 build** (plugin `d8f1bc32`; `#2`'s fix itself landed one wave earlier in `2ba21649`), editor pid 18592, `/Game/Maps/PW_VegetationTest`. **Verified the way `#1` framed the defect: by what happened downstream, never by a read-back.** That constraint is the whole point of this ticket — `#1` established that `property.get` walks the same resolver to the same inner memory and reports the write faithfully whether or not anything was notified, so a read-back cannot distinguish fixed from broken and was deliberately not used as evidence here.

  **Repro, built from scratch in this session so nothing was inherited.** New graph `/Game/PinWrightScratch/PWScratch_HopVerify1332` with `PCGGetLandscapeSettings` → `PCGSurfaceSamplerSettings.Surface` and the sampler's `Out` wired to the graph Output node (so `pointCount` is reported at all — an unwired Out gives `graphOutputAvailable: false`, which is a different failure and was hit once on the way). A `PCGVolume` at `(-18000, -17600, 300)`, `volume.set_volume_extent` to a 500 half-extent, i.e. 10 m x 10 m of landscape. Then, all through `property.set` rooted at the **node** with the dotted path `SettingsInterface.PointsPerSquaredMeter` — the `FObjectProperty` hop this ticket is about — with `pcg.generate {cleanupFirst: true}` after each write:

  | write | `pointCount` |
  |---|---|
  | baseline, property untouched | **9** |
  | `SettingsInterface.PointsPerSquaredMeter = 1` | **100** |
  | `SettingsInterface.PointsPerSquaredMeter = 0.1` | **9** |

  Bidirectional, deterministic, and returning to exactly the baseline. `#1`'s measurement was the same shape and did not move at all: `LowerBound = 0.9` through the hop left `pcg.generate` at **26664**, unchanged, while the same value written directly against the inner object gave **2093**. The engine chain `#1` traced is what makes this a valid instrument — `UPCGSettings::PostEditChangeProperty` (`PCGSettings.cpp:719`) is the only thing that broadcasts `OnSettingsChangedDelegate` (`:744`), and without it the graph is never dirtied and a regenerate reuses its cached result. A point count that moves 9 → 100 → 9 is that delegate firing on the object `#1` proved was never notified.

  **The two honest-looking signals `#1` indicts were observed again and are still not evidence.** `applied: true` and `markedDirty: true` came back on all three writes, exactly as they did on the broken build. They are reported here only so a reader can see that nothing about the response distinguishes the two builds — the point count does.

  **`#2`'s claim that struct hops did not move was not re-tested here** and is not part of this close; it rests on `#2`'s `PinWright.property.set.StructHopStillNotifiesTheRootObject` guard and on `B-property-set-container-empty-change-event` `#7`'s pixel measurement. Also not re-tested: the array-element hop (`InnerArray[0].Source`) and the `container.*` verbs, both of which `#2` says are fixed by the same retarget and both of which are covered only by its unrun tests.

  **Waves 2 and 3 did not regress the retarget, checked rather than assumed** — the concern that prompted this re-verification. Diffing `2ba21649` against HEAD `1a9e5778`: `Utils/PropertyInspection.cpp`, which holds the whole walk (`LastCrossedObject`, `FillNotifyTarget`, both hop sites), is **unchanged**; `PropertyInspection.h`'s only delta is a `PINWRIGHT_API` on `ResolvePropertyOnObject` so the gated PCG module can link it. `UtilityPropertyHandler.cpp` changed by 71 lines, none of them the retarget: `NotifyReflectedPropertyChanged` still resolves the notify target and sends `PinWright::NotifyPropertyChanged` **and** `PushRenderStateForComponentTarget` to `NotifiedObject`, and the wave-2/3 addition is one further call on that same object (`RefreshGrassConsumersForNotifiedTarget`, for `B-grass-varieties-edit-does-not-reach-renderer`) plus an optional out-param defaulted to null. Additive, on the corrected target.

  **Housekeeping:** the scratch graph and volume were created under `/Game/PinWrightScratch/` and never written to disk (`existsOnDisk: false` throughout). The volume actor was deleted; `asset.delete` on the graph was correctly refused `ASSET_IN_USE` because the transaction buffer still holds the deleted actor's `PCGGraphInstance`. `force:true` was NOT used — the package is not on disk and the reference clears on editor restart. `F-pcg-set-node-property`, whose implementation `#2` says this fix satisfies generically, is deliberately untouched.
