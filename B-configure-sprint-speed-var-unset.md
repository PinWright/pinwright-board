...
## History
- `#2-triage` `IN-REVIEW` developer — GO (severity Medium unchanged). Verified against synced source: `configure_sprint` (`CharacterHandler.cpp:1244`) creates the `SprintSpeed` float var but writes the speed only to the CDO's `MaxCustomMovementSpeed` (`:1252`) — no `SetBPVarDefaultValue`, and the shared `AddBlueprintVariableChar` helper (`:247`) seeds no default — so the var keeps its float-zero default. Siblings `add_custom_movement_mode` (`:707-708`) and `configure_footstep_fx` (`:879-880`) already persist their value-var defaults; `configure_sprint` alone omitted it. SHIPPED: added `SetBPVarDefaultValue(Blueprint, TEXT("SprintSpeed"), FString::SanitizeFloat(static_cast<float>(SprintSpeed)))` right after the var creation in `CharacterHandler.cpp` (mirrors the footstep sibling). Plugin compiled clean (Result: Succeeded). Adopted the reporter's red test `PinWright.character.configure_sprint.SprintSpeedDefaultPersisted` (`Tests/Gameplay/TestConfigureSprintSpeedDefault.cpp`) as the regression gate — it was observed FAILING pre-fix and now runs `Result={Success}` (red→green differential). Ready for the test phase.
