...
- `#7-exposure-pin-lands-aiming-still-does-not` `IN-REVIEW` AI-stream - 2026-09-05, FPS AI build 07. Half of this is now fixed and the other half still bites, so recording both. **Works:** `editor.screenshot {width:1600, height:900, exposure:{mode:'fixed', ev100:1}}` on the PIE path returns `exposure:{pinRequested:true, pinned:true, restored:true, viewCount:1, ev100:1}` and writes a clean 1600x900 frame with **no editor chrome** and correct, repeatable exposure - which is what `#2` asked for and what `screenshot_window` could never give (it always carried the whole editor window). **Still blocks the use case:** the frame contained no subject, twice, because `captureSource` is `gameViewport` while `editor.set_camera` aims the *level editor* viewport - the residue this ticket already names at line 58. After `editor.eject` the game viewport keeps its own view target, so a caller can pin exposure OR aim the shot, not both in one call. Concretely: I froze the world at `set_global_time_dilation(0.0001)` so the subject could not move, computed a camera pose from the pawn's own transform, called `editor.set_camera`, then `editor.screenshot` - and got the same empty arena both times, while the identical pose through `editor.screenshot_window` frames the subject correctly (that is how `b05_muzzle.png` and `b05_crouch.png` were taken). So the two verbs disagree about which camera 'the viewport' means. What would close it for capture work: let `editor.screenshot` aim the game viewport (a `location`/`rotation` or `viewTarget` argument), or have `editor.set_camera` accept a `world:'game'` selector the way `editor.console_command` already does. Either one makes 'fixed-exposure frame of a named actor in PIE' a single call instead of a choice between exposure and framing. Not reopening: the exposure work landed and is good; this is the remaining half of the original report.
