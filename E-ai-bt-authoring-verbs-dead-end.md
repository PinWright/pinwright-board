...
- `#3-additional-docs-only-cross-namespace-tax` `OPEN` reporter — Additional-evidence (encounters 1->2), NEW ANGLE: the friction bites even an agent that NEVER touches the dead surface. Focus=`ai.add_task_node`, sentry-brain task (build BT_Sentry `Root->Sequence->{MoveTo,Wait}` + BB_Sentry + AIC_Sentry under /Game/AI). Trace was functionally clean — 22 RPCs, zero errors, all four success-check readbacks passed, self-report "Smooth." The agent read the `ai.add_composite_node` and `ai.add_task_node` wiki pages, saw they list only `{behaviorTreePath, taskType/compositeType}` with **no return node id, no parent/target param, no ordering param**, and could not confirm from those pages that `add_composite_node(Sequence)` then `add_task_node(MoveTo)` then `add_task_node(Wait)` would actually parent both tasks under the Sequence in authored order (this goal needed MoveTo before Wait). Its SAY rationale: "Both an ai.* path and a more explicit behavior_tree.* path exist. Since I need explicit control over wiring (Root -> Sequence -> {MoveTo, Wait}) and node IDs to connect, the behavior_tree.* graph path is the reliable choice." It then read the entire parallel `behavior_tree.*` namespace (`behavior_tree.md`, `add_node`, `connect_nodes`, `create`) — ~4 EXTRA cross-namespace discovery reads — to find a path it could trust, and never called `ai.add_task_node`/`ai.add_composite_node` at all. So even when the dead end is AVOIDED (no wasted execute calls this time, unlike #1), the missing upfront steer/cross-link in `docs/wiki-src/ai.md` still imposes a pure cross-namespace-discovery tax. This directly VALIDATES the IN-REVIEW fix (#2): the docs steer + a one-line cross-ref from the `ai.add_task_node`/`add_composite_node` pages to `behavior_tree.add_node`/`connect_nodes` would have short-circuited those ~4 reads. Judge filed nothing (outcome=clean); this is the distinct PROCESS angle. CallAnalyzer proposed a standalone low-severity docs finding ("pages omit node attach/order semantics and don't cross-link to behavior_tree.*") — folded here as it is a subset of this ticket's options 1 & 3.
