...
- `#2-blind-to-pixels-too-and-a-refuted-frame-claim` `OPEN` reporter — Additional evidence, **not a status change** and not a verification: the same blindness was hit from the other side, on frame CONTENT rather than stream layout. Turntable render 2026-09-02, job `TT_Column_v4_full`, response spilled at `Saved/PinWright/HttpResponses/20260902T092641Z/20260902T174233Z_094ab003-43fd-d5ff-b693-97be6bb50c57.json`: `success:true`, `jobSucceeded:true`, `outputFileCount:241`, `measuredFileCount:241`, `frameCount:241`, `frameRate:30`, `durationSeconds:8.0333`, `resolution 960x1080`, `totalFileSizeBytes:256235538`, `overallBitrateBps:255172320`, `bitsPerPixel:8.204`, every one of the 241 `outputFiles` `exists:true`. A rendering agent suspected the final frame was rendered short of its temporal samples and the report had **nothing to say either way** — `BuildArtifactReport` does exactly one filesystem op per path (`GetStatData`, `MRQArtifactReport.cpp:144-146`, where `exists` is literally `Stat.bIsValid && !Stat.bIsDirectory && Stat.FileSize >= 0`), and a tree-wide grep for `ffprobe`, `ffmpeg`, `luminance`, `Decode`, `ReadPixels`, `histogram` across `Source/` returns zero hits. `frameCount` comes from the pipeline's own `WorkMetrics.TotalOutputFrameCount` (`MRQHandler.cpp:71-74`), never from the container, so the bitrate is computed against the pipeline's INTENDED count and is self-consistent whatever landed. **The suspicion was then measured and is REFUTED — recorded here so nobody re-files it.** Decoding all 241 PNGs under `Saved/MovieRenders/Video/src/turntable_v4/` with a stdlib decoder (960x1080, 8-bit RGBA, alpha 255 throughout): frame 0240's whole-frame mean luma is 192.88 against 192.13 on 0239 — on the rising tail trend, not an outlier; its step from 0239 is 9.38 mean-abs against a 11.43 median for the nine preceding tail pairs, i.e. 0.82x, BELOW trend where under-accumulation would be above it; and its high-frequency content (mean |p(x)-p(x+1)|) is 0.6949 against 0.8226 on 0239 and below all 25 other frames sampled (0.7577-0.9154) — 15.5% SMOOTHER, the opposite direction from a sample-starved frame. The relayed region numbers ('plinth 135.0 vs 147.5, floor shadow 170.3 vs 130.7') are a rotation artefact plus loop closure: 241 frames is 240 steps of a full revolution, so frame 0240 re-occupies frame 0000's pose (mean abs difference 0.464 against 9.38 for any adjacent pair), and fixed rectangles therefore jump relative to 0239 with no rendering fault — swings of +-40 to +68 per cell occur in every adjacent tail pair. One genuine residual, cause NOT determined: against its identical-pose twin 0000, frame 0240 is ~15% smoother confined to the bottom third (hf ratio 0.60-0.65 there against ~1.00 in the top third) with cell means preserved to +-1.3 — over-smoothed floor, not missing content; it does not reproduce on `turntable_v3`, which has no wrap-around 241st frame. **What this ticket's defect cost here:** answering a one-line question about the plugin's own deliverable required hand-decoding 241 PNGs outside the tool, because the verb that produced them publishes no pixel measurement in either direction — while the capture verbs publish `imageStats`, `blank`, `litPixelFraction` and `subjectRegion` on every shot. Severity and status deliberately UNCHANGED: this widens the ticket's scope statement from container streams to file content generally and adds no new failure mode. Adjacent gap filed the same day: `B-mrq-shared-queue-no-management-verbs`.
