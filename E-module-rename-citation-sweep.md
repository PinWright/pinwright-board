...
more had no engine match, and 474 carry no line number at all** — every one scored neither pass
nor fail.
...
**33 tickets carry a claim that no longer holds at HEAD**, in two kinds. **14 are actionable** —
their `status:` is contradicted by the code: six sit at `IN-REVIEW` describing a defect already
fixed in source (`B-blocking-volume-no-brush-geometry`,
`B-create-datalayer-transient-asset-not-persistable`, `B-sequencer-create-dangling-else-hangs`,
`E-describe-sound-wave-omits-compression`, `B-sequence-add-keyframe-location-property-rejected`,
`F-no-response-handler-guardrail`), and eight sit at `DONE` on something that is not true
(`B-editor-save-all-no-completion-signal`, `B-pipeline-run-ubt-no-completion-signal`,
`B-level-build-all-no-completion-signal`, `B-system-run-tests-no-completion-signal`,
`B-system-run-ubt-no-completion-signal`, `F-job-control-rpcs`, `F-pcg-core-graph`,
`E-dump-rpc-parity`). The other **19 are ordinary decay** — a body sentence made false by the
ticket's own landed fix, worth knowing before quoting but needing no transition. Each is recorded
on its own ticket's repoint row; **no status was changed by this sweep**. The ones that matter
most:
...
- `#3-correct-the-broken-claim-count` `DONE` reporter — Second correction to `#1`, same discipline: "28 tickets carry a claim that no longer holds" was an undercount and, worse, it merged two different things. Re-counted from the per-ticket rows: **33**, of which **14 are actionable because the ticket’s `status:` is contradicted by HEAD** — six `IN-REVIEW` on defects already fixed in source, eight `DONE` on something untrue — and **19 are ordinary decay**, a body sentence falsified by the ticket’s own landed fix, worth knowing before quoting but needing no transition. Both lists are now in the body. Still no status changed by this sweep; the 14 want a triage pass that is not citation maintenance.
- `#4-reconcile-the-token-count` `DONE` reporter — Third and last correction to `#1`, and the reason is worth keeping: `#1` splits the 515 as "(495) and (24)", which sums to 519, not 515. Both numbers were right about different things. A raw `grep -o Source/EditorAutomationRpcGateway` returns **519 occurrences**; the sweep’s tokeniser counted **515 citations**, requiring a path to follow, and the four-token gap is bare mentions of the module root with no path after it. The correct per-root split of the 515 is **491 under `Source/EditorAutomationRpcGateway/` and 24 under `Source/EditorAutomationRpcGatewayTests/`**. The same 519-vs-515 base also made `#1`’s "478 carry no line number" four too many — it is **474** (515 − 41), and the body is corrected. Every other figure in `#1` is on the 515 base and stands.
