...
- `#3-added-niagara-audit-level` `IN-REVIEW` developer — "Added niagara.audit_level in a new Handlers/Niagara/NiagaraAuditHandler.cpp. Read-only level-scope sweep: TObjectIterator<UNiagaraComponent> (the KillSystemInstances walk, never FindBoundObjects), skipping templates, deduplicated to distinct UNiagaraSystem assets so CheckDataInterfaceCounts runs once per SYSTEM and each row carries the components and owning actors that hold it. scope:'world' (default, the editor world) / 'all' (adds preview scenes and worldless components); detail:'summary'|'findings'|'orphans', the last reusing FindOrphanResolvedDataInterfaces. No new measurement code. Verdict via PinWrightAudit::FVerdict::DerivePass with one check-table row (data_interface_counts): mismatched is an Error finding, unverified is an UNRUNNABLE row that failOn cannot reach, consistent is clean, and the three buckets sum to systems. mismatchedScripts[] and orphans[] reuse the existing per-script/per-orphan spelling from NiagaraEdit::MakeMutationResult and niagara.list_orphan_data_interfaces. Registered ERR_NIAGARA_DATA_INTERFACE_UNVERIFIED in Handlers/ErrorCodes.h (the code niagara.validate already emits raw; that file stays non-adopting). Regression test PinWright.niagara.audit_level.LevelScopeDataInterfaceSweep in Tests/Niagara/TestNiagaraDataInterfaceConsistency.cpp seeds three transient systems — consistent, genuinely mismatched, and unverifiable — each held by an UNREGISTERED worldless component so nothing can tick them, and asserts the flagged row with both counts, the clean row with no finding, the unrunnable row for the unverifiable system, that failOn:'none' still cannot pass it, and that bad scope/detail/failOn/checks are RPC errors rather than findings. Not compiled or run here."
