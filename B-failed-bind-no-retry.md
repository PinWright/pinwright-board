...
- `#5-probe-read-the-wrong-select-set` `IN-REVIEW` implementer — **Correcting entry #4's "Runtime: NOT VERIFIED": the tests were run, and they caught a real defect in the fix itself.** Filtered run `Saved/PinWright/test-runs/pw_wave_bindquit/automation.log` (11 tests performed, drained): 3 of the 5 new `port_advertisement` tests passed, **2 failed** — `ProbeSeparatesAListenerFromAnEmptyPort` (`Expected 'the same port reads as dead once the listener is gone' to be false`, TestPortAdvertisement.cpp:168) and `RetractsAnAdvertisementNothingIsServing` with it. Root cause: `IsSomethingListening` waited on `FSocket::Wait(ESocketWaitConditions::WaitForWrite)`, which maps onto select()'s WRITE set only, while a **refused** connect is reported on Windows through the **EXCEPT** set. A dead port therefore never became writable, the probe burned its full budget and returned the inconclusive answer — and inconclusive means *listening* by design, so `Judge` could only ever return `Retain`. **The retraction was unreachable and a stale advertisement survived exactly as it did before any of this existed**: the whole ticket would have been closed on a fix that did nothing, and the diagnosis is invisible by reading — `RetainsAnAdvertisementSomethingIsServing` passed throughout, because detecting a LIVE listener worked fine. Fixed at `59d4fee1` by polling `GetConnectionState()`, which reads the error state and so separates all three outcomes: `SCS_Connected` → listening, `SCS_ConnectionError` → refused (the only outcome that retracts), still pending at the 0.25 s budget → inconclusive → listening. Compile-checked clean (`-SingleFile`, real `[1/1] Compile [x64] PortAdvertisement.cpp`, `Result: Succeeded`). **Verification status, stated precisely: the probe fix is committed but NOT yet re-run.** The re-run needs a link, the link needs this project's only editor gone, and that editor is currently serving another agent (`editor.quit` correctly refused with `EDITOR_IN_USE`, `python.execute` 10 s prior, and it was not forced). So 3 of 5 tests are green on measured evidence, and 2 are green only in expectation until the re-run happens. **This is exactly the pair-of-directions argument in the entry above earning itself out:** `RetainsAn...` alone would have reported this broken implementation as a pass.
