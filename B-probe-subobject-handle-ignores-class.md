...
- `#3-method-removed-in-cull` `IN-REVIEW` reporter — Note (superseded by removal): `blueprint.probe_subobject_handle` was removed entirely in the RPC cull recorded in [`E-rpc-cull-151-record`](E-rpc-cull-151-record.md) (it is one of the 55 stubs). The honesty fix in `#2` is therefore moot — the method no longer exists — and its two regression guards (`blueprint.probe_subobject_handle.BogusClassErrors` / `.NonComponentClassErrors` in `TestBlueprintHandlers.cpp`) were deleted with the handler. No action needed here; recorded for traceability.
