...
- `#2-capture-discloses-game-view-off` `IN-REVIEW` developer — `MakeViewportInfoObject` (`Handlers/Render/PreviewViewportCaptureUtils.cpp`) now emits `viewport.gameViewWarning` whenever the capture measured `gameView: false` on a viewport with no preview scene, i.e. on the level-editor family (`render.capture_open_level`, `camera.*`, the level path of `render.capture_annotated`, `editor.screenshot`'s viewport fallback) — every capture verb routes its response through that one builder, so none of the handlers needed touching. The warning names what game view covers that `editorSprites` does not (world-axis gizmo, component visualizers, grid/selection passes), states that game view is per-viewport state this capture neither sets nor owns and that a concurrent agent can move it after a confirmed `set_game_view`, and tells the caller to re-assert it immediately before the shot and read the field off THIS response. Deliberately a warning and not a refusal, and deliberately NOT emitted on an asset-preview viewport: a preview scene is normally not in game view, carries none of that chrome, and `editor.set_game_view` cannot move it, so warning there would fire on correct output with a remedy that does not apply. No `requireGameView` parameter was added — a shared-editor condition the caller may knowingly accept must not fail the capture. Test `PinWright.render.capture_contamination.GameViewOffOnALevelCaptureCarriesAWarning` (`Tests/Render/TestCaptureContamination.cpp`) asserts the warning is present on a level capture with game view off, names `editor.set_game_view`, and is absent both when game view is on and on a preview-scene capture; all three run as pure functions over `FViewportCaptureOutput`, with no GPU and no skip path.
- `#3-verified-fixed-behaviourally` `DONE` verifier — 2026-08-28. Re-ran the ticket's own repro against the rebuilt binary at HEAD `b79ba53e` on the live editor (`/Game/Maps/Atlantis`, port 27145), as a differential pair at one fixed pose (`location {x:-9000,y:0,z:260}`, `rotation {pitch:-4,yaw:0,roll:0}`, `fov 60`, 512x512, `hideEditorSprites:true`, pinned `exposure {mode:"fixed", ev100:0}`, `r.ScreenPercentage 100`), with only `editor.set_game_view` differing between the two shots. **Game view OFF** — `render.capture_open_level` returned `viewport.gameView:false` AND `viewport.gameViewWarning`, which names the world-axis gizmo, component visualizers and the grid/selection passes, states that `editorSprites` does not cover them, that game view is per-viewport state this capture neither sets nor owns and that a concurrent agent can move it after a confirmed `set_game_view`, and tells the caller to re-assert it and read the field off THIS response. **Game view ON** — identical call → `viewport.gameView:true` and NO `gameViewWarning`. The warning is differential, not unconditional. **Vision-verified rather than taken off the JSON:** `Saved/Screenshots/OpenLevel/verify_gv_off.png` carries the world-axis gizmo drawn bottom-left over the seafloor and `verify_gv_on.png` at the identical pose does not — so the field that now warns is the field that actually invalidated the frame — and the luminance pair moved 0.342858 → 0.351488, matching the original encounter's 0.331572 → 0.343396. `editorSprites` reported `{hideRequested:true, visible:false, restored:true}` on BOTH, reproducing the ticket's point that the sprite block reads clean over a contaminated frame. Scope: confirmed on three verbs spanning all three families that share the `MakeViewportInfoObject` builder — `render.capture_open_level`, `render.capture_annotated` (`Saved/Screenshots/AnnotatedCapture/verify_annot_gv_off_annotated.png`) and `camera.frame_actor` (`Saved/Screenshots/CameraFrame/CameraFrame_20260828_091536_076_0003.png`) — each returning `viewport.gameView:false` alongside the byte-identical `gameViewWarning` text, so the warning is emitted by the shared response builder and not by one handler. The requested `requireGameView: true` parameter was deliberately NOT implemented (developer entry `#2`: a shared-editor condition the caller may knowingly accept must not fail the capture); closing on the ticket's primary ask, that the invalidating measurement stop being silent.
