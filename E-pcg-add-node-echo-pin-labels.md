...
- `#8-additional-input-pin-existence-drives-class-selection` `OPEN` reporter — Recurrence + NEW angle (8th observation, realism-mode "RockScatter" scatter build, namespace `pcg`, outcome clean/ergo). Same forced read-back — `create_graph → add_node(SurfaceSampler_0) → add_slope_filter → add_noise_filter → pcg.inspect("read node ids + pin labels") → connect_pins ×4 → save → inspect → decompile` — one mid-build `pcg.inspect` spent purely to recover the pin labels the create responses omitted, before all four `connect_pins` matched first try. NEW ANGLE: the missing pin echo forces source-diving to discover input-pin EXISTENCE for CLASS SELECTION, not just to recover label strings for wiring. To pick a point generator that can receive the graph's required incoming edge from `DefaultInputNode`, the agent read UE engine C++ (`PCGCreatePoints.cpp`, `PCGSurfaceSampler.cpp`) to learn that `PCGCreatePointsSettings` has no spatial data-input pin while `PCGSurfaceSamplerSettings` exposes a `Surface` input — a decision `add_node`'s `{nodeId,nodeClass,x,y}` response (and the wiki) give no way to make up front. Replay-verified at HEAD: `pcg.add_node` still returns only `{nodeId,nodeClass,x,y}` (no `inputPins`/`outputPins` — the #6 echo fix is reverted per #7); a scratch-graph `pcg.inspect` confirmed `PCGCreatePointsSettings` inputPins = `[Execution Dependency, Overrides, CoordinateSpace, bCullPointsOutsideVolume]` (exec/override pins only — no spatial data input, so it cannot head a chain fed from graph Input) vs `PCGSurfaceSamplerSettings` inputPins including `Surface`/`Bounding Shape`. The pins ARE recoverable via `pcg.inspect` (the documented workaround); the echo fix would surface them directly in `add_node`'s response, removing both the inspect round-trip AND the class-selection source-dive. Same fix (echo `inputPins`/`outputPins` from the create verbs); severity stays Low.
