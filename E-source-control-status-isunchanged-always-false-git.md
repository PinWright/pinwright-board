...
- `#2-fix` `IN-REVIEW` developer — Verified the defect at HEAD (`SourceControlHandler.cpp:252` folds `!IsCheckedOut()`; engine arbiter `GitSourceControlState.cpp:190-193` returns `IsCheckedOut()==IsSourceControlled()`, `:205-207` true for any tracked file, so every clean tracked git file → `isUnchanged:false`). Fix: extracted the per-file flag derivation into a shared production helper `PinWright::SourceControl::WriteStateFlags(const ISourceControlState&, FJsonObject&)` at `Plugins/PinWright/Source/PinWright/Private/Handlers/SourceControl/SourceControlStateFlags.h`, and changed the `isUnchanged` composite to `IsSourceControlled() && !IsModified() && !IsAdded() && !IsDeleted()` (drops `!IsCheckedOut()`; adds `IsSourceControlled()` so a not-under-control file is not reported "unchanged"). `SourceControlHandler.cpp` now calls the helper instead of the inline block. Deliberate cross-provider note: a Perforce checked-out-but-unmodified file now reports `isUnchanged:true` (content-clean, consistent with the engine's RevertUnchangedFiles semantics). Files: `SourceControlStateFlags.h` (new), `SourceControlHandler.cpp`, `docs/wiki-src/source_control.md` (clean-state semantics note). Regression test `PinWright.source_control.StatusCleanGitFileReportsUnchanged` in `Tests/SourceControl/TestSourceControlHandlers.cpp` drives `WriteStateFlags` with an in-code `ISourceControlState` double for the clean-git (checked-out+pristine → `isUnchanged:true`), modified, added, and not-controlled cases; it fails if the `!IsCheckedOut()` term is restored.
