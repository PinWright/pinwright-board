...
- **Still `UMaterial`-only at HEAD after the `#2` fix** (the "convenience adds
  left on the material-only path" `#2` calls out): `material.authoring.add_material_node`
  and `material.authoring.add_scalar_parameter` — the generic node-add and
  parameter-add verbs still reject a valid `UMaterialFunction` /
  `UMaterialFunctionMaterialLayerBlend` path with `[ASSET_NOT_FOUND] Could not
  load Material.`, while the widened `material.graph.add_expression` accepts the
  same function (see history `#5`).
...
- `#5-additional-authoring-add-verbs-on-layerblend` `IN-REVIEW` reporter — Additional evidence (broadens the affected-methods list + a new asset-class context). Two more `material.authoring` node/param verbs the `#2` fix left on the `UMaterial`-only loader still reject a valid `UMaterialFunction` path with the misleading `[ASSET_NOT_FOUND] Could not load Material.`: **`material.authoring.add_material_node`** (guilty line `MaterialAuthoringHandler.cpp:2837` `LoadObject<UMaterial>` → `:2840` `SendError(ASSET_NOT_FOUND, "Could not load Material.")`) and **`material.authoring.add_scalar_parameter`** (`MaterialAuthoringHandler.cpp:842` `LOAD_MATERIAL_OR_RETURN()` → `MaterialFinders.h:98`). Replay-confirmed live on a freshly created `UMaterialFunctionMaterialLayerBlend` (a layer-blend function, not a plain MaterialFunction — a new asset-class context for this bug): `create_material_layer_blend {name:"MLB_OracleReplay_AddNode", path:"/Game/Materials/LayerBlends"}` → success (`existsAfter:true, assetClass:"MaterialFunctionMaterialLayerBlend"`); then `add_material_node {nodeType:"BlendMaterialAttributes", x:0, y:0}` → `[ASSET_NOT_FOUND] Could not load Material.`, `add_scalar_parameter {parameterName:"MossCoverage", defaultValue:0.5, x:0, y:0}` → `[ASSET_NOT_FOUND] Could not load Material.`, while `material.graph.add_expression {expressionClass:"BlendMaterialAttributes", x:0, y:0}` on the SAME function SUCCEEDED (returned a nodeId) — so `#2`'s widening landed for `material.graph.add_expression` but the two authoring-namespace convenience/param adds remain broken. Surfaced in a layered-terrain material task (seed `material.authoring.set_material_layer_stack`, which itself worked and read back the stack correctly): the attempt reached for `add_material_node`/`add_scalar_parameter` to author the blend body, hit ASSET_NOT_FOUND on both, and only recovered by switching to `material.graph.add_expression`. Route `add_material_node` + `add_scalar_parameter` (and the remaining still-material-only convenience adds) through the same `FMaterialMutationTarget` / `LoadMaterialOrFunctionForMutationOrReportError` before DONE, or the asymmetry persists for the most obvious authoring-namespace add verbs. Ergonomic sub-note: `[ASSET_NOT_FOUND] "Could not load Material."` is misleading for these — the asset loads fine, it is simply not a `UMaterial`; the `#2` helper already returns `UNSUPPORTED_ASSET_CLASS` for the widened verbs, so routing these through it fixes the message too.
...
- `#6-two-more-verbs-and-a-consequence-not-just-friction` `IN-REVIEW` reporter — Additional evidence from authoring a viewmodel-FOV function (`/Game/FPS/Player/MF_ViewmodelFOV`) on UE 5.8 / EAContentExamples58, and the failure mode has **changed shape** since `#5`: these now answer `[UNSUPPORTED_ASSET_CLASS] Asset is not a Material. Received class: MaterialFunction` rather than `#5`'s misleading `[ASSET_NOT_FOUND] Could not load Material.` — an honest error at last, but still a refusal. Two more verbs to add to `#5`'s list, plus one read verb:

  | verb | on a `UMaterialFunction` |
  |---|---|
  | `material.authoring.add_custom_expression` | UNSUPPORTED_ASSET_CLASS |
  | `material.authoring.add_collection_parameter_node` | UNSUPPORTED_ASSET_CLASS |
  | `material.graph.get_node_details` | UNSUPPORTED_ASSET_CLASS |
  | `material.graph.add_node` | **works** (the `#2` fix reached this one) |
  | `add_function_input` / `add_function_output` | work |

  **The `add_custom_expression` refusal is worse than it looks**, because `material.graph.add_node` *will* create a `MaterialExpressionCustom` in the function — and then nothing in the plugin can set `Code`, `OutputType` or `Inputs`, which is everything that makes a Custom node do anything. So the generic add produces a permanently inert node. I finished the function through `MaterialEditingLibrary` in `python.execute` (`get_material_function_expressions` -> `set_editor_property('code'/'output_type'/'inputs')` -> `connect_material_expressions` -> `update_material_function`), then `asset.save {force:true}`; the resulting function compiles clean in a calling material (`compile_material` -> `compileSucceeded: true, compiledWithErrors: false`). Worth noting for the fix: `MaterialEditingLibrary` already exposes the function-aware pair (`create_material_expression_in_function`, `delete_material_expression_in_function`, `get_material_function_expressions`, `update_material_function`), so the engine-side capability the node API needs is present.

  **And one consequence that is a design limit, not friction:** with `add_collection_parameter_node` refusing functions, **a material function cannot read a MaterialParameterCollection at all through the plugin.** Any function that wants a global value must take it as a `FunctionInput` and have every call site wire its own `CollectionParameter` node. That is survivable — mine ended up collection-agnostic, which let me swap from my own MPC to another stream's with a single node change when we agreed a shared contract — but it was forced by the gap, not chosen, and it silently pushes N copies of the same collection wiring into N calling materials.
