...
- `#12-parameters-aspect-marks-overridden-entries` `IN-REVIEW` developer — Addresses `#6`'s named defect, which `#7`/`#8` left open: the `parameters` aspect now marks the entries it cannot speak for instead of publishing a bare number. `BuildParameterStoreArray` (`Source/PinWright/Private/Handlers/Niagara/NiagaraDumpBuilder.cpp`) takes an optional index and stamps `overridden: true` plus `override: {valueMode, value, emitter, module, input}` on every rapid-iteration entry whose module input also carries a graph override pin; `BuildParametersJson` and `BuildEmitterParametersJson` build that index by walking the same graphs and the same `NiagaraEdit::ClassifyModuleInputBindings` classification the `stack` aspect uses, so the two aspects' accounts of one input cannot drift. `#3`'s cost objection was to the *reverse* mapping (parse each `Constants.<emitter>.<module>.<input>` back to a module node); the forward direction is cheap — walk the modules, compute each input's constant name with `PinWrightNiagara::MakeRapidIterationConstantName`, key a map by it — and needs no name parsing at all. `#6`'s cheaper ask ships alongside it rather than instead of it: the document carries a `rapidIterationNote` stating that entries report the stored value, that `overridden` names a pin this store cannot see, and — `#10`'s point (b) — that a static switch can leave the constant it routed *to* absent while the one it routed *around* is still published, so a missing name is not a missing value. `niagara_parameters.json` aspect version 1 -> 2 (`Source/PinWright/Private/Handlers/Asset/AssetDumpCache.cpp`, first row this aspect has had). Docs: `docs/wiki-src/niagara.md` (`### niagara.inspect` and `### niagara.set_module_input`) and `docs/wiki-src/niagara.dump-files.md`. Note for the verifier: `#10`'s central claim — that the store is what the runtime reads — is now handled at the source by `B-module-input-override-inert-runtime-uses-rapid-iteration-default` `#3`, which makes `set_module_input` write the constant too, so after that fix a divergence can only come from pre-existing content or a direct `niagara.set_parameter`. No regression test of its own (`E-*`, and the change is a readback annotation over two production walks that already have theirs); the `B-*` sibling's test covers the shared name-construction helper. **Not compiled and not run.**
