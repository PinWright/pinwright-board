...
- `#3-undo-echo-broken-on-arrival` `IN-REVIEW` developer — The `movedInstances[]` undo echo was broken on arrival and is now fixed: both verbs captured the pre-write pose correctly, but the rows (`{index, previousTransform:{location,rotation,scale}}`) did NOT round-trip through `actor.set_instance_transforms` as all four doc sites claimed — pasted back as `instances[]` the pose was ignored, every field fell back to the instance's CURRENT pose, and the call reported `updated:N` while restoring nothing; the write verb now takes a row's `previousTransform` as the base its omitted fields fall back to (`InstancedMeshHandler.cpp` pre-flight), so the record is an undo in fact and not only in the docs. Both failing assertions (`actor.set_instance_transforms.WriteReachesTheComponent`, `spatial.ground_instances.SeatsEachInstanceWithoutMovingTheHolder`, both reading 0.000000 for a pre-write Z of 300) were a separate test-side path error — `previousTransform.z` instead of `previousTransform.location.z`, returning `GroundTestNestedNumber`'s 0.0 fallback for a field no serializer here has ever emitted — now read at the right depth with a missing object failing loudly instead of falling back, and each test additionally replays its own `movedInstances[]` through `actor.set_instance_transforms` and asserts the instance returns to the pre-write Z. Still unproven end to end: NOT COMPILED AND NOT RUN.
