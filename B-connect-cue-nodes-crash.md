...
- `#2-fix-pin-sync-and-index-guard` `IN-REVIEW` developer — Fixed the crash at its root in `Source/PinWright/Private/Handlers/Audio/AudioAuthoringHandler.cpp` (`connect_cue_nodes`): after growing/assigning `SourceNode->ChildNodes`, top up the paired `USoundCueGraphNode`'s input pins to match the new child count via a new file-local helper `SyncSoundCueGraphNodeInputPins()` (mirrors the engine's own `USoundNode::InsertChildNode` -> `USoundCueGraphNode::CreateInputPin()` pin creation) BEFORE `Cue->LinkGraphNodesFromSoundNodes()`, so the engine invariant `check(InputPins.Num() == ChildNodes.Num())` at `SoundCueGraph.cpp:61` holds instead of firing. Also hardened the same handler against a second crash vector: a negative `childIndex` is now rejected up front with `INVALID_CHILD_INDEX` (previously an out-of-bounds `ChildNodes[ChildIndex]` fault). Added `#include "SoundCueGraph/SoundCueGraphNode.h"` (the `AudioEditor` module is already a `PrivateDependencyModuleNames` entry; `CreateInputPin`/`GetInputCount` are `AUDIOEDITOR_API`). Regression test added at `Source/PinWright/Private/Tests/Assets/TestConnectCueNodes.cpp`: `PinWright.Assets.SoundCue.ConnectCueNodes.SyncsInputPinsNoCrash` drives the real registered handlers `create_sound_cue -> add_cue_node x2 -> connect_cue_nodes` through the dispatcher (fixture built entirely in-code, no external content); pre-fix the connect fatally `check()`-crashed the process, post-fix it succeeds with `GetInputCount() == ChildNodes.Num()` and slot 0 wired to the target, and a negative `childIndex` returns `INVALID_CHILD_INDEX`. Not compiled/tested here — later phase verifies.
