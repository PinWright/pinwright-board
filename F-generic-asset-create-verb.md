...
- `#2-no-route-to-uphysicalmaterial` `OPEN` reporter — Second independent instance of this gap, on a class far more ordinary than `UProceduralVegetation`: **`UPhysicalMaterial`**. The FPS build's WEAPONS stream had to create seven of them (`/Game/FPS/Core/PhysMat/PM_{Concrete,Metal,Wood,Dirt,Water,Flesh,Glass}`) as the very first deliverable that five other streams block on. Board-wide search for a route found none: `physics` exposes exactly three verbs (`activate_ragdoll`, `setup_physics_simulation`, `setup_ragdoll`) and none creates an asset; `static_mesh.set_material` and `asset.*` are the only other surfaces that mention physical materials, and both consume one rather than making one. Fallback used was `python.execute` with `unreal.AssetToolsHelpers.get_asset_tools().create_asset(name, folder, unreal.PhysicalMaterial, unreal.PhysicalMaterialFactoryNew())` — which worked, so this instance lands on the ticket's Medium reading rather than its High one. It cost roughly fifteen extra minutes of wiki search to establish that the absence was real rather than a missed page, which is the actual tax a floor verb would remove. Two follow-on notes for whoever builds `asset.create`: (a) `UPhysicalMaterialFactoryNew` has no `ConfigureProperties` modal, so it is a safe first target for the modal-bypass list this ticket asks for; (b) the created asset still could not be finished from Python — `pm.set_editor_property('surface_type', ...)` is unreachable because `unreal.PhysicalSurface` exposes only `SURFACE_TYPE_DEFAULT`, and the write had to go through `property.set` with the string `"SurfaceType3"` instead (filed separately as `F-physics-project-settings`). A generic create verb that returns only a path, with no way to set the type-defining property afterwards, would still leave this case half-served.
