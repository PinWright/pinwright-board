...
- `#2-reword-to-production-ensure` `IN-REVIEW` developer — REWORD: source-verified that the real defect is a production ensure, not test order-dependence. The 3 authoring notify handlers (add_notify/add_montage_notify/add_notify_state) fall back to instantiating the abstract UAnimNotify/UAnimNotifyState base (UObjectGlobals abstract-class ensure + notify nulled on save; reproduced live via add_montage_notify.AppliesRequestedTime). Retitled/rescoped to that production bug + the sibling missing-notify-track ensure. Fix direction: never NewObject an abstract class (name-only notify for notifies, reject for notify states), ensure the notify track exists before RefreshCacheData. Split the residual transient-AnimSequence test-fixture ensures (resample + track, driving 5 dump-parity/list tests) to new OPEN ticket F-anim-transient-sequence-fixture-ensures; the 2 non-anim tests are collateral. Severity stays Medium (matches the DONE short-class/CDO ensure-fix precedents).
