...
encounters: 2
lastSeen: 2026-07-02T06:57:38.7003967+03:00
---
...
- `#3-additional-create-time-texturepath-no-autoderive` `OPEN` reporter — Additional evidence (new angle: the shipped fix does NOT cover `add_texture_sample`'s own create-time `texturePath` path). Surfaced in a clean `material.authoring.create_decal_material` decal-material build (`/Game/Decals/M_LogoProjector`, seed `create_decal_material`, outcome clean) where the caller assigned the texture *at creation* via `add_texture_sample(texturePath=/Game/ExampleContent/Decals/Textures/T_UE_Logo_Simple_M)` — the recommended one-shot create path, not the "leave it unassigned" path of `#1`. `add_texture_sample` still stamped the default `SAMPLERTYPE_Color` (source: `MaterialAuthoringHandler.cpp:710` `Properties->SetNumberField("SamplerType", SamplerTypeFromName(SamplerType))`, and `SamplerTypeFromName` :357-367 returns `SAMPLERTYPE_Color` when `samplerType` is empty — it *never* looks at the assigned texture), so once the sample was wired into BaseColor the material compiled with `(Node TextureSample) Sampler type is Color, should be Linear Color for /Game/ExampleContent/Decals/Textures/T_UE_Logo_Simple_M`, forcing a `set_texture_sample_texture` (auto-derive) round-trip + a second compile — the exact cost the fix was meant to remove, just moved to the create path. Replay-confirmed at HEAD (fresh `/Game/OracleReplay/M_SamplerReplay`): identical texture, identical "omit `samplerType`" intent, two sibling verbs diverge — `add_texture_sample(texturePath=T_UE_Logo_Simple_M)` → `get_material_node_details` reports `samplerType: SAMPLERTYPE_Color` → wired compile `compiledWithErrors:true ["(Node TextureSample) Sampler type is Color, should be Linear Color …"]`; then `set_texture_sample_texture(nodeId, texturePath=T_UE_Logo_Simple_M)` (no `samplerType`) returns `samplerType: SAMPLERTYPE_LinearColor` and compiles clean. Note the orphaned (unwired) node compiles clean — `VerifySamplerType` only fires once the sample feeds an output, so the mismatch hides until connect. Scope for the reviewer: the implemented fix chose a distinct verb and explicitly left `add_texture_sample` unchanged (see `#2`); this occurrence argues the create-time path should ALSO auto-derive — when `texturePath` is set and `samplerType` is omitted, `add_texture_sample` should call `AutoSetSampleType()` (as `set_texture_sample_texture` does at :780) so the common one-shot "add a sample pointing at texture X" Just Works without a compile-fail + reassign round-trip. Still IN-REVIEW; this is additional required scope, not a re-open.
