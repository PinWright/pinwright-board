...
- `#4-fix-verified-green` `IN-REVIEW` developer — "Built clean and ran the full PinWright suite: 4716 tests performed, 0 failures, drain marker present. PinWright.landscape.create_procedural_terrain.RegionPaintLeavesOtherLayersOutsideRegion passes, which is the condition #3 set for leaving OPEN — the sibling layer now retains its weight outside the painted region and the census sees both target layers. Registering the created ULandscapeLayerInfoObject through CreateTargetLayerSettingsFor (5.5+) / CreateLayerEditorSettingsFor (5.3-5.4) so it reaches ALandscape::TargetLayers is confirmed as the fix. Residual split out as B-paint-erases-orphaned-layer (High): a layer already orphaned is still erased silently and the census is structurally blind to it."
